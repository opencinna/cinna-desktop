 ...
 * carries a typed handback note. A canned coordinator issues actual handoff /
 * finish tool calls. Neither model history nor the runner is patched by tests.
...
interface ModelBody { stream?: boolean; messages?: { role: string; content?: unknown }[];
  tools?: { function: { name: string } }[] }

async function coordinator() {
  const requests: ModelBody[] = []
  ...
  const server = createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (!body.stream || !['handoff', 'finish'].every((name) => body.tools?.some((tool) => tool.function.name === name))) {
      ...
      const control = (name: string, args: unknown): void => {
        ...
        const chunk = (delta: unknown, reason: string | null = null): void => {
          res.write(`data: ${JSON.stringify({ id: `manifest-round-${index}`, object: 'chat.completion.chunk', model: MODEL, created: 1,
            choices: [{ index: 0, delta, finish_reason: reason }] })}\n\n`)
        }
        chunk({ role: 'assistant', tool_calls: [{ index: 0, id: `manifest-call-${index}`, type: 'function',
      ...
      else { violations.push(`Unexpected extra model request ${index}`); control('finish', { summary: 'Unexpected continuation.' }) }
    })
  })
...
for (const typed of [true, false]) {
  test(typed ? 'an explicit coordinator manifest handover carries a typed note into the next owner turn' :
    ...
    try {
      ...
      const history = fake.requests[1].messages ?? []
      expect(history.some((message) => typeof message.content === 'string' && message.content.includes(notice))).toBe(true)
      ...
      expect(task).toMatchObject({ chatId, status: 'in_progress', assignee: { kind: 'model' }, runtime: { ownerTurns: 3 } })
