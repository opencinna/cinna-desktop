...
test('remote work with no ask or job opens from Tasks and a delegated child opens through its parent', async ({ cinna }) => {
  ...
  try {
    ...
    const taskList = cinna.page.getByRole('region', { name: 'Recent tasks', exact: true })
    await expect(taskList).toBeVisible()
    const rootLink = taskList.getByRole('button', { name: ROOT_TITLE })
    await expect(rootLink).toBeVisible({ timeout: 20_000 })
    await expect(taskList.getByRole('button', { name: CHILD_TITLE })).toHaveCount(0)
    await expect(taskList.getByRole('button', { name: FINISHED_TITLE })).toHaveCount(0)
    ...
    await subtasks.getByRole('button', { name: FINISHED_TITLE }).click()
    ...
    await expect(cinna.page.getByRole('heading', { name: ROOT_TITLE, level: 1, exact: true })).toBeVisible()
    await subtasks.getByRole('button', { name: CHILD_TITLE }).click()
