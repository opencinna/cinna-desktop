...
test('agent sidebar opens chat first and settings returns to the same agent composer', async ({ cinna }, testInfo) => {
  ...
  await expect(page.getByRole('dialog', { name: 'Chat routing' })).toContainText('This computer')
  await expect(page.getByRole('dialog', { name: 'Chat routing' })).toContainText(agent.path)
  await expect(page.getByRole('dialog', { name: 'Chat routing' }).getByRole('group', { name: 'Runtime summary' })).toBeVisible()
...
test('A2A agents use the same chat-first page and structured settings', async ({ cinna }, testInfo) => {
  ...
  await expect(page.getByRole('dialog', { name: 'Chat routing' })).toContainText('A2A')
  await expect(page.getByRole('dialog', { name: 'Chat routing' })).toContainText('agent.example.com')
  await page.screenshot({ path: testInfo.outputPath('a2a-connection-tooltip.png') })
  await page.keyboard.press('Escape')
  await expect(page.getByRole('dialog', { name: 'Chat routing' })).toHaveCount(0)
  await page.getByRole('button', { name: 'Settings', exact: true }).click()
