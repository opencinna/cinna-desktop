...
test('agent sidebar opens chat first and settings returns to the same agent composer', async ({ cinna }, testInfo) => {
  ...
  await page.getByPlaceholder('Type a message...').fill('A draft to keep while changing settings')
  ...
  await page.getByPlaceholder('Type a message...').hover()
  ...
  await expect(page.getByPlaceholder('Type a message...')).toBeVisible()
  await expect(page.getByPlaceholder('Type a message...')).toHaveValue('A draft to keep while changing settings')
...
test('A2A agents use the same chat-first page and structured settings', async ({ cinna }, testInfo) => {
  ...
  await page.getByPlaceholder('Type a message...').fill('Keep this external-agent draft')
  ...
  await page.getByRole('button', { name: 'Start chat', exact: true }).click()
  await expect(page.getByPlaceholder('Type a message...')).toBeVisible()
  await expect(page.getByPlaceholder('Type a message...')).toHaveValue('Keep this external-agent draft')
