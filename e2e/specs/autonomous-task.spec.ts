...
interface ModelBody { stream?: boolean; tools?: { function: { name: string } }[];
...
async function serve(interrupt: boolean): Promise<{
  host: string; server: Server; model: ModelBody[]; agents: AgentCall[]; violations: string[];
  releaseFirst: () => void; releaseFinish: () => void
}> {
  let host = ''
  let first = (): void => { throw new Error('Initial model request is not ready') }
  let final = (): void => { throw new Error('Final model request is not ready') }
  ...
  const server = createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (req.method !== 'POST' || path !== '/v1/chat/completions') { res.statusCode = 404; send({}); return }
      ...
      const validTools = CONTROLS.every((name) => tools.filter((tool) => tool === name).length === 1)
      if (!body.stream || !validTools) violations.push(`Model request ${index} is missing coordinator controls`)
      const control = (name: string, args: unknown): void => {
        res.writeHead(200, { 'content-type': 'text/event-stream', 'cache-control': 'no-cache' })
        const chunk = (delta: unknown, reason: string | null = null): void => {
          res.write(`data: ${JSON.stringify({ id: `autonomous-round-${index}`, object: 'chat.completion.chunk', created: 1, model: MODEL,
            choices: [{ index: 0, delta, finish_reason: reason }] })}\n\n`)
        }
        chunk({ role: 'assistant', tool_calls: [{ index: 0, id: `autonomous-call-${index}`, type: 'function',
      ...
      if (index === 1) {
        ...
        if (![ANALYSIS, WRITTEN, `${WRITER} handed the task back to the coordinator.`].every((marker) => history.includes(marker))) {
          violations.push('Completed specialist handback/history missing')
        }
        final = () => control('finish', { summary: SUMMARY })
      } else { violations.push(`Unexpected model continuation ${index}`); control('finish', { summary: 'Unexpected extra continuation.' }) }
    })
  })
  await new Promise<void>((resolve, reject) => { server.once('error', reject); server.listen(0, '127.0.0.1', resolve) })
  host = `http://127.0.0.1:${(server.address() as AddressInfo).port}`
  return { host, server, model, agents, violations, releaseFirst: () => first(), releaseFinish: () => final() }
}
...
async function start(cinna: CinnaApp, fake: Awaited<ReturnType<typeof serve>>): Promise<string> {
  ...
  await expect.poll(() => fake.model.length).toBe(1)
...
test('an autonomous coordinator delegates, survives a durable Inbox gate, hands off and finishes with chat closed', async ({ cinna }) => {
  ...
  try {
    ...
    await expect(cinna.page.getByRole('heading', { name: 'Inbox', exact: true })).toBeVisible()
    await expect.poll(() => fake.model.length).toBe(4)
    ...
    expect(await cinna.page.evaluate((id) => window.api.tasks.get(id), taskId))
      .toMatchObject({ status: 'in_progress', assignee: { kind: 'model' }, runtime: { ownerTurns: 4 } })
    fake.releaseFinish()
    await expect.poll(() => cinna.page.evaluate((id) => window.api.tasks.get(id), taskId))
    ...
    expect(fake.model).toHaveLength(4)
...
test('an interrupted autonomous model round requires explicit resume without repeating completed delegation', async ({ cinna }) => {
  test.setTimeout(120_000)
  const fake = await serve(true)
  try {
    const chatId = await arrange(cinna, fake.host)
    const taskId = await start(cinna, fake)
    await expect.poll(() => fake.model.length).toBe(2)
    expect(fake.agents.map((call) => call.agent)).toEqual([ANALYST])
    await cinna.relaunch()
    await cinna.skipOnboarding()
    await openTask(cinna)
    ...
    await resume.click()
    await openInbox(cinna)
    await expect.poll(() => fake.model.length).toBe(3)
    expect(fake.violations).toEqual([])
    expect(fake.agents).toHaveLength(1)
    fake.releaseFinish()
    await expect.poll(() => cinna.page.evaluate((id) => window.api.tasks.get(id), taskId))
    ...
    expect(fake.model).toHaveLength(3)
    expect(fake.agents).toHaveLength(1)
  } finally { fake.server.closeAllConnections(); await new Promise<void>((resolve) => fake.server.close(() => resolve())) }
})
