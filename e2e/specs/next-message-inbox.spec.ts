...
test('a plain A2A next-message ask survives restart and a second ask gets a new Inbox address', async ({ cinna }) => {
  ...
  try {
    ...
    const [firstAsk] = await cinna.page.evaluate(async () => (await window.api.inbox.list()).entries)
    ...
    const [afterRestart] = await cinna.page.evaluate(async () => (await window.api.inbox.list()).entries)
    ...
    await expect.poll(async () => (await cinna.page.evaluate(async () => (await window.api.inbox.list()).entries)).map((entry) => entry.request))
      .toEqual([expect.objectContaining({ kind: 'question', questions: [expect.objectContaining({ question: QUESTION_TWO })] })])
    const [secondAsk] = await cinna.page.evaluate(async () => (await window.api.inbox.list()).entries)
