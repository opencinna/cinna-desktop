...
Scans all markdown files in docs/, CLAUDE.md, .claude/agents/, and
.claude/commands/ and verifies that file path references to src/ and
docs/ actually exist in the project.

Usage:
    python3 .cinna-core-kit/scripts/check_docs_references.py [--verbose]
    python3 .cinna-core-kit/scripts/check_docs_references.py --files docs/chat/messaging/messaging.md
...
Suppressing false positives:
    1. Pattern-based (automatic) — path segments that look like template/direction
       placeholders are skipped automatically:
         - starts with 'your_'  → your_hook.ts, your_adapter.ts
         - starts with '$'      → $param.tsx
         - starts with 'entit'  → entity.ts, entities.tsx (case-insensitive)
         - contains '...'       → abbreviated paths
         - contains '[' or ']'  → generic placeholder syntax

    2. Inline annotation — add <!-- nocheck --> anywhere on a line to skip all
       reference checks on that line:
         ### Example (`src/main/llm/example.ts`) <!-- nocheck -->
...
def find_markdown_files(docs_dir, exclude_dirs=None):
    """Recursively find all .md files under a directory, optionally excluding subdirectories."""
...
def is_skippable_path(path):
    ...
    Skips:
    - Paths with '...' ellipsis (abbreviated paths)
    - Paths with '[' brackets (placeholder syntax)
    - Paths where any segment is a template/direction placeholder:
        - Starts with 'your_'  (e.g. your_adapter.ts)
        - Starts with '$'      (e.g. $param.tsx)
        - Starts with 'entit'  (e.g. entity.ts — case-insensitive)
...
def extract_backtick_references(line):
    ...
    Matches patterns like:
      ...
      `src/main/db/client.ts:runMigrations()`
      `src/renderer/src/components/chat/ChatInput.tsx`
      `docs/chat/messaging/messaging.md`
    """
    refs = []
...
def extract_markdown_link_references(line, source_file, project_root):
    ...
    Matches: [text](relative/path.md) or [text](src/main/foo.ts)
