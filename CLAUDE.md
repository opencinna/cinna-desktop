...
- `make contract ENGINE=codex` — interface contract against the real pinned Codex CLI with a fake provider (not part of `npm test`; its ratchet is); versions live only in `src/shared/runtimePins.ts`, and moving one follows `docs/development/runtime_pins/runtime_pins_llm.md`
