...
- **MCP Chip** — A pill rendered with the other capability chips below the composer, one per MCP active in the chat. On-demand engagements are removable (`×` detaches them); baseline servers (from the chat mode) are shown **locked** (no `×`) so the strip reflects everything the runtime actually gets, not just the user's own picks. The strip is the only place a chat's baseline can be seen; it is changed on the chat mode.
