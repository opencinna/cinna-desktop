...
- `src/main/mcp/manager.ts:bearerAuthHeaders()` — decrypts static tokens for SSE/HTTP. Both remote transports also support native OAuth, restore SDK state, and complete browser callbacks using the matching transport. Calls serialize per provider to protect rotating refresh tokens; queued calls refuse a replaced connection.
...
- `src/main/mcp/oauth-callback.ts:startOAuthCallback()` — binds loopback before exposing `/oauth/callback`, reusing the registered port when available. An occupied port triggers a fresh registration. The default consent window is ten minutes. Invalid state/duplicate parameters return 400 and keep the real flow pending. Valid callbacks preserve all parameters for SDK validation; timeout/abort close the listener. Cinna account OAuth shares the listener.
...
- Full client registrations, including unexpected client secrets, persist in an encrypted envelope in `client_info`; tokens are encrypted separately. Boot cleanup encrypts legacy plaintext registrations. Discovery metadata remains JSON. Native public-client registration requests `token_endpoint_auth_method: none`; no hosted CIMD URL is configured. Endpoint/auth changes clear OAuth state.
...
`mcp:status-changed` is the main-to-renderer connection-status notification; the renderer invalidates the provider query when it arrives. The Transport selector remains visible for every transport so an edit can be reversed.
