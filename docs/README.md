...
- [Orchestrated Agents](chat/orchestrated_agents/orchestrated_agents.md) — Agents-as-MCP: when a chat mixes ≥2 counterparties (LLM + agent, two agents, or agents + MCPs) the local model conducts and calls each agent as an emulated MCP tool, rendering each as an expandable sub-thread; a lone agent still talks direct A2A. `A2A`/`AI` badge on the composer
...
- [Agent Notices](chat/agent_notices/agent_notices.md) — Agent-side system messages (`cinna.content_kind: 'notice'` TextParts) stream live as muted system pills, then collapse to a clickable accent dot once persisted as `agent_transition` rows. Excluded from catch-up replay and LLM history rebuilds
