...
- Developer Tools includes the resolved OpenCode version and editable OpenCode Path, and the same pair for Codex. **The Codex row reports the Codex Cinna runs, not the `codex` on the user's PATH.** Every Codex session runs on the pinned copy Cinna downloads and verifies, so the PATH copy — still what **Open in Codex** launches — says nothing about whether an agent can run, and reporting it here would describe a binary no session uses. The cell reads `<version> managed`, `<version> unverified` for a configured path, *Installs on first use*, *Checking…* / *Downloading…* or *Unavailable*, with a failed install's sentence under the table.
...
- Both path fields are one component: committed on blur or Enter, discarded on Escape, nothing validated per keystroke, and every message a consequence of an action rendered below the control and last in the card, so its arrival moves nothing above it.
...
- [The Codex Engine](../../agents/local_agents/codex_engine.md) — the managed Codex CLI behind the Runtime row's Codex status line (*Install now* / *Try again*), the Developer Tools Codex row and the Codex Path
