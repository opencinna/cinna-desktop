...
- Developer Tools includes the resolved OpenCode version and editable OpenCode Path. Its Cinna CLI row prefers the installed desktop-managed executable, labels it **(managed)** and shows its actual version/path; a shell CLI is shown only when no managed result is available. OpenCode edits survive refresh and failed saves, including an empty draft; Escape discards.
