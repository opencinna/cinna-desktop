...
- **Default → Local Development** shows the desktop-managed CLI version and binary, terminal PATH integration, and Developer Tools. The installed CLI is read from the desktop toolchain independently of the active account's workspace readiness. A build compatibility blocker routes here and names the JSON workspace minimum, **0.4.0**, the installed version and any need for the server administrator to advance its CLI pin.
...
- Developer Tools includes the resolved OpenCode version and editable OpenCode Path, and the same pair for Codex and for Claude Code, each row built like the one beside it. **The Codex and Claude Code rows report the CLI Cinna runs, not whatever is on the user's PATH.** Every session runs on the pinned version — the copy Cinna downloads and verifies, or the user's own install when it reports exactly that version — so a PATH copy at any other version, still what **Open in…** launches, says nothing about whether an agent can run, and reporting it here would describe a binary no session uses. The cell reads `<version> managed`, `<version> (your install)`, `<version> unverified` for a configured path, *Installs on first use* (*Checked on first use* with a path saved), *Checking…* / *Downloading…* or *Unavailable*. **A runtime's failure is said under its own path field**, one truncated line with the whole sentence on hover: the three used to be stacked as unlabelled red paragraphs above all three fields, where pressing Enter on a bad path moved the field being edited down a line.
- **Claude Path** is the same field for Claude Code: empty in the normal case, an absolute path replaces the pinned CLI and is **not verified**, and a saved or cleared path takes effect at once, dropping the remembered login verdict with it. It is for a platform Cinna has no build for — Windows — or for trying a newer Claude Code; the Claude login is the same either way. See [The Claude Engine](../../agents/local_agents/claude_engine.md#which-claude-runs).
- **Codex Path** is empty in the normal case. An absolute path replaces the managed copy and is **not verified**: it runs folder agents at whatever version it is, while plain Codex chats and AI Functions refuse any version but the pinned one. It is for a platform Cinna has no build for, or for trying a newer Codex; the login is the same either way. Its tip says all of this before the user sets it. A saved or cleared path takes effect at once — the binary is resolved again and the rows follow — where a saved OpenCode Path waits for the next turn and says so in place.
- All three path fields are one component: committed on blur or Enter, discarded on Escape, nothing validated per keystroke, and every message a consequence of an action rendered below the control and last in the card, so its arrival moves nothing above it.
- The Developer Tools Cinna CLI row prefers the installed desktop-managed executable, labels it **(managed)** and shows its actual version/path; a shell CLI is shown only when no managed result is available. Path edits survive refresh and failed saves, including an empty draft; Escape discards.
- **Refresh** rereads detected tools, managed CLI and the connected server's advertised target. **Update** appears for a recognized newer server target and updates the shared managed tooling; it does not select PyPI latest, downgrade, replace an editable checkout or treat an unknown installed version as absent. Pending feedback is a disabled **Updating…** spinner and status line. Success refreshes the readouts; failures stay inline with retry available.
- CLI Update preserves unanswered/declined setup and shared host consent. It performs no Agents Home preparation, workspace setup or account-token work. An existing ready context is refreshed after success; a failure there requires toolchain attention. **Set up**, **Repair** and — for a workspace that belongs to another Cinna account — **Reconnect workspace** remain Profile actions. Reconnect is the primary button for that one reason and Repair is demoted beside it, because Repair cannot clear it.
- Build-page and runtime-details **Check again** explicitly rereads managed CLI capabilities without installing or changing consent. **Checking…** remains visible through the check and at least 600 ms for an immediate result; duplicate clicks are blocked. Capability failures are reported as check errors, never presented as evidence of an old CLI.
...
- [The Codex Engine](../../agents/local_agents/codex_engine.md) — the managed Codex CLI behind the Runtime row's Codex status line (*Install now* / *Try again* / *Fix path*), the Developer Tools Codex row and the Codex Path
- [The Claude Engine](../../agents/local_agents/claude_engine.md) — the pinned Claude Code behind the same three surfaces for Claude Agent, and the Claude Path
