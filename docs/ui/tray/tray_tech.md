...
- `src/renderer/src/components/settings/FeaturesSettingsSection.tsx` — Features tab. Two `SettingsSection`s: **AI Functions** (`autoChatTitles`, `prioritizeAccountDefaults`) and **Interface** (`enableTrayIcon`, `showHints`), each a `SettingsRows` list of the shared `SettingsToggleRow`, with each toggle's description behind its (?); toggles go through `useSetAppSetting`.
