...
- `src/renderer/src/components/settings/FeaturesSettingsSection.tsx` — Features tab. Hosts the **Enable Tray Icon** toggle in Interface through `useSetAppSetting`; appearance controls in the same group use the renderer UI store. See [Settings](../settings/settings_tech.md) for the full section inventory.
...
- `src/renderer/src/trayPanel.tsx` — popup React root. Uses `readThemePreference`/`resolveTheme` from `src/renderer/src/utils/theme.ts` at bootstrap, on `cinna-theme` storage events and on `prefers-color-scheme: dark` changes; System resolves live while fixed preferences remain fixed. See [Appearance](../appearance/appearance_tech.md). It bootstraps `auth.store.currentUser` via `window.api.auth.getCurrent()` on mount and on every window `focus` (own store instance, created hidden before activation); mounts `<QueryClientProvider><TrayPanel/></QueryClientProvider>`.
