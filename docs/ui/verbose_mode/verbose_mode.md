...
A global UI preference that toggles between a **compact** (default) chat view — no timestamps, no meta — and a **verbose** view that shows a subtle relative-time footer with an info icon beneath every message and keeps streaming tool narration auto-expanded.

## Core Concepts

- **Compact mode** — The default reading experience. Message bubbles render with no footer, no timestamps, no metadata icons. Streaming tool narration blocks stay collapsed while the model is speaking; thinking blocks are open, as in verbose mode. Tool narration headers fall back to the minimal `Tool: <name>` label even when the backend supplied structured tool input.
- **Verbose mode** — User-enabled display mode. Adds a small footer (`X minutes ago` + info button) below every message, restores the previous auto-expand behaviour for streaming tool narration blocks, surfaces structured tool input (`name(arg: value, …)`) inline in tool narration headers when the backend provides it, and exposes all available message metadata via a popup.
...
2. **Thinking, in both modes**: each `ThinkingBlock` mounts open, live and persisted, with its body height-capped and following its newest line while it streams. It is never folded into a dots group — see [Conversation UI](../../chat/conversation_ui/conversation_ui.md).
3. **Compact mode**: each streaming `ToolNarrationBlock` mounts collapsed. User can click the header to expand manually. Content still streams in live behind the header; the pulsing accent dot indicates activity.
4. **Verbose mode**: the currently-last streaming tool narration block auto-expands so the user sees it as it arrives (legacy behaviour). When the stream completes and the persisted message re-renders, the block collapses again.
...
- `ToolCallBlock` (actual MCP tool invocations) is not affected by the verbose / compact toggle — it is always collapsed by default regardless of mode and always shows the structured `ToolCallSummary` in its header. Only `ToolNarrationBlock` changes behaviour; `ThinkingBlock` is open in both modes.
- `ToolResultBlock` (A2A `tool_result`-kind parts) intentionally diverges from `ToolNarrationBlock` during streaming: it defaults to **expanded** in both modes because the output is the payload the user is waiting on (especially for `/run:*` CLI commands), not auxiliary narration. Persisted reload falls back to the default-collapsed behaviour to keep long stdout/stderr from crowding scrollback.
...
Sidebar (bottom bar)
  └── verbose toggle ─── toggleVerboseMode() ──► ui.store
        ...
        MessageStream (per persisted msg)                        MessageStream (streaming blocks)
                     │                                                              │
                     ▼                                                              ▼
     MessageMetaFooter (when verboseMode)            ToolNarrationBlock (ThinkingBlock: always open)
