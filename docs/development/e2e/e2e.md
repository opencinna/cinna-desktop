...
- `e2e/specs/run-events.spec.ts` — a folder agent that parks mid-turn on a permission ask and then on a question, run on the fake engine: the block becomes answerable from the stream's `needs_input` alone (the registry poll is pinned empty), the answer reaches the engine as OpenCode's own body, and the persisted decision renders read-only after a restart. A `test.fail()` pins that an answered question's answer is never shown
- `e2e/fixtures/fakeEngine.mjs`, `e2e/fixtures/fakeEngine.ts` — a scripted stand-in for `opencode serve`, installed through the engine path setting so the real engine manager spawns it; logs every request it is sent
