...
- `e2e/specs/ipc-wire.spec.ts` — section A of the manual plan: the observed IPC error wire string; and the single stream guard — a stubbed sender posts every `RunEvent` type interleaved with off-contract messages onto the real turn port, and both `llm.sendMessage` and `agents.sendMessage` deliver exactly the eleven types (a wrong-payload `delta` included, since the guard reads the discriminator only) while each dropped message is warned about
- `e2e/specs/run-events.spec.ts` — a folder agent that parks mid-turn on a permission ask and then on a question, run on the fake ACP agent: the block becomes answerable from the stream's `needs_input` alone (the registry poll is pinned empty), the answer reaches the agent as ACP's own outcome (`{outcome:{outcome:'selected',optionId:'once'}}` / `{action:'accept',content:{question_0:'Teal'}}`), and the persisted decision renders read-only after a restart. A `test.fail()` pins that an answered question's answer is never shown
- `e2e/specs/inbox.spec.ts` — the Inbox: a job whose folder agent parks on a permission ask, run from the job screen's **Run** (an `execute` over IPC starts no stream and so raises no ask), the user leaving the chat for the sidebar's Inbox, answering it there with the conversation closed — the answer reaching the agent as ACP's own `{outcome:{outcome:'selected',optionId:'once'}}` — and the decision waiting in the transcript when the chat is reopened from the run row. The task's whole trail is asserted, `in_progress → blocked → in_progress → completed`, made observable by a fake agent that works for a couple of seconds either side of its ask
...
- `e2e/specs/driver-readiness.spec.ts` — the composer refuses a direct send to an agent its driver called not ready: a hand-added A2A agent on a closed loopback port reads `unreachable` on `agents.list()`, Settings → Agents shows the reason beside Test Connection with a danger dot, the new-chat composer (routing badge `Direct A2A connection`) disables Send with the reason as its description and offers Check again, and Enter creates no chat and keeps the text; a fake A2A agent then starts on the same port, Check again clears the refusal, and the send creates the chat and shows the fake's reply
...
- `e2e/specs/bare-agent.spec.ts` — adopting a folder that already holds an `AGENT.md`: the + → the choice → the stubbed picker → the preview of what was found (three agents, then one), the sidebar group under the picked folder's name, a refusal that keeps the dialog open and names `AGENT.md`, the bare agent's page (no Commands tab; the folder's `README.md` rendered read-only on Overview, `Instructions` alone on Prompts), and "Remove from the list only" — gone from the list and from the next rescan, folder untouched, put back from Settings. Every adopt test snapshots the folder tree before and after: the promise is that nothing is written into the user's own folder, and no screen in the flow would show it if something were
