...
- `e2e/specs/remote-agent-actions.spec.ts` — the ⋯ menu on a Cinna-hosted agent's page: exactly `Open on the server` then `Disable in Desktop App`, with no `Delete agent…` and no `Uninstall agent…`, and the link the app asks the OS to open (`shell.openExternal` replaced in main). A hand-added A2A row in the same linked profile still offers `Delete agent…` and no server link. The remote row is written by the product's own `agents.syncRemote()` against a one-route loopback listing the spec owns; no live Cinna server and no browser.
