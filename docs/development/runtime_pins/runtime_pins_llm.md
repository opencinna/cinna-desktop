...
| `make contract-next ENGINE=<engine> VERSION=x.y.z` | Level 1 against a candidate, pin untouched. **The candidate's download URL is not looked up; it is the pinned asset URL with the version swapped in** (`candidateUrl` in `scripts/install-runtime.mjs`: `rust-v<pin>` → `rust-v<new>` for Codex, `/<pin>/` → `/<new>/` for Claude), on the assumption that a release keeps the same file names. That has only been exercised against *older* releases, so when a newer release will not install, check this first. Prints the snapshot diff pinned → candidate; the candidate snapshot and the diff go to `$TMPDIR/cinna-contract-snapshots/<engine>-<version>.{json,diff}`. **Fails only when a contract test fails** (or the candidate cannot be installed, which prints `could not install`) — a diff alone is success, and a failing test still prints the diff unless the run broke before anything was observed. Exercised for real 2026-09-18 against the previous releases, since both pins were the newest published: Codex 0.154.0 and Claude 2.1.267, 26 passed each |
...
| Step | What happens |
|---|---|
| **A** | Plain chat on the Default runtime (a restricted, chat-owned conductor, no AI credential), then a title replaces the derived one — on Codex its own thread title (Cinna's AI title must **not** run for that chat, and the E2E asserts no request carried the title prompt), on Claude Cinna's AI title |
...
On Codex, B and D also assert that the conductor's Cinna tool calls raised **no** permission ask: a conducting session allows those itself when Cinna offered the tool to it ([permissions](../../agents/local_agents/permissions.md#a-session-conducting-a-chat-is-not-asked-about-cinnas-own-tools)), and one Allow button reaching the screen is a failure.

**Step E — cancel — is in the billed live flow only**, not in `FLOW_STEPS` (no contract entry maps to it) and not in the E2E spec. E1: Stop a plain chat mid-reply (a 500-line count): the turn ends promptly, the partial reply is kept as an ordinary reply with no error row or alert, nothing streams afterwards, the runtime acknowledges the cancel rather than being retired, and a follow-up in the same chat is answered. E2: Stop a coordinator chat while its specialist (`Slow Worker`, a folder whose one job is `sleep 67`) is inside its call: conductor and specialist turns both end, the command's process is gone, and a follow-up is answered. Each verdict carries the Stop→end timings and the main-process cancel lines (`E1-cancel-log.txt`, `E2-cancel-log.txt` in the results).
- **Verified on real Codex 2026-09-18 (`ONLY=e`): 19/19 verdicts (17 of them step E's).** Stop→turn end ≈225 ms for the conductor and the specialist alike, `stopReason: cancelled`, process kept.
- **A stopped reply lives in Cinna's transcript, not in Codex's thread.** Asked afterwards what number it had reached, Codex answered "0", twice: the partial text the user saw is not in the history the CLI remembers. Observed, not asserted — E1's follow-up verdict only checks that an answer arrives.
...
8. On the real binary: `node --experimental-strip-types scripts/probes/runtime-conductor-codex.mjs <abs path to the new codex>`, then `make live-flow ENGINE=codex CONFIRM=1` — billed, so the user runs it or approves it. Nothing automated covers a paid model, a real login, the reviewer or the sandbox. Claude has no probe script and no no-billing flow: `make live-flow ENGINE=claude CONFIRM=1` is its whole-flow check, and its `live` entries whose `flow` is `none` are walked by hand — the managed binary reports the user's login with no Keychain prompt, and a session does not move `~/.local/bin/claude`. The second is meaningful only while the pin is older than the latest release: an older binary in a sandbox HOME laid out as a native install (`.local/share/claude/versions/<v>`, `.local/bin/claude` linked to it), the `buildClaudeEnv` environment, a fake provider and a recording pass-through proxy, 120 s, with and without `DISABLE_AUTOUPDATER` — never the real `~/.local/share/claude`. Its 2026-09-18 result is in the `claude.launch.autoupdater-disabled` entry: ACP sessions never update or contact `downloads.claude.ai` even without the variable; interactive `claude` updates in ~15 s without it and not with it. Users whose own install is the *old* pin stop matching after the bump and get the download on their next turn; the superseded managed copy is swept only after the new one runs, and not within 7 days of use
