...
- **External CLIs** are never packaged. Claude's is named by `CLAUDE_CODE_EXECUTABLE` and Codex's by `CODEX_PATH`, and each is the pinned CLI the app resolves at run time: the user's own install when it reports exactly the pinned version, otherwise a copy downloaded and verified into `<userData>/runtimes/` on first use (Claude Code's is a bare 215–232 MB executable, which is a reason not to put it in an installer, not a reason to), or an explicit path from Settings — see [The Claude Engine](../../agents/local_agents/claude_engine.md#which-claude-runs) and [The Codex Engine](../../agents/local_agents/codex_engine.md). Shipping the JavaScript adapter and SDK does not require shipping their fallback CLI binaries; the managed Codex CLI is a run-time download, not a packaged resource.
