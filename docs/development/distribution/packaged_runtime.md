...
- **External CLIs** are never packaged. Claude's is the user's own, named by `CLAUDE_CODE_EXECUTABLE`. Codex's is named by `CODEX_PATH` and is the pinned CLI the app downloads and verifies into `<userData>/runtimes/` on first use, or an explicit Codex Path — see [The Codex Engine](../../agents/local_agents/codex_engine.md). Shipping the JavaScript adapter and SDK does not require shipping their fallback CLI binaries; the managed Codex CLI is a run-time download, not a packaged resource.
