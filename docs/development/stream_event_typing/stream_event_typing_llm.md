...
- `user_message { text }` — ACP driver only: a message the user sent while the turn ran, which the engine took in. Posted when the engine confirms, at the point in the output where it landed; the accumulator starts a new part after it and the turn persists it as a user row there. Not posted for a message the engine confirmed only after the turn stopped waiting — main saves that one itself. See [Pending Messages](../../chat/pending_messages/pending_messages.md)
