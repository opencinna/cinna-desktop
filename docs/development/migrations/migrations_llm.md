...
- Driver legacy backfill belongs only to `src/main/db/migrations/agent-drivers.ts`, followed by `src/main/db/migrations/acp-driver.ts`. New writes name a driver. No agents-driver-populated boot heal remains: an unknown/null value during ordinary reads stays visible and refuses execution instead of guessing transport from source. Unknown non-null IDs are never overwritten by the backfill.
...
19. `migrateChatRouter` (`migrations/chat-router.ts`) — `chats.router` + `chat_agent_cursors`.
    ...
    - After this guarded legacy backfill, `src/main/db/migrations/retire-chat-mirror.ts` drops orchestrated only when table/router/mirror exist. migrateChats never recreates it. Existing router values win over stale mirror values. Cover fresh/absent, populated legacy, contradictory mirror, repeated startup and foreign keys.
