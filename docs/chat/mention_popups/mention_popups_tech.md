...
| `src/renderer/src/components/chat/ComposerPlusMenu.tsx` | The `[+]`-button consumer for the chat-modes selector (successor to the retired `ChatConfigMenu`). Its **Chat mode** sub-view renders the mode rows inline with `renderIcon` supplying a per-mode colored dot (the mode's preset border color). Mouse-only — the keyboard path is the `~` popup that `ChatInput` owns. |
...
| `src/renderer/src/components/layout/ChatWorkspace.tsx` | Owns `tildeModePopupOpen`, passed to `ChatInput.tildeModePopup.open`; the `[+]` chat-mode submenu owns its own open state inside the composer. Provides the `renderIcon` / `composeSecondary` callbacks that keep `ChatInput` agnostic of chat-mode visuals. Wraps mode selection with via-tilde variants that also `chatInputRef.clearInput()` the leftover `~`. |
