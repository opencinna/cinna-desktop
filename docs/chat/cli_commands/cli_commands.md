...
- **CLI commands bypass [Multi-Agent](../multi_agent/multi_agent.md) Smart Rewrite and catch-up replay.** The composer recognises any message whose trimmed text starts with `/run:` via `isCliCommand` (`src/shared/cliCommands.ts`) and dispatches it unmodified — rewriting would destroy the rigid `/run:<slug>` syntax, and prepending a catch-up transcript would shift the invocation off the start of the wire content and cause the backend to route it to the LLM instead of executing the shell command. The (chat, agent) catch-up cursor still advances after the send so the next non-CLI engagement of the same agent picks up the correct window.
