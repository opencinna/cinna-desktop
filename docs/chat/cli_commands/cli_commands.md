...
- **CLI commands are dispatched verbatim.** A `/run:<slug>` message is only meaningful in a direct-A2A chat — it streams unmodified to the bound agent, whose backend parses the `/run:*` invocation server-side without an LLM turn. The composer applies no transformation to the wire content. The `/` popup only surfaces for the bound or selected agent, so it does not appear in orchestrated chats (which have no single bound agent).
