...
| File | Role |
|------|------|
| `src/renderer/src/hooks/useChatStream.ts` | `handleRun` forwards every `delta` event to `chat.store.appendDelta` regardless of `kind`. `RunDeltaEvent.kind` is typed as `ContentKind` (which includes `'command_result'`) — no per-kind branching in the hook. |
| `src/renderer/src/stores/chat.store.ts` | `appendDelta` accepts `kind: 'command_result'` and stores it in `streamingBlocks` as a `TextBlock { kind: 'command_result' }`. The shared merge rule (`continuesPart` in `src/shared/partMerge.ts`: same kind + matching `toolName` — both `undefined` for command_result) groups consecutive deltas into one block. |
...
- `src/main/agents/streamPartsAccumulator.ts::StreamPartsAccumulator.appendToList()` — Merges through `continuesPart` (`src/shared/partMerge.ts`); for command_result that is same kind + equal `toolName` — both `undefined` for command_result, so consecutive same-kind deltas always merge. Same rule as `text` / `thinking`.
...
- `src/renderer/src/components/chat/MessageStream.tsx` — Module-level `pairCommandTools(items)` returns `{ pairResultIdx, consumed }`: maps each command-tool index to its paired tool_result index by `toolId`. It also consumes two results that belong to a local agent's request block rather than to a command: a `per_*`/`que_*` ask's decision, and the `AskUserQuestion` call a question was raised from, together with its restatement of the answer (see [Ask User Question tech](../ask_user_question/ask_user_question_tech.md)). Used in all three render sites:
