...
- The body is rendered through the same `react-markdown` + `remark-gfm` + `rehype-highlight` plugins as `MessageBubble`, with the shared `markdownComponents`, so markdown structure (lists, headers, code, tables) in `/files` / `/agent-status` output reads naturally. It does not get `MessageBubble`'s nested code fence repair: that repair is for model prose, and command output is platform text rendered as written.
