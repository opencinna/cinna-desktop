...
- Internal development builders show a **Local** tooltip with Driver (**Claude Code**, **Codex** or **OpenCode**), **ACP · stdio**, **This computer** and **Local Development** settings ownership. The driver is the saved builder engine; changing today's default cannot relabel an old session. Missing/invalid saved engine reads **Not recorded**. The tooltip uses public agent fields and never requests the generic command configuration that intentionally refuses builders. See [Account Build Sessions](../../agents/local_dev/build_sessions.md).
