...
- **Catch-up** — an agent receives what it missed since its per-chat cursor; replacement ACP sessions receive a separate full transcript replay.
...
- Adding a second agent applies the preference when a direct chat becomes multi-agent. Existing coordinator chats stay coordinated even after participants are removed.
...
- Catch-up cursors advance only after completed turns. The ordinary packet is capped at 4000 characters, excludes the recipient's own turns, and carries file names rather than bytes. Fresh-session replay is separate and preserves stored attachment content according to negotiated ACP capabilities.
