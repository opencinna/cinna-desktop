...
- **The renderer does not pick where a message goes.** It used to, by picking between two IPC channels — and picking one *was* the routing decision. `run:send` is now the only send channel; main reads the router and dispatches. The renderer says only which agent the user addressed.
...
- **`chats.orchestrated` is a mirror, and nothing reads it.** It is written by the same statement that writes `router` (`orchestrated === (router === 'coordinator')`) so a downgrade to the previous build still routes every chat the way it was routing. It is dropped in a later phase; a ratchet holds reads of it at zero.
...
Send -> useChatStream.startRun -> window.api.run.send -> run:send
...
- [Messaging](../messaging/messaging.md) — `run:send` is the send path for every chat; `chatStreamingService` still runs the model's half of it.
