...
- `src/main/services/runExecutionService.ts` — main-owned `start`, `isRunning`, `RunHandle`; private routing/agent preparation, catch-up, slash-command dispatch and terminal/refusal handling shared by IPC sends and Inbox continuations.
...
- This service runs one turn. It supplies no task budget loop, handoff/handback, script scheduler, multi-subscriber event hub or attach/replay API. Opening a chat does not subscribe to a headless turn already in flight; its persisted messages become available through ordinary reads. A takeover start control remains separate unfinished work.
