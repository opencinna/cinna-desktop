...
| File | Role |
|------|------|
| `src/renderer/src/utils/examplePrompts.ts` | `extractExamplePrompts(agent)` reads `agent.remoteMetadata?.example_prompts` and returns `ExamplePrompt[]` — it does not test the agent's `source`, which is why a folder agent reaches it unchanged. Three consumers: `ChatInput.tsx` (the `#` popup), `ChatWorkspace.tsx` (the new-chat tag cloud) and `hooks/useHintContext.ts` (hint gating). `splitPrompt()` parses `label: body` via regex; entries without a recognisable label get a truncated label and the full string as the body. |
...
| `src/renderer/src/components/layout/ChatWorkspace.tsx` | On the new-chat screen, derives `examplePrompts` via `useMemo(() => extractExamplePrompts(selectedAgent), [selectedAgent])` and renders `<ExamplePromptTags>` above `<ChatInput>` (the welcome heading appears only outside embedded agent pages). Tag click calls `handleNewChat(prompt.full)` which routes through `useNewChatFlow.startNewChat`. Also passes `selectedAgent` to `ChatInput` so the `#` popup knows which prompts to show before any chat exists. |
...
### ExamplePromptPopup / AgentMentionPopup

- Both are thin wrappers over the shared `MentionPopup<T>` primitive — only the icon, header, width, and field accessors differ between them
- Container, listbox semantics, outside-click handling, `selectedIndex` scroll-into-view, and theming all live in [Mention Popups](../mention_popups/mention_popups.md)
- Combobox wiring (listbox id, `aria-activedescendant`) is shared with the `#` popup via the textarea in `ChatInput`
