...
2. ArrowUp in an empty composer walks back through the user's own messages in this chat, newest first, so queued ones come first. Recalling a queued message enters edit mode: Send becomes a check-mark **Save**, the bubble's tab reads **Editing**, Enter or Save replaces the queued text, and Esc leaves edit mode and empties the input. In a recalled message of several lines the arrows move the caret; ArrowUp steps on only from the first line, ArrowDown only from the last.
...
- **Stop sits left of a Send slot, and the slot never moves.** While a turn runs the row is Stop then Send, and Send stays in layout but invisible and unfocusable until there is text or a queued message is being edited. Typing the first character must not reflow the row ([UX rule 1](../../development/ui_guidelines/ux_rules.md#1-nothing-jumps-while-the-user-is-acting)), and when the turn ends and Stop goes away, Send is where it always was — never under a pointer that was aimed at Stop. This layout was the user's choice.
