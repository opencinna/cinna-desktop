...
- `src/renderer/src/components/chat/MessageStream.tsx` — returns a fragment, not a wrapper: the pill is absolutely positioned against `ChatWorkspace`'s `relative` chat container (the same one the composer overlay anchors to), so it needs no layout box of its own and the scroll element stays the direct flex child it has always been.
