...
4. Claude and Codex modes allow runtime, model, tool policy and instruction edits without an AI credential. Runtime installation, login and policy compatibility are checked when the chat starts; saving a mode does not start a CLI.
...
- Engine, credential and model are optional. A plain chat binds the Default runtime instead of requiring an API provider. Claude/Codex engines use their CLI login; OpenCode uses the selected/default AI credential. Codex is selectable, including through an inherited machine default. Its restricted chat profile requires the exact verified CLI version on POSIX and a model in that CLI’s bundled catalog; unsupported configurations fail at launch with a remedy. See [Codex chat policy](../../agents/local_agents/codex_engine_tech.md#restricted-chat-and-ai-function-policy).
