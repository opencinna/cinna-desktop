...
Main retains at most 8 MiB of accounted baseline and event bytes and 5,000 cached event entries per active run. Adjacent compatible deltas, including adjacent deltas for the same child tool call, are merged with the shared part-merge rule, and **bytes track what is retained**: a merged fragment adds only its growth, so a tool part whose every fragment repeats its full input is counted once. `status` events are not retained, nested or not — a later watcher's projection ignores them — though existing watchers still receive them live.
