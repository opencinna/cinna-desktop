...
- **Set on send** — `src/renderer/src/hooks/useChatStream.ts` — `startLlm` / `startAgent` call `setPendingUserMessage({ content, baselineUserCount: snapshotUserCount(chatId), attachments: opts?.attachments })`; `snapshotUserCount` counts `user` rows in the cached `['chat', chatId]` query. The `attachments` are the same resolved `MessageAttachment[]` handed to `window.api.llm.sendMessage` / `agents.sendMessage` (already ingested — for the new-chat flow `useNewChatFlow` awaits `resolvePendingAttachments` before the send), so they have valid ids/source and the badges are click-to-open/download even during the optimistic window.
