...
- `src/renderer/src/hooks/useChatStream.ts` — `useChatStream()` — owns `handleRun`, the one MessagePort event handler for LLM and agent chats (`startRun`, `cancel`); single source of truth for stream-event-to-store fan-out. **One `start*` entry point, because there is one channel**: picking between `startLlm` and `startAgent` *was* the routing decision, and it moved into main — see [Chat Routing](../chat_routing/chat_routing.md)
...
- **Set on send** — `src/renderer/src/hooks/useChatStream.ts` — `startRun` calls `setPendingUserMessage({ content, baselineUserCount: snapshotUserCount(chatId), attachments: opts?.attachments })`; `snapshotUserCount` counts `user` rows in the cached `['chat', chatId]` query. The `attachments` are the same resolved `MessageAttachment[]` handed to `window.api.run.send` (already ingested — for the new-chat flow `useNewChatFlow` awaits `resolvePendingAttachments` before the send), so they have valid ids/source and the badges are click-to-open/download even during the optimistic window.
