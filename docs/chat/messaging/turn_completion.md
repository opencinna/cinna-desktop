...
At close the executor records the ordinary turn's result through `recordTurnResult` (`src/main/services/turnRecord.ts`), which writes the sidebar result and then the job run and its task (`reportStandaloneTurn`), each failure logged separately, and only then closes the live watch. The boot pass records a turn the app was killed under through the same function, and a recovered turn records its real ending through it too. The live close and a killed turn therefore cannot disagree about what a failed, canceled or budget ending writes. `terminalEventOf` is the one mapping from an outcome to its `done`/`error` event, shared by the executor's fallback, adopted runs and the boot pass. See [Interrupted Turn Recovery](../../agents/turn_recovery/turn_recovery.md).
...
A completed ACP result may carry a [manifest-authorized handback note](../../jobs/tasks/manifest_handback_tech.md). The wrapper drops it on unsuccessful outcomes, and the coordinator checks open/uncertain requests before using it. This remains contextual data for the existing owner transition.

There is still one active turn per chat. `runExecutionService.adopt` holds that slot for a turn recovered after a relaunch: it refuses a busy chat, saves nothing and records no result itself (its driver does both before resolving), and still turns a completed outcome with open continuations into `needs_input`. The [autonomous task runner](../../jobs/tasks/autonomous_tasks.md) uses this ownership to coordinate consecutive turns, handback and durable gates; its reservation also excludes ordinary sends between turns and during waits. [Script execution](../../jobs/tasks/script_execution.md) uses the same ownership per isolated child conversation. Schedules and token-budget enforcement remain separate work.

## Model and Agent Endings

The [sidebar result projection](../session_status/session_status_tech.md#services--key-methods) follows this same completion ownership. Ordinary executor closure saves the outcome under its run ID before notifying the live watch; budget or uncertain request cleanup is displayed as failed, and user cancellation has no unread badge. Runner-owned leaf turns never write the session result. Coordinator and script controllers publish fresh identities for their own completion, failure, durable wait or recoverable interruption, so a timeout is not hidden as a user cancel and an intermediate successful turn is not shown as session completion. These local result rows do not change TurnOutcome or replace task status.
...
- A2A input-required/auth-required endings return `needs_input`. Canceled endings return `canceled`. Failed, rejected or other unfinished task states at transport end return a failed result, and so does a reply the Cinna backend marks `aborted` (code `reply_cut_off`). A stream that ends without `final` is collected from `tasks/get` before its outcome is decided, on a backend that supports it. A nonstream JSON-RPC error envelope enters the error path instead of producing an empty successful answer.
...
- The direct-agent wrapper persists its result — on a failure too, with the error row after it — reports the typed ending and releases its active request before close. The successful/needs-input catch-up cursor hook remains separate; failure and cancellation do not advance it. A2A Stop keeps the session ids the stream's first task event saved (or the prior checkpoint, before any), and reports remote cancellation as confirmed only when `tasks/cancel` answers `canceled` or `completed`, or, when it answers with no state, when one `tasks/get` reads `canceled`.
