...
| File | Role |
|------|------|
| `src/renderer/src/hooks/useChatStream.ts` | `handleRun` routes every `delta` event through `chat.store.appendDelta` regardless of `kind`. `RunDeltaEvent.kind` is typed as `ContentKind`, which includes `'notice'` — no per-kind branching needed in the hook. Inside a `child` event (an orchestrated sub-thread) a notice is dropped by `appendToolSubEvent`, so the live sub-thread matches the reloaded one. |
