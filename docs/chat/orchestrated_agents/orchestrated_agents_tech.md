...
- `ChatInput` — new-chat `@` agent pick calls `onTogglePendingAgent`; in an orchestrated chat (`isOrchestratedChat = chatId && chatData.orchestrated === true`) an `@` agent pick calls `addOnDemandAgent` instead of the multi-agent `composer.switchActiveAgent`; renders `OnDemandAgentChips` (DB mode in active chats, buffer mode on new-chat) and the on-demand MCP chips; `CommPatternBadge` renders on the right, left of the `[+]` attach button (new-chat only); `selectedAgent` still drives example prompts via the agent selector
