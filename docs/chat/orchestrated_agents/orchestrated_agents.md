...
4. A specialist's permission request stays live and can be answered in its sub-thread or the Inbox. A question becomes a durable Inbox request; the conductor ends its turn instead of repeatedly calling the waiting agent.
...
- Synthetic plain-chat modes prohibit native filesystem and shell tools. **No tools** also hides attached connectors; **Connected tools** permits only the bridge's attached capabilities. A specialist keeps its own permission profile.
- Claude synthetic sessions use explicit empty native tools and isolated settings. OpenCode uses generated deny-by-default native permissions with the Cinna bridge allowed. Codex synthetic plain-chat/AI-function sessions fail closed because the pinned adapter cannot enforce the no-file-tools contract; folder Codex coordination remains structurally available.
...
- Bridge calls are bounded: ordinary turns allow 100 calls; autonomous turns consume the runner's persisted budget. A specialist budget result pauses the conductor rather than retrying the same limited login.
- The endpoint URL/token stay stable only within the running app. A descriptor/config hash is persisted; after restart the new endpoint changes that hash, so the driver creates a fresh session and replays full saved history and attachments. It does not load a session against an obsolete bearer endpoint.
- Replacement-session replay is separate from bounded per-agent catch-up. Stored error/transition rows are excluded; prior speakers and tool-call IDs are attributed. Current user content is not replayed twice.
