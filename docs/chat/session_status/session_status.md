...
- **A turn the agent starts on its own counts as work.** When a local agent answers on its own after its reply ([a follow-up turn](../../agents/local_agents/agent_turn.md#a-turn-the-agent-starts-on-its-own-is-a-follow-up-turn)), its row spins, can be interrupted, and leaves an unread result like a sent turn. The background processes and subagents the session runs *between* turns are not a spinner; they are shown under the composer ([Session Activity](../../agents/session_activity/session_activity.md)).
