...
Stopped unread rows use CircleCheck/success, CircleHelp/warning or CircleAlert/danger with CSS variable colors. Their image names are **Completed — unread results**, **Needs input — unread results** and **Failed — unread results**. Hover/focus replaces the image with Trash2; the button remains named Delete session and its title combines outcome and action. Canceled or read results have no result image. Interrupt/delete errors use the existing IPC unwrapping and render beside the row.
