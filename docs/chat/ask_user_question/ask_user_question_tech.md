...
- `pairCommandTools(items)` decides which results a local question swallows. Its own `que_*` `tool_result` becomes the block's `decision`. When `questionCallId(toolInput)` (`src/shared/localAgentRequests.ts`) names a call, and a `tool` item with that id is itself an ask-user-question tool, **that call is consumed**. So is the first later non-`stderr` `tool_result` on that id, the Claude adapter's restatement of the answer. The call has to go as well, because it is a question tool by name. Its part normally renders nothing, since it carries no input. A part that kept its `questions` would render a second card under a non-engine id, answerable, whose Answer sends a new user turn. A named call missing from the items folds nothing. Both render paths use the helper, and it sees a whole saved row. The call, question and result always share one, because a turn's row splits only at a steer and steering is withdrawn while a tool call runs (see [Agent Turn tech](../../agents/local_agents/agent_turn_tech.md)). The streaming path's block copies carry `toolStream`, so the `stderr` exception also holds mid-turn.
- Covered by `MessageStream.questionEcho.test.tsx`: folded when persisted (verbose and compact) and while streaming; a call that kept its questions leaves one card and no Answer button; kept for a non-`AskUserQuestion` call, for a named call not in the transcript, for `stderr`, and for a question without `callId`.
