...
- **Sign-out** (`signOutCleanup` + `deleteWithCascade`) — removes the local profile entirely (final flush first; never wipes remote records, so peers keep their data). When sync is active it also revokes this device server-side → the next sign-in is a fresh, Locked-untrusted device that must restore (recovery / pairing); the synced data re-pulls from the cloud. There is no "keep the device trusted" sign-out.
