...
- **Job dependencies are portable, not id-based** — a job does **not** sync device-local `nanoid`s for its agents/MCPs/mode. It syncs a **dependency manifest** (`jobs.sync_deps`): self-describing descriptors keyed by each dependency's own identity rather than by a device-local row id (remote agent → backend UUID; local agent → card URL; MCP → transport + normalized URL / stdio command+args; mode → name).

  **The folder agent is in this manifest but is not a portable identity, and listing it as one was the mistake worth naming.** Its descriptor is keyed on the `id` inside its `cinna-agent.json`, which is a key, not a passport: **local agents are not synced, so a job bound to one is a local-only job** — see [Local Agents Are Not Synced](../../agents/local_agents/local_only.md). The descriptor exists so an unresolvable dependency is *visible and blocking* rather than silently dropped. The case it genuinely serves is the **same machine** after a re-auth or reinstall, where the folders are still there. It also happens to resolve on any machine holding a folder with the same manifest id — that is a property of a keyed lookup, not a supported workflow, and the matching question (what it should *mean* for a local agent on one machine to correspond to one on another) is undesigned. On a peer the descriptor is resolved by identity (server-backed deps are zero-prompt on the same account), and on a miss a **disabled, not-connected** MCP/local-agent shell is auto-created in Default Scope so the dependency is never silently lost — surfaced as a "finish setup" state.

**The folder-agent variant is the one exception to auto-create, and the exception is the point.** A folder agent *is* a directory on disk, so a shell row would assert that a workshop is present on this machine when it is not. Nothing is created; the dependency stays in the manifest, out of the join rows, and the Jobs detail list reports it as **`unavailable`** rather than `needs-setup` — there is no in-app repair, because no settings page produces a directory, and the app deliberately does not tell the user to copy one here. What this replaces is worse than a limitation: before the variant existed the descriptor could not be built at all (a folder row has both `card_url` and `endpoint_url` null), so it was dropped from the manifest, `manifestNeedsSetup` returned `false`, and the job ran as a plain-LLM chat and recorded a success with the agent silently absent. **That run is now refused outright** (`jobService.executeLocal` throws `JobError('incomplete_setup', …)` after recomputing the unresolvable-agent set from the manifest), so the descriptor's job is no longer only to display the problem. See [Folder Agents as Counterparties](../../agents/local_agents/counterparty.md). A remote agent from a server this profile isn't on stays an unresolved (grey) descriptor; it never mis-binds and never disappears. The manifest is the synced truth; the `jobAgents`/`jobMcpProviders` join rows + `jobs.modeId` are the materialized resolvable subset. Because both devices serialize the *same* descriptor for the *same* logical dependency, the payload is byte-stable across a round trip (the server returns `unchanged`), so adding a dependency on one device propagates instead of mutually deleting.
