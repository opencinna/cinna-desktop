...
- `db/users.ts` — `userRepo.deleteWithCascade(userId)`: raw, tombstone-free transactional delete of the user row + all profile-scoped tables (chats→messages/sessions/files, jobs→runs/joins, job/note folders, notes, profile agents/overrides) — backs **both** Cinna sign-out and full delete (no separate keep-row path). It also deletes `llmProviders`/`mcpProviders`/`chatModes` by userId, which is a no-op for Cinna profiles since those are Default-scope (under `__default__`, never a Cinna userId). Raw deletes never enqueue sync tombstones, so they don't propagate as peer deletes. `updateCinnaProfile(id, …)` refreshes identity fields on a re-login rebind.
...
- `services/authService.ts` — `deleteAccount({signOut, removeDevice})`: for a Cinna profile, sign-out and full delete run the **same** local teardown — `signOutCleanup({removeDevice})` (final flush + drop sync keys) then `userRepo.deleteWithCascade` (removes the row, so the account leaves the switcher). They differ only in `removeDevice`: a sign-out revokes the device iff sync is active (`removeDevice = signOut ? input.removeDevice !== undefined : true` — the renderer sends `removeDevice` defined only when sync is on); a full delete always revokes. Neither calls `syncApi.wipe`/`resetEncryption`, so remote records and peers are untouched. Runs regardless of `wasCurrent`. `registerCinna` rebinds a returning Cinna identity (refresh tokens + `updateCinnaProfile`, reactivate); since sign-out fully removed the local profile, re-login mints a fresh row and bootstrap-pulls synced data from the server.
...
- `components/auth/UserMenu.tsx` — the sign-out modal; for `cinna_user` it sends `{signOut: true, removeDevice: syncActive ? true : undefined}` (no toggle — signing out **always** removes the device when sync is on). When `syncActive` (`isCinnaUser && syncState.initialized && !syncState.disconnected`, derived from `useSyncState`) it shows a **danger box** ("This device will be removed from your account — you'll need your recovery key or another signed-in device to restore your data…") plus the "notes return on next sign-in" sentence; with sync off both are hidden and `removeDevice` is `undefined`. The confirm button is disabled while `useSyncState().isLoading` (cold open) so an active-sync sign-out isn't mis-routed before the state resolves. If the state query instead *errors* (no data, not loading), `syncActive` falls to false and the device revoke is skipped — a safe degradation (local profile still fully removed, remote data untouched, stale device revocable from another device); `getState` rarely hard-errors since it falls back to the local flag offline. Local profiles keep "Remove Account".
