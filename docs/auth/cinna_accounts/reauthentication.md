...
### Trigger: discoverability via the user menu
1. Any time the current user is a Cinna account, a "Re-authenticate" entry appears in the user menu's Actions section
2. When the user has no tokens (`hasCinnaTokens === false`), the entry is rendered in the accent colour to draw attention
3. Clicking it runs the same OAuth flow. On failure, the menu reopens with an inline error so the user can retry
