...
| Gateway | `src/main/agents/drivers/a2aConnection.ts` | `resolveAccessToken` lets `CinnaReauthRequired` bubble (previously masked all errors as session expiry). `rethrowAsReauthIfCinna401` converts a 401/403 (`authRejectionStatus`, from `A2aHttpError` or `AgentCardFetchError`) into `CinnaReauthRequired` — but only when `capabilitiesFor(agent).auth === 'cinna'`, i.e. a Cinna-synced agent. Hand-added A2A agents keep the original error. Both moved here from `agentService` |
...
| Stream service | `src/main/services/a2aStreamingService.ts` | `runAgentTurn` takes `isCinnaTokenAuth?: boolean` (`RunAgentTurnInput`, `:161`). When it is true and the caught error is an `A2aHttpError` with status 401/403 (the `isAuthRejection` type guard, `:447`), the error is replaced with `CINNA_SESSION_EXPIRED_MESSAGE` and tagged with the reauth code. No string-matching — the detection is purely structural. **`streamToAgent` no longer takes the flag.** The A2A driver sets it from `capabilitiesFor(agent).auth === 'cinna'` (`src/main/agents/drivers/a2aDriver.ts:157`), so a direct chat and the orchestrator's agent tool decide it the same way |
...
- `resolveAccessToken` (moved here from `agentService`) — Refactored from a bare `try/catch` that re-threw everything as `'Cinna session expired…'`. It now lets `CinnaReauthRequired` bubble, so callers can tell a reauth from an unrelated error: the A2A driver's `run` maps it to `CINNA_SESSION_EXPIRED_MESSAGE` plus the reauth code, and its readiness check to `not_logged_in`. Other errors propagate with their original message intact
...
- **Token storage**: reuses `storeCinnaTokens` → `userRepo.setCinnaTokens`, which encrypts via `safeStorage` (OS keychain). Storage advances the per-user credential-session generation: a refresh begun before re-authentication can neither overwrite nor clear this replacement, and new callers do not join its promise. Ordinary token rotation retains the generation. Stale operations throw `CinnaSessionChanged`, which does not trigger the global re-authentication notification; a healthy replacement session stays signed in. See [token lifecycle internals](token_lifecycle_tech.md). No new keystore plumbing
