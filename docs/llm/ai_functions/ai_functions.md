...
1. Select an AI Functions credential/model or leave Default runtime selected. Changing credential clears the model selection. The **Runs on** line under the picker names what will actually run: the credential and model, or `Default runtime`, or — when the chosen credential is unusable — `Default runtime — the chosen credential is inactive` / `… is missing` / `… has no model`. Main decides it (the same checks `resolveBackend` makes), so the line and the backend agree — except for a stored key that no longer decrypts (a reset keychain): the line cannot see that without decrypting, while the call falls back to the Default runtime and logs a warning. The picker keeps showing the stale choice (`Missing credential`), so the user can see what they picked and replace it.
...
`src/main/services/aiFunctionsService.ts` owns resolution and common timeout/output limits; `src/main/services/aiFunctionRuntimeService.ts` owns the throwaway session lifecycle; `src/main/agents/drivers/index.ts` prepares runtime plans; `src/main/services/syntheticRuntimePooling.ts` owns compatibility keys. Settings keys are aiFunctionsCredentialId and aiFunctionsModelId; empty strings mean no explicit selection. The upgrade backfill is `backfillAiFunctionsCredential` in `src/main/db/migrations/app-settings.ts`, run last by `runAllMigrations` (after the legacy `user_id` backfill, since it reads `chat_modes.user_id`); its marker row `AI_FUNCTIONS_BACKFILL_MARKER` is not an `AppSettingsSchema` key, so `appSettingsRepo.getAll` never surfaces it. The **Runs on** line is `FeaturesSettingsSection.tsx`, rendered only from `aiFunctions.describeBackend()` — DB-only, no key decrypted, no adapter built, sharing `checkEligibility()` with `tryResolve` — which reaches the renderer as `AiFunctionsBackendStatus` (`src/shared/aiFunctions.ts`) over `settings:ai-functions-backend` (`useAiFunctionsBackend`, invalidated on setting writes, provider upsert/delete and account-config sync; auth changes reset every query). The model name comes from the renderer's `useModels()` list for that credential, else the id. `localAgentDraftService.draft` no longer has a skipped-for-no-backend path.
