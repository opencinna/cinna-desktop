...
- `src/main/services/aiFunctionsService.ts` owns one-shot adapter execution; `src/main/services/conductorBridge.ts` owns conversational tools through ACP. Legacy llm:cancel cancels the main run by ID.
