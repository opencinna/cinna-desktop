...
- **A dependency the manifest names but this device cannot supply is *reported*; an unresolvable **agent** is also *enforced*.** `getDependencyStatus` distinguishes `needs-setup` (a row is here and needs finishing — the "Set up →" button opens it) from `unavailable` (nothing here to open). A folder agent whose workshop is absent is `unavailable`; there is no in-app repair, and the app deliberately does not suggest one — see [Local Agents Are Not Synced](../../agents/local_agents/local_only.md).
...
- **A refusal is an ending too, and for a long time nothing reported one.** A turn refused *before* either streaming service owns it — the chat is gone, its provider/model was deleted, no adapter is available for it, the bound agent is no longer configured — posts an error, closes the port and throws; the send path swallows that throw by design, because a service that owns the port owns its ending. Nothing else reported an outcome, so the job run behind such a turn stayed `running` for the life of the app: nothing reaps a stale run, and `countInProgressByJob` counts it, so the job advertised itself as busy for ever. Six exits now finalize the run as `failed` carrying the refusal's own message — the three guards at the top of `chatStreamingService.stream`, and the missing chat, the missing agent and the catch in `run.ipc.ts`, through `reportRefusal`. `reportRefusal` **swallows its own errors by contract**: one call site sits inside a catch whose entire guarantee is that it does not throw, and a rejection there posts nothing, closes nothing and leaves the renderer streaming for ever, so bookkeeping is never allowed to break the one path whose job is to report a failure. The stale run row predates tasks; what made it urgent is that the run's task also stayed wherever its caller had put it, so a re-run started from the task page could claim a stuck task and then leave it claimed with no surface able to move it again. One unowned exit still reports nothing, and that is a decision rather than an oversight: the activation guard, which refuses a send arriving while no user is activated, is the app declining to do anything profile-scoped, and a job-run write there would be the only place main touches a locked profile's rows. A run left `running` when the profile locks was orphaned by the *lock*, not by the send that follows it — and **nothing reaps a stale `running` run at unlock or at boot** (`runConsistencyChecks` does three things and none of them is job runs), so that reaper, not a write smuggled past the activation guard, is the fix for the whole class.
...
Run flow (local)
  Run button -> useExecuteJob -> window.api.jobs.execute(jobId)
     -> jobService.execute() -> executeLocal()
        ...
        -> newChatRouter(agentIds, mcpIds) -> direct | human | coordinator
        -> jobRunsRepo.createLocalChatAndRun() [single transaction]
              direct      : chat(agent_id=rootAgent) when there is an agent
              human       : chat(agent_id=null) + chat_on_demand_agents
              coordinator : chat(agent_id=null) + chat_on_demand_agents
              chat_on_demand_mcps whenever the model is the answerer
              + job_runs + chat.originating_job_run_id
        -> taskService.create() + start()   [after the chat and the run, so a refusal
                                             above leaves no orphan task behind]
        -> jobRunsRepo.setTaskId(runId, taskId)
  -> execute result.agentId  = who the first message goes to (null = the model)
  -> renderer resolves provider/model, navigates, fires
     startRun(chatId, prompt, { target })
  -> chatStreamingService / a2aStreamingService -> jobService.reportRunCompletion()
       -> taskService.applyRunState() on the run's task (best-effort)
  -> a turn refused before either service owns it:
     run.ipc.ts reportRefusal -> jobService.reportRunCompletion(chatId, 'failed', message)

Run history row (local, run has a task)
  -> useOpenTask(taskId) -> setActiveTaskId + setActiveView('task')
  -> MainArea -> TaskView   (src/renderer/src/components/tasks/TaskView.tsx)
       -> "Chat" action on the row still opens the spawned chat via useOpenChatFromRun
