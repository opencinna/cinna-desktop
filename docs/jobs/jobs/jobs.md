...
- **A dependency the manifest names but this device cannot supply is *reported*, not enforced.** `getDependencyStatus` distinguishes `needs-setup` (a row is here and needs finishing — the "Set up →" button opens it) from `unavailable` (nothing here to open). A folder agent whose workshop is absent is `unavailable`, because the repair is copying a directory onto the machine. **Neither state blocks a run:** `executeLocal` reads the `job_agents` join rows only and never consults the manifest or the `needsSetup` flag, so a job whose folder agent did not resolve here runs as a plain-LLM chat with the agent absent and records a success. Displaying it is what 7c added; gating on it is an open product decision.
