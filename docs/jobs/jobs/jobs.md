...
- **Run Status** — Local runs flip via the chat-stream completion hook (success when the first assistant turn finishes, failure when the stream errors, **cancelled when the user stops it**). Cinna runs flip via polling cinna-core's task status while non-terminal.
...
4. The `execute` result carries `agentId` (non-null ⇒ A2A, null ⇒ orchestrated/LLM), so the renderer dispatches `startAgent` vs `startLlm` with the existing logic. For the LLM path it resolves provider/model the same way the new-chat flow does (falling back to the workspace's default chat mode when the job left `modeId` null), then navigates into the spawned chat without leaving the Jobs sidebar tab and kicks off the stream with the prompt.
5. When the chat's stream finalizes (`done`), the run flips to `succeeded`; on stream error, it flips to `failed` with the error message.
...
- **A stop is an ending, not a non-event.** Pressing Stop in a job-spawned chat finalizes the run as `cancelled` — with no error message, because a stop is not a failure — down **every** exit the stream can take, and there are five across the two streaming services. A run that already reached a terminal status is left alone, so a late stop cannot rewrite a recorded outcome. This is separate from `job:cancel-run`, which is the explicit action on a run row rather than a stop on the chat.
- **Finalizing a stop as `cancelled` changes the outcome and nothing else.** Where the turn ends cleanly — always on an agent-backed run, since a cancelled runner returns what it streamed with no error, and on an LLM run when the stop lands between tool rounds — `done` still goes to the renderer and the streamed text is still saved as the assistant's message. The run is recorded as cancelled beside an answer the user can read; stopping a turn is not undoing it.
- **One case does lose the last round's text, and it is the adapter's exit rather than the run's.** A stop that lands mid-stream on an LLM turn makes the adapter reject, so that round's assistant message was never saved and no `done` is posted — the earlier rounds are already persisted. That is a property of where the abort surfaces, not of how the run is finalized, and it is the same on all three adapters.
