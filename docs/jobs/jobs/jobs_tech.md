...
### Main process
- `src/main/db/schema.ts` — `jobs` (with `folderId` / `position` columns; `agentId` now legacy/dormant), `jobFolders`, `jobMcpProviders`, `jobAgents` (multi-agent join table mirroring `jobMcpProviders`), `jobRuns` tables; `chats.originatingJobRunId` / `chats.orchestrated` columns
...
- `src/main/db/jobs.ts` — `jobsRepo` (CRUD + soft delete + `minPositionInFolder` + `reorderInGroup` transaction that rewrites every passed id with the target group's `folderId` and a fresh `position = index`), `jobFoldersRepo` (CRUD + `reorder` transaction + `delete` transaction that detaches contained jobs to root before dropping the folder row), `jobMcpRepo` (`listProviderIds`, `setProviderIds` — dedups ids before the delete-then-insert), `jobAgentRepo` (`listAgentIds`, `setAgentIds` — same dedup-then-replace shape as `jobMcpRepo`), `jobRunsRepo` (`listByJob` — LEFT-joins `chats` and returns `JobRunRowWithMeta` with a derived `chatHidden` boolean —, `countInProgressByJob` — single `GROUP BY job_id` aggregate over rows with `status IN ('pending','running')`, used by the sidebar spinner —, `getById`, `getByLocalChatId`, `create`, `updateStatus`, `createLocalChatAndRun` orchestration-aware transaction: sets `hiddenFromList = true`, writes the A2A root agent (`agentId`) OR the orchestrated `chat_on_demand_agents` + `chat_on_demand_mcps` rows depending on the routing). New jobs land at `position = min(position) - 1` of the root group so they appear at the top of the ungrouped list.
...
- `src/main/services/jobService.ts` — `jobService` — `list` (returns `JobListItem[]` = `JobRow & { inProgressRunsCount: number }`, joining `jobRunsRepo.countInProgressByJob` so the sidebar can render a per-row spinner without a second IPC call), `getDetail` (returns `JobDetail` with `agentIds` + `mcpProviderIds`), `create`, `update`, `softDelete`, `setMcpProviders`, `setAgents` (replaces the job's agent set; drops stale ids via `filterExistingAgents`, then `jobAgentRepo.setAgentIds`), `listRuns`, `execute` (discriminated dispatcher), `executeLocal`, `executeCinnaTask`, `refreshCinnaRun`, `reportRunCompletion`, `setRunStatus`, `getCinnaServerUrl`, `listFolders`, `createFolder` (trims + validates name non-empty), `updateFolder` (validates + filters patch keys, refetches the row to return), `deleteFolder`, `reorderFolders` (logs `folders reordered { count }`), `reorderJobs` (validates target folder ownership when non-null, walks `orderedJobIds` and asserts every id maps to a non-deleted job in the active scope, then calls `jobsRepo.reorderInGroup`; logs `jobs reordered { targetFolderId, count }`). Private `filterExistingAgents(agentIds)` resolves ids against `getAgentLookupScope()` (settings + profile) via `agentRepo.getOwned`, returning only the ones that still exist. Private `mapCinnaStatus()` translates cinna-core status values into local `JobRunStatus`. Private `enrichRun(userId, run)` attaches the `chatHidden` boolean for single-row methods (`refreshCinnaRun`, `setRunStatus`) so every IPC return matches the same `JobRunRowWithMeta` shape as `listRuns`. Imports `derivePattern` from `src/shared/commPattern.ts` to route runs (see `executeLocal` below).
...
### Shared
- `src/shared/jobs.ts` — `JobData` (now carries `folderId: string | null` + `position: number`), `JobRunData`, `JobDetailData` (now carries `agentIds: string[]`), `JobCreateInputDto`, `JobPatchDto`, `JobType`, `JobRunStatus`, `JobFolderData`, `JobFolderCreateInputDto`, `JobFolderPatchDto` — imported by both preload and renderer (the preload `.ts` is not exposed to the renderer compiler, so DTOs live in `shared/`)
- `src/shared/commPattern.ts` — `CommPattern` type + `derivePattern(agentIds, mcpIds)`. Single source of truth shared by the new-chat composer (`useNewChatFlow` / `CommPatternBadge`) and the job runner (`jobService.executeLocal`): one agent + zero MCPs → `'A2A'`, anything else → `'AI'`. Moved here from the renderer so the main process can import it

### Preload
- `src/preload/index.ts`:
  - `window.api.jobs` — `list`, `get`, `create`, `update`, `delete`, `setMcpProviders`, `setAgents(jobId, agentIds[])`, `listRuns`, `execute` (returns discriminated union `{ type: 'local' | 'cinna_task', ... }`), `cancelRun`, `refreshRun(runId, options?: { force?: boolean })`, `cinnaServerUrl`, `reorder(targetFolderId, orderedJobIds)`
...
- `src/renderer/src/components/jobs/JobDetail.tsx` — Read-only view: header (title + type pill + **icon-only Edit pencil** + **green Run pill**), `<JobSummary>` (private subcomponent: prompt block + flex-wrap chip strip for non-default config), run-history section. Run uses `bg-[var(--color-success)] hover:brightness-110 text-white` (matches the sidebar's run pill / spinner); Edit is a square outlined icon-only button (`p-1.5`, Pencil glyph, `title="Edit job"`) that fires `setActiveView('job-edit')`. `useCinnaRunPoll(runs)` keeps cinna statuses fresh while the view is open. `JobSummary` resolves names via `useAgents` / `useChatModes` / `useMcpProviders` / `useCinnaAgents`; the `CINNA_DEFAULT_PRIORITY = 'normal'` constant gates whether the priority chip is shown. For local jobs it derives `derivePattern(job.agentIds, job.mcpProviderIds)` and renders a `<CommPatternBadge>` (always, even for a bare LLM job) plus **one `AgentChip` per attached agent**, the mode chip, and one MCP chip each. Chips reuse the chat composer's pill pattern via a local `<Chip>` primitive (`pl-1.5 pr-2.5 py-1 rounded-lg border` with `neutral | accent | danger` tones); typed wrappers `AgentChip` (Bot, accent), `ModeChip` (color dot via `getPreset(colorPreset)`), `McpChip` (Plug), `PriorityChip` (Flag), `MissingChip` (danger tone for "No Cinna agent").
...
- `orchestrated` INTEGER — set by `createLocalChatAndRun` when an orchestrated job run attaches ≥1 agent (LLM-root chat that conducts agents/MCPs as tools). See [Orchestrated Agents](../../chat/orchestrated_agents/orchestrated_agents.md)
...
- `job:execute(jobId)` — Discriminated union:
  - `{ type: 'local', chatId, runId, prompt, agentId, modeId }` — `agentId` non-null ⇒ direct-A2A run (renderer fires `startAgent`); null ⇒ orchestrated/plain-LLM run (renderer resolves provider/model, fires `startLlm`). The chat's `orchestrated` flag + on-demand agent/MCP rows are already written server-side, so the renderer dispatch logic is unchanged from the single-agent era.
...
- `src/main/db/jobs.ts`:
  ...
  - `jobRunsRepo.createLocalChatAndRun(input)` — single `db.transaction(...)`, orchestration-aware. Inserts the chat row (with `agentId = rootAgentId` and `orchestrated`), the `job_runs` row, back-fills `chats.originating_job_run_id`, and — for the orchestrated/AI case — inserts `chat_on_demand_agents` + `chat_on_demand_mcps` rows (each with `pendingAnnounce = true`, inlined rather than calling the on-demand repos so the inserts share the job's transaction). Returns `{ chatId, runId }`. Note: job MCPs land in `chat_on_demand_mcps`, NOT the chat-mode baseline `chat_mcp_providers`, so they count toward `derivePattern` and the orchestrator unions them.
...
- `src/main/services/jobService.ts`:
  ...
  - `executeLocal(userId, jobId)` — reads `jobAgentRepo.listAgentIds`, throws `missing_dependency` if any attached agent no longer resolves, validates mode existence, filters stale MCP ids, then `derivePattern(agentIds, mcpIds)` routes: A2A → `createLocalChatAndRun` with `rootAgentId = agentIds[0]`, `orchestrated = false`, empty on-demand sets; otherwise → `rootAgentId = null`, `orchestrated = agents.length > 0`, agents/MCPs passed as on-demand sets. Returns `{ chatId, runId, prompt, agentId, modeId }` where `agentId` is the A2A root (or null), preserving the renderer dispatch contract.
...
- `JobDetail` (read-only view; editing lives on `JobEditPage`):
  ...
  - `JobSummary` card: prompt block + chip strip. For local jobs the strip leads with a `CommPatternBadge` (always shown) followed by one agent chip per attached agent, the mode chip, and one MCP chip each.
...
- `JobEditForm`:
  ...
  - Agents + connectors are one control: chips (Bot for agents, Plug for MCPs) + a single "Add" button opening the multi-select "Agents & Connectors" `AgentPickerModal`. A `CommPatternBadge` previews `A2A`/`AI` from the current agent+MCP selection.
