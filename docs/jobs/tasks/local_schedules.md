...
- **Only a runnable, effectively enabled kit agent with stable identity can declare local execution.** Runnable means readiness `ok` or `credentials_needed`: missing `credentials/.env` values only warn, as they do for chat and tasks, so a folder under development keeps its schedules. `invalid` and `contract_too_new` still refuse. Names are unique, trimmed and 1–255 characters; prompts are nonblank and at most 64000 characters. Only static_prompt executes. Command/script-trigger declarations remain unsupported.
