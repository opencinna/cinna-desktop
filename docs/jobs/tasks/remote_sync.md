...
- **Nothing calls it.** There is no timer, no IPC channel and no service caller: `taskSyncService` is imported by its own tests and by nothing else in the app. This is the same shape as the handoff-note writer landing before anything wrote a note, and the adapter seam landing before its first adapter. The producers arrive with the step that folds the `cinna_task` job path onto `adapter.create` + `adapter.execute`; until then the sync loop is written, tested and inert
- **`resetCursors()` has no caller either, and the commit that first calls `pull` owes it one** — on unlink and re-link, not on sign-out. The map is keyed per `(profile, adapter)`, so a profile *switch* is already safe; the hazard is that a profile id is this device's, not the account's. Unlink a profile from one account and link it to another and the old cursor survives, so the new account's first pull is a **delta rather than the active set** — every task older than that cursor is never fetched, and the reconcile does not run either because the pass count is no longer zero. The profile looks half-empty with nothing on screen to explain it
