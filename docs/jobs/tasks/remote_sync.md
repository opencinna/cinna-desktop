...
`jobRunStatusForTask` in `src/shared/taskStatus.ts` maps completed/archived to succeeded, error to failed, cancelled to cancelled, new to pending, and other states (including blocked) to running. A failed projection carries the task's error. Terminal attempt rows are left alone, preserving their finish time and error across forced refreshes. `jobService.refreshRun` asks for a pull but does not repeat the status write. Jobs and run-history queries poll every five seconds only while their cached rows contain active attempts; this also updates local jobs handed to a remote service.
...
- **replicas and locally authored remote-executor tasks are candidates.** A locally authored task is kept and unbound on confirmed loss; only replicas are removed. Desktop-owned local mirrors retain lazy confirmation. This lets the scheduler detect vanished remote Job work without a separate network timer per history row.
- **only candidates the desktop still believes are live.** A finished replica is absent from the active set by construction, so confirming them all would be one detail request each at every app start and every twentieth pull, for work nobody is waiting on. The consequence, stated rather than hidden: a replica that finished here and was *then* deleted on the service stays as local history — the better of the two wrongs, since it is a record of work that really happened
...
### Inbox counts come from open asks

The Inbox enumerates open asks through the adapter's ask capability. The unused activity-count probe and `remoteWork` service API were removed; no surface relies on a profile-wide activity count as an Inbox count.
...
A confirmed network `not_ours` passes `confirmedMissing: true` to `taskService.unbindRemote`. In the same transaction as clearing the binding, it fails only the scoped pending/running Job attempt whose task and nullable chat links still match, while the task still has a remote executor and no unresolved handoff receipt. Terminal history is untouched. Relink/reset unbinding leaves this flag false: changing accounts does not assert that remote work was deleted.

The scheduler uses the same path before removing a missing replica. Previously a Task-page read could clear the binding first and leave the Job marked running forever; its later refresh had no remote address left to check. Loss settlement now belongs to the task lifecycle that obtained the evidence.
...
- `src/main/services/taskSyncService.ts` — `getWatched`, `getChildren`, `listChildren`, `invalidatePending`, `handOff`, `takeOver`, `liveSession`, `preferredAdapterId`, `push`, `pushAll`, `pullOne`, `pull`, `reconcile`, `resetCursors`, `forgetBindings`; module-private `pushOne`, `upsert`, `dropMissing`, `resolveParent`, `wouldChange`, `consequenceOf`
