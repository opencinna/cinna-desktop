...
- **Taking over claims; Continue starts.** After a no-chat task is claimed here, the task page offers a local agent/default-model selector and Continue. `taskExecutionService` creates the conversation and dispatches the full goal, distinct description and handoff note through the shared executor, preserving task/binding/job provenance. See [task start](tasks_tech.md). There is still no general `task:hand-off` channel or remote assignee picker; the handover service gets its production calls from jobs.
