...
So the first pass makes a second, **cursored** request covering `FIRST_PASS_BACKFILL_MS` (a week) and merges the two lists by remote id, first writer winning — they overlap by construction, and both came from the same service in the same instant. A cursored list carries no status filter, which is what makes it the only way a terminal task arrives at all.

Two things follow that are easy to get wrong. The merged list is what gets upserted, but the **active set alone** is what the reconcile is handed: the backfill is mostly terminal tasks, and offering them as "still listed as active" is a claim about the service that is not true. And the window is a per-app-start cost, not a per-poll one — later passes have a cursor and ask once.
