...
This is the file channel of the [Handover Bus](handover_bus.md); kit and cloud targets use its other channels.
...
7. **The requester follows up.** A `revisions/001.md` beside the brief — the answer to a `blocked` report, or more to say about work under way — arrives as a new turn on the handover's own conversation where Cinna runs the executor, so the session and everything the executor has already read are still there. An executor outside the app reads the file itself.
...
- **Cinna writes requester files only when asked through the handover tools, and deletes no protocol files.** Every desktop-side fact — the task id, the run, what the gate said, why something was refused — is a row in SQLite (the canonical delegation plus file reconciliation), and the task page is where it is read. There is no mirror file, because a second copy of the truth is a second thing that can be wrong. Retention is the requester's: the protocol says it may delete its own handover folder once the report is final and read, and a person deletes by hand.
...
`MAX_HANDOVER_DEPTH` is 2: a ticket manager at depth 0 hands to project agents at depth 1, which may hand on once more. Above that the brief is recorded, refused with a visible reason, and its task cancelled. **This is also what stops two folders handing work to each other for ever** — a cycle dies within two rounds. Depth comes from the canonical delegation of the validated origin task — or, when the brief names no task, of the origin chat's task — plus one, and the brief's own `depth` can only raise that figure, never lower it: an executor that left `origin.task` out, or wrote `depth: 1`, would otherwise start a fresh chain and the cap would stop nothing. The chain's root is found the same way, so such a brief also stays in its writer's chain rather than founding a new one. A brief whose origin resolves to no chain keeps its declared depth. This chain is independent of the one-level task hierarchy: a delegated task has no parent merely because another task requested it.
...
- **The executor's turn ending is not the report.** The Inbox finishes a hand-opened chat's task when its turn ends, because there the turn *is* the work; a handover's task belongs to its report, so that path steps aside for it. A `running` row is left to the outcome and lost-run handling above. A `blocked` row stays `blocked` whatever the task says — an Inbox answer the executor got during that turn moves the task back to `in_progress` with no report behind it, so it is put back — and waits for the requester's revision. Any other row whose task is no longer `in_progress` already says what its report said: a `failed` report's error stays an error, which a revision may still retry. This was a real defect, watched on 2026-09-18 against a real orchestrator: a `blocked` report was applied mid-turn, the turn's end completed the task a moment later, and the requester's `revisions/001.md` was then refused as arriving on finished work — the handover hung for good. A `failed` report lost its retry the same way. The Inbox still ends two kinds of handover task, because nothing else would: one the user started by hand from its task page (a claimed brief, a gate never answered), and a settled executor chat the user reopened by talking on in it.
...
- **A resolved origin task is a delegation link, not a parent.** The task page exposes Delegated from and Delegated to; the canonical delegation owns the chain depth and root, so ordinary Parent task/Subtasks navigation keeps its existing meaning.
...
- **The requester is taught both, not left to guess.** The requester section on every folder prompt says to answer a `blocked` report — or add to work under way — with `revisions/001.md` (then `002.md`) carrying the `cinna_handover: 1` marker, and in so many words *not* with a new brief; and to give every brief of a fan-out the same `group: <id>` so it is woken once. The `blocked` packet repeats the first with the exact path, `.cinna/handovers/<id>/revisions/NNN.md`, and the marker. Both are there because a real orchestrator told only "a new brief or a revision" answered a question with a second brief — a new task in a fresh conversation, the first one left blocked for ever — and fanned the same work out to two projects with no group, so it was woken twice. The texts promise delivery into the executor's own conversation only *where Cinna runs the executor*: an executor outside the app reads the file itself.
...
- **Kit agents are not file-channel targets.** A kit folder is published and Cinna already writes into it, so a `.cinna/handovers` there would travel to whoever installed the kit. Kit agents, the model coordinator and scripts are all perfectly good *requesters*. The bus reaches kit targets through its local channel.
- **Remote agents are not file-channel targets** — A2A, Managed and WebSocket ACP agents have no folder on this disk to hold a brief. Supported remote task adapters provide the bus's cloud channel.
...
- **No new Inbox component, no new ask kind, no new task origin.** `tasks.origin` stays `local`; the handover row links the folder and its canonical delegation links the task, requester and return lifecycle.
...
requester (agent / terminal / script / person)
   ...
   + minute scheduler (activation, window focus, wake)
                    |
              handover intake  ──► handovers row + delegation + task (new)
                    |
         ┌──────────┴───────────┐
