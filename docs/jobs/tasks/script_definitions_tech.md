...
- Preload: `src/preload/index.ts` reuses shared job create/update DTOs. Renderer: existing Jobs and Task pages receive data; there is no script editor or graph view. The main-dispatch hook is documented in [script execution](script_execution_tech.md).
...
No extra descriptor keys are accepted. Definitions store these descriptors directly; validation does not look up agents, fetch cards or make their availability a claim. Existing job `sync_deps` and attachment joins are not rebuilt from script aliases; runtime resolves the definition aliases separately.
...
Templates support only `{{goal}}` and `{{step_id.text}}`, allowing whitespace inside the braces. An output reference must name a direct or transitive dependency. Unknown expressions and unmatched double braces are rejected. `expandScriptTemplate` requires each referenced output to be an own string-valued property and inserts it exactly once; inserted braces remain literal. Expanded text may not exceed 64,000 characters. `compactScriptOutput` limits context output to 16,000 characters, including an explicit shortening notice. The [script runtime](script_execution_tech.md) uses these helpers for isolated step conversations and dependency scheduling.
...
- `jobsRepo` create/upsert and `taskRepo` create/upsert carry script fields explicitly. Runtime progress is not stored in script JSON. Coordinator and script checkpoints remain device-local; the separate `task_script_runtimes` table is documented in [script execution](script_execution_tech.md).
...
`job:execute` admits explicit supported routers through the matching main-owned engine, described in [script execution](script_execution_tech.md). Ordinary null-router jobs retain the legacy dispatch path. `jobService.executeLocal` validates any non-null runtime definition and refuses with “This job requires the autonomous job executor.” before creating a chat, task or attempt. The remote execution branch also validates unexpected runtime data rather than ignoring it. `taskExecutionService.start` refuses both `router === 'script'` and any non-null script payload, preventing a script-bearing task from falling through ordinary Continue.
...
- `readyScriptSteps(script, states)` returns pending steps whose dependencies are completed, in definition order. It neither claims nor dispatches them; scriptRuntimeService performs durable claims and dispatch using this selection.
...
The existing job form still edits ordinary prompt/agent/mode/connector fields. It offers no versioned script authoring controls. Run starts stored explicit definitions in main. JobDetail shows the explicit router, including a display-only Script routes badge; ordinary dependency setup panels describe attachment joins, while script aliases are checked by main at admission.

## Configuration

Definition bounds are constants in `scriptRouter.ts`. Execution shares the autonomous task/agent queues and supported turn/time budgets; see [runtime configuration](script_execution_tech.md#configuration). No schedule or environment variable is introduced.
