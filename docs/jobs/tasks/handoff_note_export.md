...
- **Transactional acceptance exports only after commit.** `taskService.beginDesktopChat` binds the task inside `messageRepo.saveUser`’s transaction. It returns a DTO without exporting; `taskExecutionService.start` exports after acceptance resolves. A failed acceptance rolls back the message and task binding, and must not leave a file claiming work started.
- **The export follows task writes after acceptance commits, not only the note write.** Five of the frontmatter fields — `title`, `status`, `assignee`, `parent`, `updated` — change from somewhere other than the note, and **nothing ever reads the file back**, so nothing would ever correct a stale one. Exporting only when the note was edited would have left a file claiming `in_progress` under a task that finished an hour ago, which is worse than no file at all. Fourteen of `taskService`'s tails return through the `written(userId, row)` helper — including the four that write a binding, one of which sets the short code that is itself frontmatter; the two that return a DTO without writing are the no-ops (an `update` with an empty patch, an `applyRunState` that finds nothing to act on), which changed no row and so can change no file. Three methods keep the file in step without being one of those tails: `adoptUnclaimed` calls the same helper once per task it claims at enrolment, and the two [cross-device](cross_device.md) apply methods do it themselves, because `applySyncedTask` has a case the helper does not — a *soft-deleted* task that arrives with a note still has one, so the file has to be removed rather than re-exported.
- **The guarantee is exactly as wide as the service.** Every claim above holds because `taskService` is the only writer of the `tasks` table — a write that went through `taskRepo` directly would skip the export, and a write to a soft-deleted row would *resurrect* its file. Anything that writes tasks has to go through the service or bring that protection with it. [The remote sync service](remote_sync.md) does: every binding write, every pulled snapshot and every cleared dirty marker is a `taskService` call. **So does the apply path for a task arriving from [another of the user's devices](cross_device.md)** — `sync/collections.ts` calls `taskService.applySyncedTask` / `taskService.removeSyncedTask` and never `taskRepo`, which is why both are on the pinned list in `taskService.test.ts` that classifies every method as a read, immediate-export write or deferred-export write. The soft-delete case is now guarded twice rather than once: `taskRepo.update` grew its own `deletedAt` filter, so going round the service has to be wrong in two places to resurrect a file — the repo filter is a second lock on the same door, not a substitute for the service's own lookup.
...
- **Deleting a task removes the file.** The row survives, soft-deleted — that is how the delete reaches the user's other devices, as an ordinary upsert carrying `deleted: true` rather than as a sync tombstone — but the file is a view of a task the user can no longer open, so it goes. A soft delete is still a delete to anything reading the folder. A delete that arrives *from* another device removes it the same way. A hard-delete tombstone (which nothing in this build writes for a task) removes it **only when a row of this profile's actually went**: `<userData>/tasks/` is shared by every profile on the install and the file is keyed on the task id alone, so an id belonging to another profile would otherwise leave that profile's row intact and take its note.
...
- **A task id is validated before it becomes a path segment.** Ids are usually nanoids, but a row is also written under whatever id a sync payload carried (`taskRepo.create` takes an `id`, and `upsertFromSync` writes under the one that arrived), which makes this the only place a value from another machine becomes a filename — and the only one of the five synced collections' primary keys that does. `SAFE_ID` in `taskFileService` is the guard the other four need no equivalent of. Anything outside `[A-Za-z0-9_-]{1,128}` is refused and nothing is written. The warning fires **once per id** (capped at 100 remembered ids), because the export runs on every write and a repeating warning about a condition the user cannot act on just fills the log they would otherwise read.
...
taskService.<any write> -> taskRepo (SQLite, committed)
                        -> written(userId, row) -> taskFileService.exportHandoff(dto)
...
sync apply (a peer's row)
  taskService.applySyncedTask -> taskRepo.upsertFromSync
                              -> exportHandoff, or removeHandoff for a row
                                 that arrived already deleted
  taskService.removeSyncedTask -> taskRepo.deleteOwned
                               -> removeHandoff only if a row of ours went
```
...
- `src/main/services/taskService.ts` — `written(userId, row)`, the tail every mutating method returns through, plus `applySyncedTask` / `removeSyncedTask`, which keep the file in step for a row that arrived from another device. The user id is a parameter because the DTO the helper builds now answers `runsHere`, which needs this profile's sync device id
...
- [A Task on the User's Other Devices](cross_device.md) — the sync apply path that also has to keep this file current, and why it goes through the service
