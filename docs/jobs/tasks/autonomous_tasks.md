...
1. In an existing chat with a **<conductor> routes** badge, open the composer’s **+** menu and choose **Run on its own…**. Review the goal, initially copied from the composer. **More options** exposes a turn limit and time limit, defaulting to twenty turns and sixty minutes.
...
- **Completion has a saved answer.** A trusted finish summary becomes the final assistant text even when the runtime stops before emitting it. Existing final text ending with that summary is not duplicated, and a user cancellation cannot manufacture completion.
...
- **Sidebar results belong to the runner.** Working reservations also keep the sidebar spinner and interrupt action across chat switches. The runner publishes completed, failed or needs-input outcomes only at its own terminal/wait/interruption boundary; a time/turn limit is failed, while user Stop clears the unread indicator. Stopped conversations become deletable. See [Sidebar Session Status](../../chat/session_status/session_status.md).
...
- **One level of hierarchy, with one bounded exception.** A delegated agent is a tool call inside its coordinator's turn, and a task's parent chain is one level deep — a subtask has no subtasks of its own. [File handovers](file_handovers.md) are the explicit exception, and they are capped rather than open: a brief carries the requester's depth plus one, and above depth **2** it is recorded and refused with a visible reason. That cap is also what stops two folders handing work to each other for ever. A handover whose requesting task is *itself* a subtask still runs; it simply hangs off nothing, and the row says so.
- **A row the desktop wrote is shown as the desktop's.** A runner's continuation prompt and a handover's returned report are `system` rows, and the transcript draws them as a collapsed **Cinna Desktop · …** block rather than as speech. Until it did, they fell through to the generic tail and rendered as an assistant bubble — a message nobody in the conversation said, looking exactly like the agent saying it. Collapsed by default, because what the agent *did* with it is the interesting part.
...
- [File handovers](file_handovers.md) — the asynchronous, file-borne channel between projects, the depth cap above, and the other durable Inbox gate.
...
A coordinator may be an enabled Local folder/stdio ACP agent or a chat-owned runtime. No SDK adapter or API credential is required merely to own the coordinator role; actual engine readiness still applies. Synthetic No tools modes are refused at admission with a request to choose Connected tools, because an engine without runner tools could never call finish or ask_user.

The ACP bridge exposes fixed runner controls, preserves the captured coordinator agent on handback, and consumes a persisted tool-call budget in addition to owner-turn/time limits. `update_task` and `finish` are not counted against that budget: they do no outside work, and a coordinator that had spent its last call must still be able to record progress and finish rather than be stopped with its result unsaved (the 100-call per-turn ceiling still bounds them). A follow-up run the engine opens between the user's turns carries no budget of its own, so it is given the task's — otherwise it would run on the per-turn ceiling alone and could exceed the task's limit. That holds only while the task's current owner is the coordinator and the coordinator is the agent running: after a handoff to a specialist, a follow-up the coordinator's session starts between turns is capped only by the 100-call per-turn ceiling. A nested specialist question releases the turn into a durable Inbox wait; its resumed result names the earlier tool call before coordination continues. Enabling AI routing alone does not start autonomous execution.
