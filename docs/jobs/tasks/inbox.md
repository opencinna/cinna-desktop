...
6. Below the asks, **Recent tasks** lists root work, including tasks with no ask and no job. Clicking a row opens the task page. A task with neither a parent nor a job — or whose job has been deleted — offers **Back to the Inbox** from there.
...
- **Show more keeps the button under the pointer.** The new rows would push the button ten rows down and put a task row where it was, so a second click would open a task nobody chose. The page scrolls by exactly the added height. This is allowed because it responds directly to the user's click. On the last page the button itself disappears, and scrolling by the rows alone would then leave a task row where it was, so once the list has been expanded an **All N shown** line of the button's height fills its slot and lands under the pointer instead.
...
- **It is not the Subtasks list, but it shares its row and its rules.** A task page lists its children through `TaskList`: the same `TaskRow` at the page's compact scale, and the same fixed order and in-place Show more through `useTaskRowsInPlace`. What stays separate is the page size (twenty), the empty copy, and the half-failed remote refresh only children have — saved rows stay openable above a retry line.
...
- `src/renderer/src/components/tasks/useTaskRowsInPlace.ts` — the first-seen order and the in-place Show more, shared with a task page's Subtasks.
...
- `src/renderer/src/components/tasks/TaskView.tsx` — Inbox navigation in a separate slot from remote takeover. The header's single back arrow is **Back to the Inbox** only for a task with neither a parent nor a live job; otherwise it is **Parent task** or **Back to {job}**.
