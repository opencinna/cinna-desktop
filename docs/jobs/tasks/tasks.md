...
1. **Browse work.** Open the Inbox and use **Recent tasks** below the asks to open a root task, including work with no job or open ask. A root page lists Subtasks; a child page offers **Parent task**, a job's task offers **Back to {job}**, and a task with neither offers **Back to the Inbox**. Finished children can be discovered from the parent even when the service’s active list omits them.
...
- **Browsing does not start work.** Root and child rows open the same task page. Both lists exclude archived records by default and put the most recently updated work first. Recent tasks shows ten rows with **Show more tasks** for ten more, and keeps the order it first showed for as long as the Inbox is open, so a poll cannot move a row under the pointer. Subtasks shows twenty with increments of twenty. See [Recent Tasks](inbox.md#recent-tasks).
