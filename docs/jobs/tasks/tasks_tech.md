...
| Services | `src/main/services/taskService.ts`, `src/main/services/inboxService.ts`, `src/main/services/runExecutionService.ts`, `src/main/services/remoteInboxService.ts`, `src/main/services/taskSyncService.ts`, `src/main/services/taskSyncScheduler.ts`, `src/main/services/taskFileService.ts` |
...
- The later orchestration phase still owns autonomous task execution, handback, script routing and attach/replay; the existing shared executor only continues an accepted turn; subsequent cleanup/protocol/new-driver work is separate from this scheduler.
