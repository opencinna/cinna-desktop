...
- **This is one-turn continuation, not autonomous orchestration.** It creates a parent for persisted asks and continues their owner. The separate [task-start service](tasks_tech.md) supplies explicit Continue after takeover. The separate [autonomous runner](autonomous_tasks.md) supplies coordinator handback, durable gates and consecutive owner turns; [script execution](script_execution.md) supplies isolated child conversations and graph dependencies. [Live-run replay](../../chat/messaging/live_runs.md) independently attaches the selected conversation.
