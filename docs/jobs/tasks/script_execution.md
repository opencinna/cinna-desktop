...
- **Sidebar outcomes follow each conversation's work.** Working roots/steps retain their spinner across selection changes. The controller publishes latest unread outcomes for the root and relevant children, including human gates; the root needs its own result because it has no agent turn. User Stop removes pending-result indicators, while limit failures remain failed. Already completed siblings retain their result and read state. See [Sidebar Session Status](../../chat/session_status/session_status.md).
