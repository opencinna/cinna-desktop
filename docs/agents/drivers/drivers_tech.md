...
- `src/main/ipc/agent_a2a.ipc.ts` —
  ...
  - `agent:answer-request`: parses the payload, then calls `inboxService.answerFromTranscript`; durable rows share Inbox commitment and refused existing rows never fall back to live delivery
...
| `agent:answer-request` | invoke | `AskAnswerPayload → InboxAnswerResult` | Shared durable Inbox path; rowless fallback only for ACP-origin registrations |
...
| `respondAsync?(ask, resolution, context)` | Optional remote acceptance only; returns accepted/not_sent/uncertain, never releases the local park. Context supplies its cancellation signal |
