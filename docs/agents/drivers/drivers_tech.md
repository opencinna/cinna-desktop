...
- Shared: `src/shared/agentDrivers.ts` holds AgentDriverId (a2a/acp/managed), AcpLauncherId (opencode/claude/codex/custom; gemini recognized but unimplemented), capabilities/readiness DTOs and guards, including `readinessBlocksTurn(readiness, capabilities)` (true for any non-null state but `ok`, except `credentials_needed` when `capabilities.cwd`; covered by `agentDrivers.test.ts`). `src/shared/runEvents.ts` is the run event vocabulary.
...
agentReadinessService holds main-memory cached results, schedules enabled rows without blocking list responses, discards superseded checks and broadcasts visible changes. A2A probes card/auth with a deadline; OpenCode uses folder readiness without downloading on a list read; Claude adds installed/login rungs; Codex adds a login rung and refuses as `not_installed` only when its managed install **failed** — a CLI not fetched yet stays `ok`, since the turn fetches it, and a fresh check retries a failed install without waiting for the download. Both are still asked when the folder is `credentials_needed`; a non-`ok` launcher answer replaces that warning. Managed validates local credential/configuration availability. Custom ordinary reads return its latest binding-keyed check or null; only explicit Test/Check again initializes a command.

`useComposerReadiness` returns `notice` (any non-`ok` answer, which drives the warning panel and its action) and `refusal` (a notice `readinessBlocksTurn` blocks for the agent's capabilities, which alone sets `blocksSend` and Send's tooltip). The composer refuses the directly addressed agent on an established refusal and retains the draft; a folder agent's `credentials_needed` notice warns with Send enabled. Null never refuses. `ChatWorkspace` dims example prompts on `readinessRefusal`, `TaskView`'s start picker filters with `readinessBlocksTurn`, and `taskExecutionService.start` throws `invalid_input` on the same test. Bare catalog commands remain eligible independently of model readiness. Tool specialists report failure through their tool result. Cinna reauthentication is selected by capabilities.auth, not source. useAgents invalidates with cancelRefetch false so multiple mounted listeners do not restart the same fetch.
