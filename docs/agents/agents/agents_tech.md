...
- `cancel(requestId)` — Aborts the turn's `AbortController`, and nothing more. The A2A driver's own abort listener sends `cancelTask` once a task exists, so a user's Stop and an orchestrator abort cancel the same way and the request goes out once.
