...
Status sources own reported data and refresh policy independently of agent transport. The service combines the folder and Cinna list legs; per-agent requests resolve an optional source from a fresh owned row.
...
| Purpose | File |
|---------|------|
| Service — list aggregation, scope resolution, intent validation and degradation | `src/main/services/agentStatusService.ts` |
| Optional status-source contract and ownership factory | `src/main/agents/status/contract.ts`, `src/main/agents/status/index.ts` |
| Folder and Cinna per-agent policies | `src/main/agents/status/folderStatusSource.ts`, `src/main/agents/status/cinnaStatusSource.ts` |
...
| Snapshot, refresh intent and descriptions | `src/shared/agentStatus.ts` — `AgentStatusSnapshot`, `StatusRefreshIntent`, `isStatusRefreshIntent` |

### Preload

| Purpose | File |
|---------|------|
| Bridge API | `src/preload/index.ts` — `api.agentStatus.list()`, `api.agentStatus.get({ agentId, intent? })` |
| Snapshot type | `src/preload/index.ts` re-exports `src/shared/agentStatus.ts`; main producers use the same type. |
| `remoteError` on the list result | `src/preload/index.ts`; partial remote failure remains separate from per-agent snapshots. |

### Renderer

| Purpose | File |
|---------|------|
| Data hook (batch poll, re-raises partial failure) | `src/renderer/src/hooks/useAgentStatus.ts` — `useAgentStatus()` |
| Per-agent force refresh | `src/renderer/src/hooks/useAgentStatus.ts` — `useForceRefreshAgentStatus()` |
| Passive and after-turn reads | `src/renderer/src/hooks/useAgentStatus.ts` — `useRereadAgentStatus()`, `useAfterTurnAgentStatus()` |
| Mass refresh intent | `src/renderer/src/hooks/useAgentStatus.ts` — `useForceRefreshAllAgentStatuses()` |
...
No status schema changes. The remote leg matches backend UUIDs against `agents.remote_target_id`; the folder leg reads `agents` rows with `source = 'folder'` and takes everything else off disk. **Files are the truth, the database is an index** — nothing here writes to SQLite.
...
| `agent-status:get` | handle | `{ agentId, intent?: StatusRefreshIntent }` (default `read`) | `{ success: true, item: AgentStatusSnapshot \| null }` or `{ success: false, code, error }` |
...
### Refresh intent contract

`StatusRefreshIntent` is `read | manual | batch | after_turn`. Both IPC and the service validate it; omission at IPC defaults to `read`. The source owns the mapping:

| Intent | Folder | Cinna |
|---|---|---|
| `read` | Read saved STATUS.md | Read cached status |
| `manual` | Run declared command, then read | Refresh environment status |
| `batch` | Read saved STATUS.md | Refresh environment status |
| `after_turn` | Read saved STATUS.md | Refresh environment status |

`refreshDescription` is required on each shared snapshot and describes the explicit per-agent action. Main supplies it; `statusViews.tsx` displays it directly. Renderer ID prefixes and driver names do not select policy.
...
| `GET /api/v1/agents/{agent_id}/status?force_refresh=<bool>` | `cinnaStatusSource.read(intent)` | Only `read` uses false; other intents refresh the environment. 429 returns `null`. |
...
| Method | File | Purpose |
|--------|------|---------|
| `agentStatusService.list(scope)` | `src/main/services/agentStatusService.ts` | Folder leg first, then remote; returns items plus optional remote failure. |
| `degraded(err)` | `src/main/services/agentStatusService.ts` | Keeps folder rows with `remoteError`; rethrows if no rows survive. |
| `listFolderSnapshots(defaultUserId)` | `src/main/services/agentStatusService.ts` | Reads all indexed folder statuses; no commands; one timestamp per batch. |
| `folderStatus(defaultUserId, agentId, name, forceRefresh)` | `src/main/agents/status/folderStatusSource.ts` | Internal implementation selected by the source; manual refresh runs the command before reading. |
| `agentStatusService.get(scope, agentId, intent)` | `src/main/services/agentStatusService.ts` | Validates intent, resolves the owned row, asks `statusSourceFor` for its optional data source. |
| `statusSourceFor(userId, agent)` | `src/main/agents/status/index.ts` | Folder source for folder rows; Cinna source for synced remote rows with a remote target; otherwise no source. Driver identity alone grants neither. |
| `getCinnaContext(userId)` | `src/main/agents/status/cinnaStatusSource.ts` | Captured owner’s Cinna URL/token; no context for a non-Cinna profile. |
| `errorFromStatus(status, statusText, url)` | `src/main/agents/status/cinnaStatusSource.ts` | Maps remote HTTP failure into `AgentStatusError`. |
...
| Field | Value | Why |
|-------|-------|-----|
| `environmentId` | `'local'` | `statusViews.tsx:281` reads `null` as "the remote environment is not running" and prints so; over a file read off local disk that is a visible falsehood. A non-null sentinel says "not applicable" without adding a field to the shared snapshot type. |
...
|------------------|------|------|
| `useAgentStatus()` | `useAgentStatus.ts` | `refetchInterval: 45_000`, `staleTime: 15_000`, `refetchOnWindowFocus`. **No `enabled` gate** — it ran only for `cinna_user` (with the interval off otherwise), which made every surface dead for a local-only account. |
| `useForceRefreshAgentStatus()` | `useAgentStatus.ts` | Sends `manual`; only this intent can run a folder command. |
| `useRereadAgentStatus()` | `useAgentStatus.ts` | Sends `read`; passive data access. |
...
| `useLiveRunWatch` post-turn pull | `useLiveRunWatch.ts` | Calls `useAfterTurnAgentStatus` only for live terminal events; snapshot replay has no refresh side effect. |
...
- **Built Electron coverage is local.** `e2e/specs/agent-status-intents.spec.ts` covers real folder manual/bulk/read behavior, ACP tool attribution and restart persistence. A temporary measured UX review at 800/1200 verifies zero control movement across idle/pending/failed grid and detail states, retained markdown, actual exit-code errors and retry success. Remote Cinna wake/rate-limit behavior is outside that coverage.
- **`useTrayIcon` has no test of its own.**
- **`runStatusRefresh`'s `signal` parameter has no production caller** — `folderStatus` calls it with three arguments (`src/main/agents/status/folderStatusSource.ts`), so the abort branch is covered only by `statusRefresh.test.ts`.
