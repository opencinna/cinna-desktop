...
**Agents list** (Settings → Profile → Agents)

4. Each `AgentCard` reads its own `agent.remoteMetadata.bundle_version`. A catalog-install card whose install is behind gets an amber **"Update"** pill in the header (next to the green "Bundle" pill) and, when expanded, an amber banner **"Bundle update available · v1.0 → v1.2"** with an **"Update to v\<latest>"** button
