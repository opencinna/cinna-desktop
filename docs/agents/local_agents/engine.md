...
2. **Advanced** swaps the tier for the raw model list. It is remembered, but it is a preference and not a mode: the *manifest* decides which picker an agent opens on, since a panel showing a tier over a file that pins a model would misreport what the agent runs on. So the checkbox **converts** — a model becomes the tier it belongs to, a tier becomes the model it currently resolves to — and says in the status line what it did, because it is rewriting a file the user commits. A conversion with nothing to write still moves the view (the file keeps its tier and the line says which); a model that matches no complexity at all — a gateway's hand-written id — leaves the checkbox **ticked and disabled** with a standing line saying why, rather than a control that springs back under the pointer that just used it
...
5. Choosing writes a credential **name** and *either* a model id *or* a tier into `cinna-agent.json` — never a key, never this app's internal provider id, and never both at once
...
7. The write goes through the same stamped `local-agent:update-field` path as every other editable file, so an assistant editing the manifest cannot be clobbered
8. Clearing the choice removes the `runtime` block entirely and the agent falls back to the Default runtime, and below that to the Medium floor
...
A runtime is `{credential, model}` and it is resolved from two sources, in order: the manifest's own `runtime` block, then the **Default runtime** derived from the user's default chat mode. The manifest may name the model outright (`runtime.model`) or name a work complexity instead (`runtime.complexity`); both arrive at a model through the one chain below.
...
- **A credential of an unrecognised type is matched against every family**, not against one provider's. An `openai_compatible` gateway proxies other people's models, so the id is the only evidence there is about what it is
...
- The panel resolved a manifest's credential reference against **usable credentials only**, so a configured-but-keyless credential fell through to the *default* credential — and with it, the catalogue a tier is resolved against. The panel would have labelled the picker from a catalogue belonging to a credential the agent does not name
