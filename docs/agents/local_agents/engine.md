...
A credential is offered to the generator when `isCredentialUsable` says it can make a call at all: it is not flagged `unsupported` (an Anthropic OAuth token is not an API key) and it either has a stored key or is of a **keyless** type, which needs none. That predicate is shared with the "Runs with" panel and every picker on purpose — a hand-written `hasApiKey` here would have made the panel offer a local credential the generator then silently skipped. Own **and** server-managed credentials both count — excluding managed ones would make an account-provisioned machine unable to run a local agent at all. A key the keystore refuses to decrypt is skipped with a warning rather than failing the whole start; the other credentials still work.

**A credential's `enabled` toggle is not consulted here, and that is a gap rather than a design.** `collectEngineProviders` reads `listMerged()` and filters on `isCredentialUsable` alone, so a credential the user has switched off in Settings still becomes a provider entry — and for a *canonical* type, one carrying a working key, so a folder agent pinned to it keeps running after the user turned it off. It is less visible for a custom entry, whose `models` map comes from the adapter registry and is empty once the credential is unregistered. This is pre-existing and predates keyless credentials, which neither introduced nor widened it; it is recorded here because the paragraph above otherwise reads as a complete account of what is admitted. The fix is one condition, but it is a behaviour change for anyone whose agent is running on a disabled credential today, so it wants its own change rather than a quiet edit.
