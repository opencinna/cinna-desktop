...
- **Skip** — a folder agent the generated config deliberately left out, with a reason phrase the Runtime card renders
...
1. The user opens Settings → Local Agents (or the Runtime card on an agent page) and presses **Start**
...
2. Starting is an explicit act — the Settings button, the Runtime card's button, or (Phase 6) the first turn
...
### Choosing a runtime
1. The Runtime card on the agent page offers the credentials this app can actually call with, and the models the registry lists for the chosen one
2. Choosing writes a credential **name** and a model id into `cinna-agent.json` — never a key, never this app's internal provider id
3. The write goes through the same stamped `local-agent:update-field` path as every other card, so an assistant editing the manifest cannot be clobbered
4. Clearing both removes the `runtime` block entirely and the agent falls back to the Default runtime
5. The save fires a reconcile, which is a no-op unless the generated bytes actually moved
6. **Which credential of a type it is has a consequence the card does not show.** The first Anthropic credential becomes the engine's canonical `anthropic` entry and inherits the real context and reply windows of every model. A *second* one becomes a custom entry the engine has no catalog for, so the desktop has to declare those windows itself from a per-type floor — see [A second credential's models have to be told how big they are](#a-second-credentials-models-have-to-be-told-how-big-they-are). An agent on the second key is therefore capped at that floor rather than at what its model actually supports
...
2. The Runtime card renders that reason: "The engine skipped this agent because its runtime credential is not available to the local engine."
...
5. a per-agent runtime written from the Runtime card
...
**Do not add per-site `applyConfigChange` calls.** The one that exists (`local-agent:update-field`) is a latency optimisation, not the mechanism: it is fire-and-forget, never blocks the save, and is a no-op whenever the generated bytes are identical — which is the common case, since most manifest fields the engine never reads.
...
| Runtime resolves to no credential the engine can use, or to no model | **Skipped**, with a reason recorded on the generated config | The Runtime card's skip line |
...
A runtime is `{credential, model}` and resolution has exactly two steps, in order: the manifest's own `runtime` block, then the **Default runtime** derived from the user's default chat mode.

**There is deliberately no third step.** A "first credential that happens to have a key" fallback would make an agent run on a credential the user never chose and never saw — a billing surprise at best.
...
- Only when neither step yields a credential (or a model) is the runtime unresolved, and the reason line says which is missing
...
What the Runtime card writes is the credential's **name**, because the file travels: it is committed to the user's git repository, opened by their assistant, and uploaded to a Cinna instance on publish. Our own row ids are nanoids that mean nothing outside this machine's database.
...
- **Every renderer change in this phase is typechecked and bundled but never rendered.** The engine-path field, the Start/Stop button and the skip line went in on the compiler's word alone. This is the pre-existing jsdom gap described in [Agents Tab & Agent Page](agents_tab.md)
...
```
Settings → Local Agents          Agent page → Runtime card
   │  Start / Stop / engine path      │  credential + model pickers, engine line, skip line
   ▼                                  ▼
...
- [Agents Tab & Agent Page](agents_tab.md) — the Runtime card (now interactive), the Settings → Local Agents engine controls, and the stamped `update-field` path a runtime write reuses
