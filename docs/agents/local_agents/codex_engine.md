...
- **The folder prompt supplements Codex.** Kit prompt assembly and bare instructions-file assembly (`AGENT.md`, `AGENTS.md` or `CLAUDE.md`) feed developer instructions; Codex retains its own core instructions and normal configuration. Cinna does not apply Claude's settings isolation to Codex, and an empty injected MCP list does not promise that Codex has no configured MCP servers.
