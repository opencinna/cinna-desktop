...
- The button is additionally gated on the dependency having **resolved to a local id**, and routed on whether that id is a folder agent's — a folder agent goes to application **Settings → Default → Agents** for runtime setup, while an A2A dependency opens its own agent page in **Settings** mode with the Agents sidebar selected. MCP dependencies retain **Settings → MCP Providers**.
