...
| `isPermissionRequestTool(toolName)` | `src/shared/localAgentRequests.ts:52` | Exact match on the reserved name. The **only** permission detector; used in `MessageStream.tsx` by `renderRequestBlock`, the request branch of each render path and `renderSubagentGroup` |
| `isAskUserQuestionTool(toolName)` | `src/renderer/src/utils/askUserQuestion.ts:39` | `toLowerCase().replace(/[^a-z]/g,'') === 'askuserquestion'` — the existing normalising match, unchanged by this phase. Used in `MessageStream.tsx` by `renderRequestBlock`, `pairCommandTools` and the request branch of each render path |
| `isEngineRequestId(toolId)` | `src/shared/localAgentRequests.ts:70` | Separates a **live engine address** (`per_*` / `que_*`) from a cloud agent's question id, which is what makes a persisted local request block render read-only. `MessageStream.tsx` (`renderRequestBlock`, `pairCommandTools`) |
...
- `acpMessages.ts` — `AcpMessageStream`: `apply(notification)`, `applyExt(method, params)`, `askPermission`, `settlePermission`, `askQuestion(requestId, questions, callId?)`, `settleQuestion`, `note`, `toolName(toolCallId)`; plus `describeAcpToolCall` and `describeAcpPermission`. Pure — no Electron, no filesystem, no clock. Subagent lanes: module-level `laneOf(update)` reads `_meta.claudeCode.parentToolUseId`; `laneGeneration`, `laneMessage`, `toolLane` and `requestLane` hold each lane's own text generation, the message it is filed under (`messageOfLane`), the lane a call was first seen in, and the lane an ask belongs to (`laneOfRequest`); every lane part is written with `PARENT_TOOL_ID_METADATA_KEY`. `acpDriver.ts:finish` builds the turn's fallback `text` from main-lane parts only
...
- `src/main/agents/drivers/acp/acpMessages.test.ts` — the fixtures folded into parts: tool names, the text/tool ordering rule, unknown kinds ignored, and subagent lanes (the parent's text one part while child frames interleave, the child's text split around its own call, a subagent's asks and decisions in its lane)
