...
- `src/main/services/followUpTurnService.ts` — `followUpTurnService.open(request)`, `FOLLOW_UP_BUSY_POLL_MS` (1 s), `FOLLOW_UP_MAX_WAIT_MS` (20 min): the guards, the wait for a busy chat, and the run
...
| `FOLLOW_UP_MAX_WAIT_MS` | `followUpTurnService.ts` | 20 min | How long a follow-up waits for a busy chat; matches the turn ceiling |
