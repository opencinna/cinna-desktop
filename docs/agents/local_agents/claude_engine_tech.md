...
| Channel | Signature |
|---|---|
| `local-tools:claude-auth` | `() → ClaudeAuthStatus`. `requireActivated()`, then the shared probe's cached answer. Carries no account: `claudeAuth.ts` never reads the email or organisation id out of the CLI's JSON, so nothing at this boundary has to remember not to forward them |
...
- `parseClaudeAuthStatus(stdout)` — pure. `loggedIn` **must be a boolean** or the answer is `unknown`; `!!record.loggedIn` would read a missing field as a definite "logged out" and lock the user out of a working engine. Reads `authMethod` and `subscriptionType` and **nothing else** — the account's `email`, `orgId` and `orgName` are in that JSON and are never lifted out of it
...
- **The account behind the login is never read.** `claude auth status` answers with the account's email, organisation id and organisation name; `parseClaudeAuthStatus` lifts none of them out of the JSON, so they reach neither `ClaudeAuthStatus`, nor a log line, nor the renderer. **The absence of the read is the defence** — a rule saying "do not log the account" is one careless edit from being untrue, whereas a field nothing reads cannot leak. What crosses is the state, the CLI's own `authMethod` word, and a plan tier
