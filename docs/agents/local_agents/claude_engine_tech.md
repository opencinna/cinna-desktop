...
| `src/main/services/agentTurn/index.ts` | Production wiring: `claudeDeps` (including `approval`, which reads the setting off the agent's desktop state and applies the default), `claudeAgentTurnRunner`, `resolveTurnRunner()` and the private `folderEngine()` |
