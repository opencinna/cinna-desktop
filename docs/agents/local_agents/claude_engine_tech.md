...
| File | Role |
|---|---|
| `src/renderer/src/components/agents/local/RuntimePanel.tsx` | The "Runs with" panel: the `Runs on` select grouping AI credentials, Claude, and Codex — offered unless its managed install failed, and kept even then for an agent that names it — `EngineRow`, `ClaudeStatus`, `commitCredential()`, `changeRuntimeTarget()`, the Claude branches of the status line and the disabled/advanced gates |
...
|---|---|
| `RuntimePanel` | The first select is labelled **`Runs on`** (was `Credential`) and groups `On this machine` with a `Claude Agent` option valued `engine:claude`, and `AI credentials` with the existing list; the conditional `Codex CLI` group adds Codex unless its managed install failed, or whenever the agent explicitly selects it. The pending model placeholder was renamed `Model choice`, because two controls on one surface must not announce the same name |
...
| `acpLaunchers.test.ts` (Claude half) | the readiness rungs — a `logged_out` install refused **before anything is spawned**, an `unknown` probe running the turn anyway — the declared `elicitation.form` and AIR capabilities, `endsTurnsWithCostedUsage`, the `_meta` options (including `settingSources: []`, `strictMcpConfig` and no `allowedTools`), `CLAUDE_CODE_EXECUTABLE` naming the user's binary, and the mode the approval setting maps to |
