...
| `src/main/services/agentTurn/claudeMessages.ts` | `ClaudeMessageStream` — the fold from SDK messages to the A2A-shaped cumulative message, the `background_tasks_changed` level signal, `note()` / `noteBackgroundWait()`, plus `describeClaudeToolCall()` and `describeClaudePermission()` |
...
`system/init` yields `apiKeySource`, `model` and `cliVersion`; `system/background_tasks_changed` yields `backgroundTasks`; `result` yields `{isError, text, usage, numTurns}`; `auth_status` yields `authError`. **The runner consumes only `apiKeySource`, `sessionId`, `message`, `ended` and `backgroundTasks`.**
