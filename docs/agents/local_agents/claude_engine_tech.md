...
Three things about this engine's stream still need saying here:
...
- **A subagent's frames are a lane.** Child-session frames carry `_meta.claudeCode.parentToolUseId`; `laneOf` reads it and the translator writes the parts with `cinna.parent_tool_id`, in the message of the `Agent` call, with a generation of their own, so they neither cut the parent's text nor move its current message. The rule is shared, but only this adapter sends the key ([The Agent Turn](agent_turn.md#the-translator-maintains-a-cumulative-message-the-accumulator-computes-the-delta))
...
| `acpMessages.test.ts` | the fixtures under `__fixtures__/claude/` folded into parts: the tool-name precedence (`_meta.claudeCode.toolName` over `name` over the first title), tool calls arriving before the first chunk, a mode update, an unknown update kind ignored, and subagent lanes |
...
| `acpActivity.test.ts`, `acpActivity.driver.test.ts` | the recorded Claude background-shell, subagent and stop fixtures read into activity items, a subagent turn's saved transcript identical to the one produced before `nativeSubagentSessions`, and the recorded background subagent's two parent messages kept two parts (the second opening with `newPart`) when only lane parts came between them |
