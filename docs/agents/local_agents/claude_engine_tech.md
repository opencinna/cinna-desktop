...
| File | Role |
|---|---|
| `src/main/services/agentTurn/claudeAgentTurnRunner.ts` | `ClaudeAgentTurnRunner` — the third `AgentTurnRunner`. Readiness, lock, environment, the SDK call, the permission gate, the `permissionMode` mapping and the approval-fallback notice, session save, the never-throws contract |
...
| `src/main/services/agentTurn/claudeMessages.ts` | `ClaudeMessageStream` — the fold from SDK messages to the A2A-shaped cumulative message, the `background_tasks_changed` level signal, `permissionMode` off `init`, `note()` / `noteBackgroundWait()`, plus `describeClaudeToolCall()` and `describeClaudePermission()` |
...
| `src/main/agents/drivers/index.ts` | Production wiring: `claudeAuthProbe`, `claudeDeps` (including `approval`, which reads the setting off the agent's desktop state and applies the default), `claudeAgentTurnRunner`, the `claude` driver's fresh-aware `claudePath` / `claudeAuth`, and `driverFor()` |
| `src/main/agents/drivers/claudeDriver.ts` | `createClaudeDriver()` — the `claude` driver: a folder driver whose readiness adds the install and login rungs after the folder's |
| `src/main/agents/drivers/folderDriver.ts` | What the two folder drivers share: the per-turn reconcile against the folder's engine, folder readiness, and the answer path |
...
| `approval` | `desktopStateService.read(agent.path, agent.kind).claudeApproval ?? DEFAULT_CLAUDE_APPROVAL`, and the default again on any throw. The default is applied **here**, so the runner never sees "no choice made", and an unreadable state file still runs the turn |
...
| `claudeAuth` | `claudeAuthProbe.status()` — the shared `ClaudeAuthProbe`, built in `src/main/agents/drivers/index.ts` on `buildClaudeEnv(...)` so the probe runs in the environment the turn will. The `claude` driver's readiness asks the same probe, and calls `refresh()` instead when the check is fresh |
| `shellEnv` | `getShellEnv()` |
| `appVersion` | `app.getVersion()` |
| `query`, `turnCeilingMs`, `backgroundGraceMs` | tests only |
...
- **the prompt is an async generator, not a string** — it yields one `SDKUserMessage` (`{type:'user', message:{role:'user', content}, parent_tool_use_id:null}`) and then awaits `inputReleased`, a promise `releaseInput()` settles. The SDK closes the CLI's stdin when the iterable ends and never before; for a string prompt it does so at the first `result`, which is the defect in the header's finding 5. `drain`'s `finally` calls `releaseInput()` on every exit, so a throw, an abort or a generator that ended without a result cannot leave the iterable awaited for ever
- `liveTasks` / `holding()` — the CLI's live background set, **replaced whole** on every `backgroundTasks` update, filtered by `CLAUDE_TURN_FREE_TASK_TYPES` (`local_bash`, `shell`, `in_process_teammate`, `dream` — the CLI's own idle gate, read from the binary; `shell` is the SDK's label for `local_bash`). On a `result`: `holding()` empty → `releaseInput()`; otherwise `waiting = true`, a log line, and `stream.noteBackgroundWait(held)` ingested into the accumulator. `ended` is overwritten by each `result`, so the last is what `finish` reports
- the **grace** — an emptied set while `waiting` starts `setTimeout(releaseInput, backgroundGraceMs ?? CLAUDE_BACKGROUND_GRACE_MS)` (5 s, `unref`'d). `cancelGrace()` runs on any `stream_event` or `assistant` kind, on an `init` (detected through `update.apiKeySource`, since that is what `init` yields), and on a set that repopulates with a holding task. The clock is one-shot per empty set (`!grace` guard), so two empty messages in a row do not restart it
- `permissionMode` / `reportedModel` / `approvalFallback()` — set from the `init` update. `approvalFallback()` answers the model name when the agent asked for `auto` and the CLI reported anything else (`reportedModel ?? model ?? 'this model'`), and null otherwise — including when the CLI reported no mode at all, so an older CLI is not read as having fallen back. Logged at `info` when it happens
...
- exit order: **aborted → ceiling with a `result` seen → ceiling → error → `result.is_error`**. Abort first, so a deliberate stop is never reported as a failure. The ceiling-after-result branch is new with the open iterable: the model answered and what ran out of time was background work, so it is a `stream.note(...)` in the transcript and a `warn`, not the *"stopped responding"* error the bare ceiling still reports
- `finish(...)` is the single exit: it saves the session (a failure there is logged, never fatal — continuity is a convenience) and returns `parts`, `text`, `notices` and `contextId`, keeping whatever streamed even on the error paths. It also takes the observed `apiKeySource` and appends the billing notice when that is anything but `'none'`, and `approvalFallback()`, appending the notice keyed `claude:approval-fallback` — *"Automatic approvals are not available on `<model>`, so this turn asked before each action instead."* — when that is a model name. **All four exits pass both — completion, error, cancel and ceiling — because each observation is made at the init message, so the exit path a turn happens to take cannot decide whether the user is told.** A turn that reported the wrong account and then failed has still been billed to it, and the ceiling is the most expensive way to get this wrong: twenty minutes of work on an account nobody chose. The completion path alone carried it at first, and nothing failed, because the test written for it covered a turn that failed *before* init — a different case
- the permission gate posts `needs_input { resume: 'reply' }` after the registration and after the ask's block is ingested, so an answer sent the moment it arrives finds both; and `input_resolved` as soon as `handle.answered` settles — **before** the decision line, so the renderer stops offering the block before it reads what was decided. The promise settles once, so each is said at most once per ask. Both are gated by `open`
- a `finally` closes `open` **first**, then cancels every request this turn parked on — so the sweep posts no `input_resolved`: nobody answered, and the `done` or `error` above the runner already says the park is gone. A request left registered keeps reporting as pending, so a persisted block goes on rendering as answerable and answering it reports success into a turn that ended

The SDK options passed, and why each is not a default: `prompt` (**an async iterable, never a string** — a string closes stdin at the first `result`), `cwd` (the agent folder), `pathToClaudeCodeExecutable` (**the user's binary, never the SDK's bundled one**), `systemPrompt` (a plain string, never the `claude_code` preset), `model` (omitted entirely when the runtime names none), `settingSources: []`, `strictMcpConfig: true`, `mcpServers: {}`, `includePartialMessages: true` (without it the turn appears to hang until the first tool call and the text path never runs), `env`, `canUseTool` (**on both approval settings** — Automatic is a classifier in front of the desktop, not instead of it), `permissionMode` (`'auto'` for `approval === 'auto'`, `'default'` otherwise — the one place the desktop's two-valued choice meets the SDK's six-valued one; never `bypassPermissions` or `dontAsk`, which remove the callback from the decision), `abortController`, `agents` (**omitted rather than passed empty** when `readFolderAgents` found none, so a folder without them hands the SDK exactly what it was handed before the option existed), `resume`. **No `allowedTools`** — see the contract, and the `CLAUDE_SDK_CAN_USE_TOOL_SHADOWED` warning the SDK prints when one is present.

`readFolderAgents(agent.path)` runs once per turn, before `drain`, beside the system prompt and for the same reason — a definition edited while the app runs is on the next turn. Every skipped file is logged at `warn` with its reason; the names offered are logged at `info`.
...
`system/init` yields `apiKeySource`, `model`, `cliVersion` and `permissionMode` — the mode the CLI is **actually** running in, which is not always the one it was asked for, and `undefined` when the message does not carry one; `system/background_tasks_changed` yields `backgroundTasks`; `result` yields `{isError, text, usage, numTurns}`; `auth_status` yields `authError`. **The runner consumes only `apiKeySource`, `sessionId`, `message`, `ended`, `backgroundTasks`, `permissionMode` and — for the fallback notice's model name — `model`.**

`backgroundTasks` is `ClaudeBackgroundTask[]` — `{id, type, description}` from `task_id`, `task_type` (defaulting to `'task'`) and `description`. The SDK documents the message as **replace semantics**, and the translator honours that: it is the whole live set every time, never a diff, and never paired with the `task_started` / `task_updated` / `task_notification` edges, which are folded as unknown kinds. Entries with `ambient: true` are left out, as `sdk.d.ts` asks hosts to — they are the CLI's own housekeeping, and a set that carried them would never empty. An entry with no `task_id`, or a payload that is not a list, is tolerated: the set is what could be read, and an empty array is reported as an empty array rather than as nothing, because *empty* is the signal the runner's grace runs on.

`note(text)` and `noteBackgroundWait(tasks)` are the desktop speaking in the transcript. Each writes a `text` part carrying `cinna.content_kind: 'notice'` under the current message, keyed `note:<n>` on a counter so a turn that waits twice says so twice rather than editing the first line; `noteBackgroundWait` names up to three task descriptions joined by `; `, or *"N background task(s)"* when none has one. The accumulator routes a notice-kind part to `snapshotNotices()` and posts its delta to the port in arrival order, which is why the same notice sits mid-answer live and above the assistant row after a reload — `a2aStreamingService` persists notices before the assistant message.
...
`permissionMode` is the other, and reaches the user only when it differs from what was asked: `auto` on a model without a reviewer comes back as `default`, the CLI says so here and nowhere else, and the runner turns it into a notice — see [the rule](claude_engine.md#a-fallback-the-cli-reports-only-at-init-is-said-in-the-transcript).

`cliVersion`, `usage` and `numTurns` are folded and read by nothing, deliberately — no cost and no token figure reaches the transcript; `model` is read for the fallback notice's name and for nothing else. `authError` is the same as the unread three, for a different reason: **`auth_status` was never observed in any probe**, including the not-logged-in run, and it is folded because it is in the union and silently dropping the one message that names an auth problem directly would be worse than handling a message that may never arrive. "Folded but never consumed" is the accurate description of those four and not a defect in any of them.
...
**Dispatch.** `driverFor(agent)` (`index.ts:365`) reads `agents.driver` and nothing else, so no folder is read for an agent that has no engine to choose. The scanner writes that column from the folder's runtime (`folderIndexDriver`), and a runtime write in the app moves it at once.

**A turn still does not trust the column.** The folder driver's `run` (`folderDriver.ts:110`) reads the folder through `readFolder` (`index.ts:267`) and maps `runtime.engine` through `driverOfFolder`. If the folder names the other engine, it hands the turn to the sibling driver. **It keeps the stored driver when the folder cannot be read, or is `invalid` / `contract_too_new`.** A manifest caught mid-save reads as having no runtime, and that would otherwise send a Claude agent to OpenCode — while both runners refuse such a folder with the same sentence anyway.

**Readiness.** The `claude` driver's `readiness` (`claudeDriver.ts:26`) runs the runner's own two pre-turn checks ahead of any turn, for the composer and Settings:
- no `claude` → `not_installed`
- a definite `logged_out` → `not_logged_in`

Both carry the `describeEngineSkip` sentences the runner uses. A list-time check answers from the tool-detection memo and the probe's 30-second cache. A **fresh** check (*Check again*) runs detection again when no binary was found, and calls `claudeAuthProbe.refresh()`. See [Agent Drivers — Technical Details](../drivers/drivers_tech.md#the-claude-driver).
...
- **Two exported constants in `claudeAgentTurnRunner.ts`, neither read from the environment**: `CLAUDE_BACKGROUND_GRACE_MS` (5 000 — how long a turn stays open after the background set empties with no follow-up turn in sight; overridable per instance through `ClaudeTurnDeps.backgroundGraceMs`, tests only) and `CLAUDE_TURN_FREE_TASK_TYPES` (the task types that never hold a turn). `CLAUDE_TURN_CEILING_MS` is unchanged at 20 minutes
- **`CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` is deliberately not set** into the child. It would remove `run_in_background` from the CLI's tool schemas and make a string prompt safe again; the engine wants the harness as the user has it
...
- **Nothing is pre-approved by the desktop.** No `allowedTools`, so every gated tool the CLI hands on reaches `canUseTool`. Read-only tools are **not** gated by the SDK at all, and the [permissions](permissions.md) surface must not claim a completeness the mechanism does not have
- **On *Automatic* the CLI's own reviewer approves before `canUseTool` is consulted, and it was not seen to decline.** Seven probes — a force push, a global git config rewrite, a write under the home directory, an instruction injected through the system prompt — all ran with the callback never fired ([contract §10](claude_contract.md#10-auto-mode--the-classifier-in-front-of-canusetool-and-what-it-approved)). The callback is still passed, so the grants and the block stand behind whatever the reviewer would decline; but the setting is described everywhere as running without a gate, and the default is *Automatic* because *Ask every time* asked for `ls`
...
| `claudeMessages.test.ts` | text, tool calls, subagents, the wider union, the facts off `system/init` and `result` — including `permissionMode` read as reported, and **absent as absent** rather than as a fallback — and the background-task level signal — every live task by id, the empty set as an empty array, ambient entries left out, a missing id or a non-list payload tolerated, the `init` facts untouched by a message of the same type — plus the wait notice, one per wait |
| `claudeAgentTurnRunner.test.ts` | readiness on the login rung — a `logged_out` install refused **before anything is spawned**, an `unknown` probe running the turn anyway, a probe that *rejects* not becoming a thrown turn, and no install never consulting the probe at all; who paid for the turn — silent on `'none'`, the notice on anything else with the answer still intact, and the same notice on **each of the four exits** (one test per path, each mutation-checked separately), silent again when no turn ever reported one; the approval setting — `auto` → `permissionMode: 'auto'`, `ask` → `'default'`, and `canUseTool` a function on **both**; the fallback notice on a turn the CLI ran `default` after being asked for `auto`, absent when the CLI ran what it was asked for, absent on *Ask every time* where `default` is what was asked for, and present on a turn that failed after init; the options handed to the SDK (boundary closed, partial messages on, **no `allowedTools`**, the user's binary, a plain-string system prompt, a key-free environment, model omitted when none); readiness before the turn; never-throws across a thrown failure, a yielded error result and a lock refusal; cancellation including an abort that lands during pre-flight (**no child spawned at all**); the four negative cases of the session retry; the ceiling; the permission paths, including that an unreadable grant store asks rather than allows, and that an expired park denies and says so; **background work that outlives the model's turn**, driven by a stub that consumes the prompt iterable the way the SDK does and records whether it had ended at each point — the user message as the one item of an open iterable; stdin closed at the result when nothing runs; **open across a result while a task is live and closed after the last**; the last result reported, not the first; the grace ending the turn when no follow-up comes and cancelled when one starts; a background shell not waited on; a ceiling reached while waiting reported as a notice; the iterable finishing on a throw and on a cancel; and **the folder's own subagents** handed to the SDK, with no `agents` option at all for a folder without them |
...
| `folderDriver.test.ts`, `claudeDriver.test.ts` (`src/main/agents/drivers/`) | The reconcile in both directions, and its refusal to move an unreadable or invalid folder. The `claude` driver's readiness: not installed without asking the login probe, `not_logged_in` only on a definite logout, and `fresh` reaching both probes |
