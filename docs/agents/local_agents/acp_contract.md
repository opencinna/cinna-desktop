...
- Codex CLI 0.154.0-alpha.6.2, adapter 1.11.0: disabling shell alone does not remove native patch/read tools, and the adapter's ReadOnly mode still supplies workspaceWrite. The later isolated catalog-policy probe below establishes a restricted tool configuration on this exact binary; account authentication and real-provider behavior remain unverified.
...
The reproducible [Codex catalog-policy probe](../../../scripts/probes/runtime-conductor-codex.mjs) ran 39 checks against the real binary and a loopback fake Responses provider. All eleven bundled model slugs received transformed metadata through the startup `model_catalog_json` override. Feature overrides at both startup and thread creation removed native file, shell, image and delegation tools; fabricated native tool calls were rejected. Actual Cinna MCP calls returned the deterministic result. ACP reported `title: "mcp.cinna.probe"`, `rawInput: { server: "cinna", tool: "probe", arguments: {} }` and `_meta.is_mcp_tool_call: true`.

Inherited MCP isolation requires preserving explicit per-server disables when the adapter merges injected descriptors. The probe compares the original adapter with a scratch copy containing that narrow merge change; it never edits personal configuration or the installed CLI. Empty maps and startup CLI disables alone did not isolate inherited servers. A same-name native HTTP entry was replaced successfully, including when initially disabled; a same-name stdio entry failed closed at session creation before any model request. Two checks record expected stdio refusals, one from the scratch adapter and one from the production helper. Inline TOML tables safely disabled literal server names containing dots, quotes, a semicolon and a newline.
