...
### Renderer
- `src/renderer/src/components/agents/local/ReadOnlyCards.tsx:103-181` — `CommandsCard`. `run(name)` (`:110-136`) calls `useNewChatFlow().startNewChat({message: '/run:<name>', agentIds:[agent.id], mode:null, providerId:null, providers:undefined, allModels:undefined, mcpIds:[]})` after `setActiveView('chat')` + `setSidebarTab('chats')` — the same two navigation calls the sidebar chat shortcut makes, since a Run lands the user in a conversation too; both are made **before** the `await`, so the screen is already the chat the output streams into. `runningName` state disables every Run button while one is in flight
- `src/renderer/src/components/agents/local/LocalAgentPage.tsx` — Settings exposes the Commands tab. The header's Start chat switches back to the embedded composer without creating a conversation; the sidebar shortcut uses `pendingAgentId`, consumed by `ChatWorkspace`.
...
- `src/renderer/src/components/agents/local/LocalAgentPage.test.tsx` — Settings/Start chat switches between detail tabs and the embedded composer while retaining the shared header.
