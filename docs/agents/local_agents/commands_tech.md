...
- `src/main/services/localAgents/commandService.ts` (net-new) — the whole feature: catalog lookup, localisation call, subprocess execution, turn-lock, and shaping into `RunAgentTurnResult`
  ...
  - `resolveCommandRunner(commands, wireContent, agentOwnerId, agentId, fallback)` (`:473-486`) — the interception point itself, extracted as a pure function so the dispatch decision is directly testable without spinning up `ipcMain`. `commands` is the agent's `AgentCapabilities['commands']`, and `fallback` and the return value are a `TurnRun` (`(io) => Promise<RunAgentTurnResult>`). Returns `fallback` unchanged unless `commands === 'catalog'` **and** `matchRunCommand` hits

### Main process — call site
- `src/main/ipc/agent_a2a.ipc.ts:188` — `driverFor(agent)`
- `src/main/ipc/agent_a2a.ipc.ts:193` — `messageRoutingService.prepareAgentSend`, which is what produces `wireContent`
- `src/main/ipc/agent_a2a.ipc.ts:207` — `resolveCommandRunner(driver.capabilities(agent).commands, wireContent, agentOwnerId, agentId, (io) => driver.run(agentOwnerId, agent, {…}))`, called after `wireContent` is known and before `streamToAgent`
- `src/main/ipc/agent_a2a.ipc.ts:222` — `a2aStreamingService.streamToAgent({run, chatId, agentId, port})` — the swap that actually matters is which `run` reaches this call; see [Testing notes](#testing-notes) for why it has its own test file
...
- `src/main/services/agentService.ts:425` — `listFolderCliCommands(userId, agentId)`: locates the agent, reads the catalog, maps each entry to the shared `CliCommand` shape (`src/shared/cliCommands.ts`) with `command: '/run:${name}'`. Never throws — a read failure logs a warning and returns `[]`, since this backs a popup, not a page
- `src/main/services/agentService.ts:453` — `listCliCommands(userId, agentId)` — the one function `useCliCommands` calls through IPC. `:456-457` dispatch to `listFolderCliCommands` when `capabilitiesFor(agent).commands === 'catalog'`, **before** the `agent.protocol !== 'a2a'` short-circuit that once answered `[]` for every folder agent (the same dead-branch-until-a-phase-adds-it shape as `agentStatusService.list`'s remote-only filter)
...
### Renderer
- `src/renderer/src/components/agents/local/ReadOnlyCards.tsx:103-181` — `CommandsCard`. `run(name)` (`:110-136`) calls `useNewChatFlow().startNewChat({message: '/run:<name>', agentIds:[agent.id], mode:null, providerId:null, providers:undefined, allModels:undefined, mcpIds:[]})` after `setActiveView('chat')` + `setSidebarTab('chats')` — the same two navigation calls both **Start chat** buttons make, since a Run lands the user in a conversation too; both are made **before** the `await`, so the screen is already the chat the output streams into. `runningName` state disables every Run button while one is in flight
- `src/renderer/src/components/agents/local/LocalAgentPage.tsx:257-274` — the **Start chat** button, no longer `disabled`. `onClick` is `setActiveView('chat')` + `setPendingAgentId(agent.id)` — the same `pendingAgentId` path `AgentStatusOverlay`'s own "Start chat" already used for a remote agent, seeded into `pendingAgentIds` by `MainArea.tsx` — plus `setSidebarTab('chats')`, so the three calls match the sidebar row's chat button exactly ([Agents Tab & Agent Page](agents_tab.md#a-chat-from-the-row-without-opening-the-page))
- `src/renderer/src/components/chat/ComposerReadiness.tsx:76` — `isCatalogCommand(agent, typed)`: `capabilities.commands === 'catalog'` and `RUN_REFERENCE_PATTERN` on the trimmed text. It keeps Send enabled for a bare `/run:<name>` to a folder agent the composer otherwise refuses on readiness — see [Agent Drivers](../drivers/drivers.md#what-the-composer-refuses-and-what-it-deliberately-does-not)
- `src/renderer/src/components/chat/ChatInput.tsx` — `promptSourceAgent = boundAgent ?? selectedAgent ?? null`, then `useCliCommands(promptSourceAgent?.id)`. Unchanged by this slice; a folder agent reaches the popup only because `listCliCommands` now answers for one
...
No new channel. `agent:send-message` (`agent_a2a.ipc.ts:145`) is unchanged in shape. The only work this slice adds on that path is the `resolveCommandRunner` call and the swap of which `run` reaches `streamToAgent`, described above. `agent:list-cli-commands` (`src/preload/index.ts:582`, the existing channel `useCliCommands` calls) is unchanged in signature; only `agentService.listCliCommands`'s folder branch behind it is new.
...
- `src/renderer/src/components/chat/ChatInput.readiness.test.tsx` — *a catalog /run: always runs*:
  - a bare `/run:` goes through to a folder agent that is not ready
  - Send is enabled for it while the refusal notice stays
  - a message that only *starts* with `/run:` is still refused
  - `/run:` to an agent whose commands are not a folder catalog is still refused

  `ComposerReadiness.test.tsx` pins `isCatalogCommand` against the inputs main would hand to the engine instead
- `src/main/ipc/agent_a2a.commandDispatch.test.ts` (188 lines) — added as a review fix-up after a hand-run mutation (handing `streamToAgent` the unresolved turn instead of the one `resolveCommandRunner` returned) survived the full suite twice, because nothing had exercised the `agent:send-message` handler at all. Everything upstream and downstream of that one line is mocked to a canned happy path with distinguishable sentinel objects; the only assertion that matters is *which* sentinel `streamToAgent` receives — for a folder agent sending `/run:check`, and for a remote agent sending ordinary text. `resolveCommandRunner`'s own correctness is not re-proven here — that's `commandService.test.ts`'s job
...
- `src/renderer/src/components/agents/local/ReadOnlyCards.test.tsx` — `CommandsCard`'s Run button: switches to the chat view, moves the sidebar to Chats and starts a direct chat sending `/run:<name>`, disables every Run button while one is in flight and re-enables once it settles, and the empty-state copy with no Run button when the catalog is empty
- `src/renderer/src/components/agents/local/LocalAgentPage.test.tsx` — **Start chat**: switches to the chat view, hands the agent id to the pending-agent path and moves the sidebar to Chats, and asserts the button is enabled — explicitly named as pinning the flip from the prior hard-coded `disabled`
