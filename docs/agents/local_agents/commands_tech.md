...
Commands use normal run:start/watch (or lower-level run:send). runExecutionService checks catalog capability and selects resolveCommandRunner before passing run into streamToAgent. agent:list-cli-commands remains the catalog lookup through agentService.
