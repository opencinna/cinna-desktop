...
**Flow step** names the step of the Level 2 whole flow that exercises the entry — **A** plain chat on the Default runtime, then its AI title; **B** a specialist @-added mid-chat, called through Cinna’s MCP server; **C** continuity: a later turn remembers the first; **D** a specialist attached before the first turn. A dash means no step does, and the note says why. Two variants walk the same steps: `make e2e-one SPEC=runtime-flow` (the built app and the real pinned CLI against a loopback fake provider, no billing, opt-in) and `make live-flow ENGINE=codex` (the user’s real login, billed, manual).
