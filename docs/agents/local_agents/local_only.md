...
- It does **not** say "it will run on a device where that agent is set up." That names a device the app cannot know exists. The state is reachable by a **single user on a single machine who has never enabled sync**: `rebuildJobManifest` (`src/main/sync/manifest.ts:108`) runs on job create and on every agent/MCP/mode change, gated on neither sync being active nor the profile being a Cinna one, so attaching a folder agent and then moving or deleting its directory blocks the job on the only machine there is. The app now offers that deletion itself — the agent page's **Delete agent…** trashes the folder and lets the rescan prune the row. The job's `job_agents` link cascades away with the row, but the run gate reads the job's *stored* manifest, which still carries the `source: 'folder'` descriptor (and a later rebuild carries it forward rather than dropping it), so the job is refused with `incomplete_setup` rather than run agentless. Restoring the folder from the Trash lifts the block but does not re-attach the job — the `job_agents` row cascaded away and nothing recreates it — which is why the dialog says the job "will refuse to run" and not "until it is back". Reasoned from `unresolvableAgentLabels` and `rebuildJobManifest`'s carry-forward, not observed end to end.
