...
- `src/main/ipc/agent_a2a.ipc.ts:245` — `agent:answer-request`; `:251` — `pendingRequests.owner()`; `:313` — the call into `driverFor(row).respond`, or into `respondToOrphanedAsk` when the agent's row is gone. The conversion itself is `respondToAcpAsk` (`src/main/agents/drivers/acp/acpDriver.ts`) → `rememberIfAlways()`, between the registry lookup and the resolve — **synchronous, and the order is only safe because it is**: the grant is written before the park is settled, because the resolution has to carry `remembered` into the transcript
