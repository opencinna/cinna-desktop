...
3. **`always` must never be posted to the engine.** Two locks: `rememberIfAlways` on the answer path (`src/main/agents/drivers/folderDriver.ts:244`, reached through a folder driver's `respond`) converts it, and `LocalAgentTurnRunner.reply` (`localAgentTurnRunner.ts:704`) downgrades any stray one to `once` and logs at `error`. Removing either leaves a user-global row authorising every folder agent, and no test at the HTTP fake would see it
...
- `src/main/agents/drivers/index.ts:242` — `claudeDeps.approval`, which reads the setting off the agent's desktop state and applies `DEFAULT_CLAUDE_APPROVAL` there, on null and on any throw
...
- `src/main/agents/drivers/index.ts:168` — `localDeps.isGranted`, the reading half; `:300` — `rememberGrant()`, the writing half. It resolves the agent's **DTO** rather than just its path, to get the kind. It lives in the drivers' wiring, reached through a folder driver's `respond`, so `agent_a2a.ipc.ts` does not pull the folder stack into its import graph
...
- `src/main/services/agentTurn/localAgentTurnRunner.ts` — `AUTO_REPLY_RETRY_MS` (`:113`), `LocalTurnDeps.isGranted` (`:159`, now `(agentDir, agentKind, request)`), the `TurnStream` construction that injects it (`:353`), `park()` (`:589`), `autoAllow()` (`:643`), `deliverAutomatic()` (`:673`), `reply()` (`:704`)
- `src/main/services/agentTurn/pendingRequests.ts` — `Entry.request` carries the engine's ask; `RequestResolution` — re-exported from `src/shared/localAgentRequests.ts`, because an `input_resolved` stream event carries it to the renderer — has `remembered?` on the permission variant; `owner()` returns the ask alongside the ids
- `src/main/ipc/agent_a2a.ipc.ts:245` — `agent:answer-request`; `:251` — `pendingRequests.owner()`; `:313` — the call into `driverFor(row).respond`, or into `respondToOrphanedAsk` when the agent's row is gone. The conversion itself is `respondToParkedAsk` (`src/main/agents/drivers/folderDriver.ts:206`) → `rememberIfAlways()` (`:244`), between the registry lookup and the resolve
