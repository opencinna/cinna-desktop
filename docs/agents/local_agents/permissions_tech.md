...
- `src/renderer/src/components/agents/local/PermissionsCard.tsx` — the agent page's **Settings → Permissions** tab body. `:255` — `OpenCodeProfile`, the fixed profile paragraphs; `:351` — `ClaudeApprovals`, the two-setting paragraph, the **Approvals** select and its one-line error slot, rendered instead when `agent.runtime?.engine === 'claude'`
