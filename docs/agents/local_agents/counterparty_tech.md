...
- `src/renderer/src/utils/examplePrompts.ts:24` — `extractExamplePrompts(agent)` reads `agent.remoteMetadata?.example_prompts` and tests no `source`, which is why a folder agent reaches it unchanged. Three consumers: `ChatInput.tsx:504`, `src/renderer/src/components/layout/ChatWorkspace.tsx` (the shared new-chat tag cloud) and `hooks/useHintContext.ts:40`.
...
- `src/renderer/src/components/jobs/JobDetail.tsx` — `openSetup(dep)` routes MCP to settings `mcp`, folder ids to settings `local-agents` (visible Default → Agents), and resolved external agent ids to `external-agent` with `agentPageMode: settings` and the Agents sidebar selected. The id guard still prevents a setup link for an unavailable folder.
