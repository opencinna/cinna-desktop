...
2. Settings → Default → Local Development → Developer Tools → **Refresh** (beside the section title) drops the cached `PATH` lookups and re-detects
...
- **A bare folder is briefed from `README.md`, then its instructions file** — a different list, in a deliberate order. Those are the only two documents such a folder is known to have, and the README is the one written for whoever develops the agent: what it is, how to run it, what it needs. The instructions file — `AGENT.md`, `AGENTS.md` or `CLAUDE.md`, whichever the folder resolves to — is the agent's own instructions, which read to a builder as a job description rather than as a briefing. That holds even when it is an `AGENTS.md` or `CLAUDE.md`, the files a kit folder is briefed from first: in a bare folder they are what the agent is told, not a builder's guide. This is the same agent-role / builder-role split that keeps `README.md` out of the [assembled system prompt](bare_agents.md#the-prompt-is-the-instructions-file-and-nothing-else-the-folder-contains) — the two features read the same two files in opposite orders, on purpose
...
- [Bare Agents & External Roots](bare_agents.md) — an adopted folder is a registered root like any other, so it is an allowed area for this guard; and its init prompt reads `README.md` before its instructions file
