...
`dismissed` records "Not now" for the window. Explicit folder setup buttons use `reopen`; the former mount-triggered raising hook has no production caller, so refetching the list does not reopen the modal.

### `src/renderer/src/hooks/useLocalAgents.ts`
- `useRaiseAgentsHomeQuestion()` — retained helper calling `request(access)`, currently with no production caller. `LocalAgentsList` uses `useAgentsHomeQuestion` for inline state and explicit setup actions
...
| `LocalAgentsList` | The `+` opens Add an agent. Its New agent choice calls `onCreateFolder`, which closes the choice modal and calls `reopen(access)` only if folder setup is needed. External connections and Add a folder bypass this gate. The empty state reads "Your agents need a folder" with **Set one up** / **Pick another folder** — named for the question they open, not for the modal's own `Choose folder…`, which is one click further in (rule 10) |
