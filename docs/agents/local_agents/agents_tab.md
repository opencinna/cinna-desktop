...
The **Agents** sidebar brings folder agents and external connections together. Selecting a row opens a conversation composer with that agent selected; **Settings** exposes its configuration and folder viewers. The shared header keeps Open in and occasional actions within reach in either mode.

Folder creation and adoption build on [Agents Home, Scanner & Folder Index](folder_index.md) and [Open in…](open_in_tools.md). Installation-wide roots, runtimes and developer tools live in **Settings → Default → Agents**. The separate **Settings → Profile → Agents** manages the active Cinna server's catalog; external connection details belong to [Remote Agents](../remote_agents/remote_agents.md).

## A note on paths

Three trees are discussed here and their paths look alike, so they are written differently throughout — the same convention as [Agents Home, Scanner & Folder Index](folder_index.md):
...
- **Agents tab** — Fourth entry in the sidebar tab strip (`Chats | Jobs | Notes | Agents`). Folder agents, active-server agents, direct A2A connections, ACP connections and managed agents share name-only rows with stable type icons and a hover/focus chat shortcut. An `invalid` folder has no shortcut; the page explains its readiness
- **Agent page** — A shared header (type icon, name, description, folder path, **Open in**, **Settings / Start chat**, **⋯**) over two modes. Chat mode shows a compact runtime summary and the shared new-chat composer. Settings mode shows the full **Runs with** panel, whose first picker is **Runs on**, and six kit tabs: **Overview**, **Prompts**, **Commands**, **Schedules**, **Permissions**, **Folder**. Readiness and action feedback remain available in either mode
- **Bare agent page** — The same chat/settings shell for an adopted `AGENT.md` folder, with four Settings tabs (no **Commands** or **Schedules**). Overview contains the locally stored **Name** and the folder's read-only `Readme` when present; Prompts contains **Instructions**. Runtime choices are stored in Cinna rather than in the folder
- **⋯ menu** — What a user does to an agent *occasionally*: Rescan folder, Reveal folder, Open terminal here, Stamp identity (legacy folders only), and **Delete agent…** — **Remove agent…** for a bare agent — which always confirms
...
4. **Copy prompt for another tool** is the reach beyond what the desktop can launch: it puts a short briefing on the clipboard — where the folder is, what the agent is called, which file to read first — for an assistant running somewhere this app cannot start it. It is the one menu item that leaves the menu open, because a clipboard write changes nothing else on screen; the item itself reads "Copied" for a moment instead
...
This installation-wide entry is labelled **Agents**. **Profile → Agents** is a separate server-owned catalog, not a second location for folder configuration.

Three titled sections — **Agent Folders**, **Runtime**, **Tasks** — separate registered folders, execution defaults and autonomous admission. Shared development-tool inspection and the OpenCode executable override live under **Default → Local Development → Developer Tools**.

1. **Agent Folders** — while the agents home does not exist, a row at the head of the list holds its place: the state as the label (`No agents folder yet`, or which of macOS and the folder itself refused it), the path, and in the same row the button that resolves it (rule 12). The home is missing from the list, not the list itself — a folder the user adopted is still there and still theirs — and this is where someone who answered "Not now", or whose Documents folder macOS refused, comes to finish it ([The Agents Folder Question](home_access.md)). Then each registered root with its path, agent count and resolved kit contract version; Reveal on each; Forget on the added ones (the home cannot be removed); a **Rescan** button beside the section title, labelled rather than a bare icon, because it re-reads every folder and so belongs to the section rather than to the first row. An adopted folder carries an **Added folder** badge and reads `N agents · not a kit folder` instead of a contract version — there is no `.cinna-kit/` in it and none is coming, so naming a version would name one that governs nothing there. Not "read only": the badge's tooltip says Cinna installs nothing there and that the only file it writes is an agent's `AGENT.md`, and only when the user edits it on the agent page. Promising read-only would leave them with a modified working tree in a shared repository and a blocked **Update** two rows below. Under a root holding agents that are not in the list — removed, or never ticked when the folder was adopted — a line with the count that points to **Manage agents** in that row's controls, where the set is chosen. An **Add them** button used to sit there, and it could only add all or nothing when the set the user wants is usually neither. A capped root also reads "(the first found)". Under a root that is a git working tree: the update check ([Agents Folder Updates](folder_updates.md)). A failed row action such as Reveal or Forget replaces that row's own summary line, truncated with the full text in `title` — it used to be one shared message under the Add button, three rows from where the user acted and pushing everything below it down as it arrived
2. **Add an agents folder** — the compact secondary button beside Rescan in the Agent Folders heading; opens the OS directory picker in main, and a non-empty folder gets a native confirmation before templates are installed. This is still the *workshop* adoption; a folder that already holds agents is adopted from the Agents tab's **+** instead, and gains nothing
3. **Runtime** — what a folder agent that names nothing of its own runs on. Every standing explanation here — what a runtime is, how the default is chosen, what the credential pays for, where the binary comes from — is behind a (?) beside its label. Three explanatory paragraphs above three controls pushed the controls below the fold on every visit after the first.
   - **Default runtime** is a row of buttons (Claude Agent, Codex, Custom OpenCode). Each says two things: whether this machine has the runtime, and whether agents are set to use it. Pressing one the machine lacks installs it, after showing the command, and then selects it. Codex is selectable and runs folder agents through its installed CLI and the bundled ACP adapter. The first machine selection prefers installed Claude, then Codex, then OpenCode; established choices remain fixed.
   - **The Default runtime is decided once, then left to the user.** It is decided on the first launch that can answer. On a fresh install the first runtime found wins, so agents run before anything is configured. An install that already has folder agents locks to OpenCode whatever is detected, because moving those agents onto a subscription and a different permission reviewer because the app was upgraded would be a billing and safety surprise. After that nothing moves it but the picker: installing another CLI later changes nothing on screen.
   - **The lock keeps a pick made while it was deciding.** Detection is a login-shell probe, and the picker is live while it runs, so the lock reads the setting again once detection returns and keeps whatever the user chose in that window. Without that second read, the write that followed replaced the user's pick with the machine's guess.
   - **One status line under the buttons, about the selected runtime only.** For OpenCode it says which `opencode` was found and where from, that it is downloading, or that it failed, with **Try again** as a text action on the same line. For Claude Agent and Codex it says which CLI is installed, or that none was found. A binary nobody has resolved yet reads, muted, "Not resolved yet — the next agent chat resolves it." It is not a warning: it resolves itself, and an alarm over it on every visit teaches the user to skip the alarm for the state that does need them. The line is reserved at exactly one line because it has something true to say in every settled state, and every sentence is written to fit one line at the 800px minimum. The failure messages main composes are the only ones that may run longer, and they truncate with the full text in `title` rather than wrap.
   - **Credential OpenCode runs on** pins the key the OpenCode runner spends for agents that name no credential. It offers only credentials that can actually run, `isCredentialActive`, rather than offering one and then warning about it: the engine is not given a switched-off credential, so pinning one here would be a setting whose only effect is to break the user's agents at the next turn. The select still carries the pinned row as a synthetic option (suffixed `— inactive`), so it cannot render blank over a pin that is in force.
   - **A pin that has *become* unrunnable is a different situation and keeps its warning**, in three flavours: no usable key, switched off, and no longer on this machine. The warning is rendered only while one applies, last in the card, on one line. The credential's name is the part that truncates; the sentence saying agents will not run never does. Nothing is reserved for it, because a slot that was empty in the healthy state read as a card with the wrong bottom edge.
   - **Open agents with** selects an installed assistant/editor or Ask each time. Its auto-open checkbox stays below it and is disabled until a default resolves. The credential and opening-tool selects occupy the rightmost third of their Runtime rows, while default-runtime choice buttons keep their arrangement. Related Runtime rows use 16 px inset dividers.
4. **Tasks** holds Autonomous task concurrency in a separate card ([Settings](../../ui/settings/settings.md#autonomous-task-concurrency)).
5. **Default → Local Development → Developer Tools** reports other detected tools, the resolved OpenCode binary/version, the OpenCode Path override and the bundled kit contract. Runtime assistants remain beside their choice under Agents. See [Local Development](../local_dev/local_dev.md).

## Business Rules

- **Selection is distinct from agent type.** Folder and external sidebar names use the secondary text color while unselected and the primary text color on the selected page. External labels inherit the row color; an unconditional primary label made every remote row appear selected in dark theme.

### The page opens a conversation; Settings holds the controls and viewers

The shared header uses a neutral bordered **Settings** action while chatting and an accent **Start chat** action while configuring, on both folder and external agent pages. Styling follows the action currently offered; switching modes still preserves the composer draft.

Opening configuration on every selection put another action between choosing an agent and talking to it. The composer is therefore the default landing body, with the controls and file viewers behind an explicit Settings action.

- **The header stays shared across modes.** Open in and the ⋯ menu remain available. Chat mode places compact runtime badges below the path; Settings mode renders the full runtime form above the detail tabs. The no-description nudge enters Settings → Overview, and the readiness findings link enters Settings → Folder
- **The compact summary is informational.** It names OpenCode and the resolved credential, Claude Agent, or Codex, plus a model and relevant setup issue when known. Claude gets “with subscription” only for a logged-in auth result whose method is `claude.ai` or that names a subscription type. Unknown authentication must not become a subscription claim. The Claude model badge currently shows the complexity-derived alias; a manually declared model override is not reflected there. Codex shows its declared model or CLI default and reasoning effort, with installation/login remedies. Full runtime resolution and edit rules belong to [The Local Engine](engine.md)
- **The Runs with panel says one thing at a time, in a line that is always there.** It has a lot it *could* say — a refused save, what the last click on the secrets line did, a model it just dropped, what a picker conversion just did to the file, a credential the manifest names and this machine lacks, no credential at all, a credential with no usable key, a model registry still loading or failed, a model that belongs to another catalogue, a work complexity this credential lists no model for, a model the catalogue dropped and the sibling now running in its place, no model at all, no models listed yet, why Advanced is unavailable for a pinned model no tier describes, which tier resolved to which model when everything is fine, and the engine's own reason for skipping the agent. Stacked as separate conditional rows they changed the panel's height on every credential change, which moved the tab strip out from under the pointer that had just used the select. So there is one reserved fixed-height line, and the messages are **prioritised rather than concatenated**: what blocks the agent first and what the user can act on. The engine's skip reason sits below every warning the panel worked out for itself, so the one skip that would restate a message already on screen — "its runtime names no model" — never takes the line. Below the skip is the only entry that reports a *healthy* state: which tier this agent resolved to and what that costs. It is last because any warning above it matters more — a user whose credential is missing does not need to be told what Medium resolved to. What the pickers offer and what a credential change does to the model belong to [The Local Engine](engine.md#choosing-a-runtime). The line truncates with the full sentence in `title`, because at the 800px minimum window a long credential name would otherwise wrap into a second row and reintroduce the shift; the surviving half of each sentence is the half naming the action
- **The picker the panel opens on is decided by the file, not by the checkbox.** The second control asks how hard the work is — Simple, Medium, Complex — and **Advanced** swaps it for the raw model list, remembered for the next agent. That preference only ever breaks a tie: an agent whose manifest names a model always opens on the model list and one that names a tier always opens on the tier, because a panel showing a choice the file does not make misreports what the agent runs on. Ticking the box therefore rewrites the manifest — a model becomes the tier it belongs to, a tier becomes the model it resolves to right now — and the status line says which, since this is a file the user commits. Where a conversion cannot be made honestly it is not approximated, and it is not left to fail on click either: a model that matches no complexity — a gateway's hand-written id — leaves Advanced ticked and **disabled**, with a standing line saying why, because a control that snaps back teaches nothing and invites the same click again. The mechanics are in [The Local Engine](engine.md#choosing-a-runtime)
- **A link that names a file opens that file.** The panel's secrets line ends in "Add them in `credentials/.env`" — "Edit" once nothing is missing — and the click opens that file for editing. It used to reveal the folder around it, which stopped one step short of what the label promised, and on the agent that needs it most there was nothing in the folder to find: the scaffold writes `credentials/.env.example` and deliberately never the real `.env`, so an agent created a minute ago has no file to reveal. It is therefore **created on the click**, readable only by its owner, listing the variable names the manifest declares — each one **commented out**. The scanner counts a bare `VENDOR_PORTAL_TOKEN=` as a key that is set, so a helpful-looking placeholder would report every credential satisfied the instant the user looked at the file, and the readiness strip would fall silent with nothing filled in. That seed is the only write the desktop ever makes to the file, and it carries no value either; reading is unchanged — names out, never a value ([Agents Home, Scanner & Folder Index](folder_index.md)). `.env` has no default application on most machines, so a click the OS cannot honour falls back rather than dead-ending: on macOS to the default *text* editor, otherwise to revealing the file in the file manager
- **The click reports the two outcomes the user cannot see, in the line that is already there.** A reveal means the editor step did not happen, and a refusal — a read-only folder, an ignore rule that would leave the file committable — otherwise leaves the click completely inert; both take the panel's **one reserved status line**, never a new row, and rank directly under a refused save, because what the user has just clicked outranks a note about the write before it. A successful open says nothing at all: the file is open in front of them. The message is cleared by the next runtime save, so a sentence about `credentials/.env` cannot outlive an action since taken in the pickers above it, and the link is disabled while the click is in flight — the macOS fallback waits on the editor for up to fifteen seconds, and a second click in that window would open a second one
- **Creating the secrets file must not be the thing that breaks the agent.** A `credentials/.env` that no `.gitignore` rule covers is a validator **error**, an error makes the folder `invalid`, and an invalid folder is dropped from the engine's config and refuses a turn — so seeding the file blindly would let the click that exists to *fix* a missing credential be the click that stops the agent running. Where nothing covers the file, the contract's own `credentials/.gitignore` is installed — the contract's copy, since that rule already exists in four places and a fifth hand-written one is exactly how the four drift apart ([Kit Contract & Manifest Layer](kit_contract.md)). Where the folder un-ignores `.env` on purpose, the click is refused with a sentence naming the fix rather than leaving a committable secrets file behind. And because seeding is a write into the folder it takes the same per-agent **turn lock** every other write takes: mid-turn it refuses as busy instead of dropping a file under a running agent. Opening a file that is already there writes nothing and takes no lock
- **The Folder tab's Credentials card keeps the weaker promise, deliberately.** It reveals the `credentials/` folder and its tooltip says "folder" rather than naming a file it never opens: a `.env` that does not exist cannot be revealed — `showItemInFolder` on a missing path is a silent no-op — and a card on that tab has nowhere to report a failure. One affordance creates and opens, the other only points, and each says which
- **The header carries the description, or an honest blank.** A folder created from a name alone carries that name as its description, because the schema demands one; rendering it under the name says nothing. `describedAs` (in `src/shared/localAgents.ts`) returns `''` when the description is only the name repeated, and the header shows "No description yet — add one under Overview" instead. The **index row** uses the same function: the scanner writes `agents.description` as `null` for such a folder, so the composer `@` popup, the `[+]` picker and the Jobs agent picker, which read that column and never see the manifest, show no "Alpha — Alpha" sub-line either. One function, applied where the row is built and where it is rendered, so the surfaces cannot disagree
- **The readiness strip renders nothing for an `ok` folder.** A green banner on every healthy page taught users to skip the banner; stable type icons carry identity instead of readiness dots. Consequence, deliberate: a folder that is `ok` *with validator warnings* shows no strip either — the warnings are on the Folder tab's Validation card, in full, and the **Folder tab carries a count badge** (errors + warnings) so they stay discoverable without opening it. The strip for a folder that is not `ok` is one line plus a "N findings" link to that tab, rather than the first six findings inline
- **The Folder tab renders infos.** They were produced by the validator and displayed nowhere — for *every* kind of agent. They sit under the errors and warnings on the Validation card, quieter, and stay **out** of the tab's count badge, which remains errors + warnings: a badge on every healthy folder is the banner-in-the-healthy-state failure rule 2 forbids, and an info is by definition not attention.
- **A surface that names a file is asserting that file exists** ([UX Rules](../../development/ui_guidelines/ux_rules.md), rules 9 and 10 — both written from this page's review). Ten surfaces here asserted a file only a kit folder has, and the fix in each case was to say something true about the other kind rather than to soften the sentence; the per-surface list is in [Bare Agents & External Roots](bare_agents.md#every-surface-says-something-true-of-this-folder). Rule 10 is the sibling: the sidebar `+` and the delete dialog both carried accessible names written when only one kind of agent existed, and an `aria-label` beside a conditional heading is a control that says one thing to one user and another to the next. [Rule 11](../../development/ui_guidelines/ux_rules.md) came later from the same page — a control must not be the colour and size of the prose beside it — and its origin is on this list too: a "Show more" toggle on the bare `Readme` card, muted grey directly above the card's muted footer note.
- **A bare agent's page shows only what its folder can answer for.** The Commands tab is removed rather than left empty — a permanently empty tab saying "no commands" is about `docs/CLI_COMMANDS.yaml`, a file the folder was never asked to have. Three of Overview's four cards name a kit-only file (`app-data/storage/STATUS.md`, and the manifest twice) and are replaced by the one thing a bare folder has that the user can change: its **Name**. The Runs-with panel keeps its **credential and model controls** — an agent adopted from the user's own repository is not a lesser agent, and "which credential pays for this" is the question they most need an answer to — but the answer is kept in Cinna rather than in the folder, and one static note says so; what is dropped is the panel's third control, the declared-secrets line, which could only report "none declared" about a file the folder never had. And the header's "No description yet — add one under Overview" invitation is withheld, because there is no field there to accept it (rule 7). What is *kept* matters as much: the header, the type icon, Open in, the chat/settings switch, Permissions and Folder are the same page, so the shape does not change with the kind of agent — only what it can honestly claim <!-- nocheck -->
- **The Name card is the one card on this page that names no file.** Every other card is a viewer over a file and says which; this value is held on this machine, beside the rest of that agent's local state, because the folder may be a repository the user shares with other people. `AgentCard`'s `file` prop is therefore optional, and the card says out loud where the name is kept and what clearing it falls back to. It saves on blur and Enter rather than on a pause — there is no file, no stamp and no conflict to resolve, so the inline-file-editor pattern would be borrowing machinery that guards nothing
- **The Folder tab is where the rest of that branching lands.** For a bare agent the Files card lists the folder's own two files instead of the kit layout's seven; Identity drops the `Kit` row (a folder with no manifest was reporting "legacy manifest" — an *old* one, which is false and also the wrong story, since legacy is a kit folder Stamp identity can repair) and gains the warning that the agent is identified by where its folder sits; Runs names no file and says the run state is kept outside the folder; and Credentials and Published do not render at all, each having nothing to say but "none" about a file the folder never had.
- **The bare `Readme` card sits on Overview, and renders read-only.** Overview asks what this agent is, and for an adopted folder the README is usually the only prose that answers. On Prompts it was the longer of two cards on the tab whose point is the shorter one, and a document beside `Instructions` reads as a second thing the agent is told — which it is not: `AGENT.md` is the whole system prompt. Read-only because the README is the *builder's* document — what an assistant opening the folder is briefed from — so editing it from the agent's own page is the agent editing its own briefing, the one thing the assembled prompt tells it not to do; it is one click away in the user's editor, and the card's footer says which document is which. The card renders the **whole** file, no clamp and no expand control: it is a viewer over a document, the page already scrolls, and a fade over half a paragraph asks the reader to press something before they can read what they came for. **Where the folder has no `README.md` there is no card at all** — a card whose only possible content is "there is no README here" states a finding, and the Folder tab already carries that one (`bare.readme.missing`) with the consequence attached. A README that exists and is empty is treated the same way, being just as little use to the reader. `InlineFileEditor` lost its `readOnly` prop with this move; the only read-only state left there is `!canSave`, which means "the file is not there" and says so
- **The remembered tab falls back for the render only.** The selected tab persists across agents, so someone on Commands who clicks a bare agent would land on a tab that is not there. `setTab` is untouched, so going back to a kit agent returns to Commands
- **The selected tab is kept across agents.** Someone working through the prompts of three agents does not want to click "Prompts" three times. The Commands tab carries a count badge when the catalog is non-empty; the Folder tab's badge is the findings count above; the Permissions tab's badge is how many standing grants the agent holds, which is what makes one discoverable without opening the tab
- **Permissions is a tab, not a card on Overview.** In the common case it is a fixed paragraph identical for every folder agent plus "nothing remembered yet" — knowledge, not a control. The one control it holds is for an agent on the [Claude engine](claude_engine.md), whose profile paragraph gives way to an **Approvals** setting, because on that engine the profile describes rules that are not in force. The paragraph is there because the default profile *changed*: an agent now reads, writes and runs commands inside its own folder outright, and a user who notices that it stopped asking deserves to find the reason on the agent's own page rather than infer it. It also says plainly that a command is not fenced the way the file tools are, because the paragraph above it would otherwise read as a boundary the shell does not have, and it names any permission the folder's own manifest has replaced. Revoking a grant is the only action, and it does not confirm — nothing is lost that the agent cannot ask for again. The full model is [Local Agent Permissions](permissions.md)
...
- **The header has one error slot, owned by the page.** Open in and the ⋯ menu report through an `onError` prop into a single `role="alert"` line under the header row; neither renders its own text, with one deliberate exception — the Open-in menu's "Copy prompt for another tool" does not close the menu, and an open `below-right` popover covers this slot, so *that* failure is drawn inside the menu and reaches the page slot only when it lands after the menu has closed. It is cleared at the start of every header action and whenever the selected agent changes. Two absolutely positioned messages, one per menu, used to overlap when a second action failed while the first's message was still showing. The slot is **always rendered, at a fixed minimum height** — a conditional one moved the Runs-with panel down by the line's height the moment a refusal appeared, which is the same jump rule 1 forbids in the dialogs
- **Nothing moves while the user reads or types** ([UX Rules](../../development/ui_guidelines/ux_rules.md), rule 1). The New-agent dialog's error line — on both steps — and the Delete dialog's are always-rendered, fixed-height slots, so a refusal appears without pushing the buttons down; the Delete dialog's confirm button has a fixed minimum width, so "Deleting…" does not pull Cancel sideways. A runtime save in flight is a spinner in the Runs-with panel's corner, not a row under the selects, and that panel's status line is a reserved fixed-height slot for the same reason, so neither saving nor switching credential moves the tabs. The page's scroll container reserves its scrollbar gutter, so switching to a tab that overflows does not shift the header by a scrollbar's width. And a legacy folder whose only validation errors are the missing `id` gets **one** banner — the legacy notice, with Stamp identity and the "N findings" link — rather than that plus a readiness line saying "`id` is required."; the readiness line returns as soon as a second, unrelated error exists

### A chat from the row, without opening the page

Beside every eligible sidebar row sits a small accent-coloured chat shortcut. It opens the main new-chat composer with the agent attached and the sidebar on Chats. The page's **Start chat** has a different navigation purpose: it returns from Settings to the embedded composer while preserving its draft.

- **Navigation follows the action.** The row shortcut moves to the main composer immediately. A page stays on Agents while the user prepares the conversation, then moves to Chats when its first send succeeds
- **The button is a sibling of the row, never a child of it.** The row stays a real `<button>`, and the two sit side by side under one hover group. Nesting the chat button inside the row would have forced the row to become a `div` with `role="button"` — a button may not contain a button — and a labelled button inside a `role="button"` joins the row's accessible name while it is rendered: the row read as the agent's name at rest and as the name plus *Start a new chat with <name>* under the pointer, which is exactly where every E2E locator and every screen reader finds it. As a sibling, the button lends the row nothing in any state. The open-page click sits on the wrapper so the whole highlighted row, trailing slot included, opens the page; the chat button stops its own click from reaching it
- **It is hidden by opacity, not unmounted, so the keyboard reaches it.** Invisible until the pointer is over the row or the button itself has keyboard focus — the chat list's delete-button pattern. Because it stays in the tree, Tab reaches it from the row and it shows itself when it does; a button mounted only on hover would have had no keyboard route at all. Rule 11 says hover is not an affordance, and the row makes the trade the jobs row makes: the always-visible route is opening the agent row and using its composer, and the row's is for someone who has learned it is there. One consequence for tests: the button's own name contains the agent's, and it is in the tree at rest, so a substring query for the agent's name matches two buttons on every runnable row — the specs match the row's name exactly
- **The row reserves the button's 16×16 slot whether or not the button is in it**, so the name truncates at the same point before and after the pointer arrives, and at the same point on a row that has no button at all ([UX Rules](../../development/ui_guidelines/ux_rules.md), rule 1). A row that re-flows under the pointer that just reached it reads as the interface jumping
- **It is withheld from a row the chat could not attach to.** A folder whose readiness is `invalid` — a manifest the validator refuses, a folder that lost an id collision — shows no chat button. A duplicate-id folder is listed but never indexed, so the new-chat screen would look the agent up, find nothing and show nothing: the click would have failed silently, with the view already switched and the sidebar already on Chats. An invalid manifest is never handed to the engine either. The jobs row withholds run-now from a job this device cannot run for the same reason, and the page explains the folder's readiness. Every other state keeps the button — a switched-off credential, unfilled `credentials/.env` values — as the page's Start chat is not gated on them either: readiness is evaluated by the composer and again by main when a turn is submitted

### A file rendered in a card is not a chat message

A chat message owns its region; a `README.md` or an `AGENT.md` is drawn inside a card, on a page with a heading and other cards. `documentMarkdownComponents` is the map for the second case, and every difference from the chat map is a defect it closes:

- **Headings are demoted.** A README opening `# Alpha` — the ordinary shape — emitted a second `<h1>` on a page whose title is already one, so the outline claimed two top-level headings and a screen reader announced the folder's README title as the page. The page is `h1` and a card title is `h2`, so a document starts at `h3` and floors at `h6`
- **Images render as their alt text.** The renderer's CSP is `img-src 'self' data:` and the card is not served from the agent's folder, so a repository README's logo and its shields.io badges — the two things such a file opens with — could only ever be broken-image glyphs
- **A link that goes nowhere is not a link.** `setWindowOpenHandler` opens `http(s)` externally and drops everything else silently, so a README's relative link to a file beside it — its licence, its own setup notes — looked clickable and did nothing. Non-`http(s)` links render as plain text, the target kept in `title`
- **Raw HTML is stripped before rendering** (`remarkStripHtml`). `react-markdown` neither runs nor hides it: with no `rehype-raw` it *escapes* it, so an `AGENT.md` beginning `<!-- generated; edit AGENT_SRC.md -->` showed that comment as its opening sentence, and a README's centred `<p align="center">` badge block arrived as a paragraph of visible markup. Every viewer these files are otherwise read in — the user's editor, the forge hosting the folder — hides both, so this matches them rather than inventing a third behaviour. A fenced block is a single `code` node, so nothing inside one is touched

**Which cards render and which show raw text is per card, and the choice is about the reader.** A bare agent's `Instructions` renders: `AGENT.md` is prose its author wrote and reads as markdown everywhere else they open it. The kit's three prompt documents do not: they are written for a model, and the scaffold template leads with an HTML comment telling their author what to put where — the same comment `remarkStripHtml` drops — so rendering them would hide the one line the author most needs from a card that claims to be a viewer over the file. The manifest-backed editors — description, example prompts, router trigger — pass `markdown={false}` too: each is a short field rather than a document, and the whole of what they hold is already on screen. Either way the click that starts editing puts the file's **own bytes** in the textarea: rendering changes how a file reads, never what is saved.
...
**A card that says a file is missing says it in that folder's own terms.** `PromptDocCard`'s default note tells the user to run the agent's scaffold again, which is right for a kit prompt and nonsense for an adopted folder — nothing scaffolded it, so that is an instruction its owner cannot follow (rule 7). A missing `AGENT.md` is reported as the file that makes the folder an agent, to be added there.
...
- **A bare agent is the one exception, and it asks.** `DeleteLocalAgentInput.trashFolder` is what the dialog's two radios set; everything below describes the `true` branch, which is the only branch a kit agent has. `false` writes a `hidden` flag into that agent's state and leaves the folder alone — still under the turn lock, still followed by a rescan, and still leaving through the ordinary prune, since the scan simply stops producing that id. Trashing a bare folder additionally deletes its state file, which lives under `<userData>` and would otherwise be inherited by whatever is next created at that path
...
- **The channel returns a `LocalAgentOutcome`**, the third one to do so, because `turn_in_progress` is the one refusal the dialog has to *explain* rather than report. Deleting an agent pushes nothing at an engine — since phase 3 there is no shared config listing every folder agent — but it does **retire that agent's process** (`acpProcessPool.retire`), because a child left running with its cwd in a folder that has just gone to the trash is a process nothing will ask for again. *Retire*, not a kill: a turn still streaming keeps its process until it finishes, which is the courtesy every other change gets.
...
### Chat controls shipped disabled before a turn could run

Both are live now — the embedded chat composer (returned to by **Start chat** in Settings) and **Run** on each command row, the latter sending `/run:<name>` ([Catalog Commands](commands.md)). Until the runner landed in Phase 6 they were present, disabled and titled with why: hiding them would have made the finished page's shape invisible, and enabling them would have produced an error where the user expected a reply.
...
- **The list load error still renders `err.message` directly** in `LocalAgentsList.tsx`. OpenCode Path saves use `unwrapIpcError` in `OpenCodeSettingsFields.tsx` and retain the attempted draft after failure. Other unrelated renderer error sites are outside this settings change; do not infer an app-wide count from this example.
- **App-wide the same defect had 33 sites at `b566d83`, or 36 on a stricter reading** — the ambiguity is four handlers whose `requireActivated()` sits *outside* their internal try, so they reject with a prefixed message only for an unactivated session. Deliberately left as a range rather than resolved into one confident number, which is how the count above got to six. Six have since been repaired (the five jobs sites and `useAuth.ts:102`, the startup-failure screen), leaving **27, or 30 on the stricter reading** at `12686f0`; the page redesign then wrapped two of that set (the Open-in row's and the create form's), and the settings redesign four more (reveal, forget root, restore hidden, add root), so **22, or 25** — and **21, or 24** if that walk did not share the local count's blind spot over restore-hidden. Both figures assume nothing else has moved: the app-wide set has not been re-walked for either change. `unwrapIpcError` in `src/renderer/src/utils/ipcError.ts` is the fix, now used from twelve renderer files, and has its own unit test. **Note what is being counted: sites that put the string in front of a user, which is not the same set as sites that touch `.message`.** `JobEditForm.tsx:152` catches and returns `{ok: false, error}` and the rendering happens one level up in `JobEditPage.tsx:49`; counting by grep alone both over-counts (log-only and outcome-built sites) and under-counts (this indirection), which is the mechanism by which any figure here drifts
...
Settings ── Agents ──► LocalAgentsSettingsSection (roots, readiness, tools, default tool, auto-open)
  ...
  useLocalAgents / useAgentFileEditor / useLocalTools / useDefaultTool   (the only window.api callers)
       ...
       └─ Zustand       activeLocalAgentId, agentPageMode, pendingDraftAgentId (UI only)
       │
       ▼
  local-agent:list | :get* | :create | :draft | :update-field* | :delete* | :read-doc
                   | :rescan | :validate | :open-path | :open-credentials
...
- [Local Agent Permissions](permissions.md) — the Permissions tab: the profile it describes, the grants it lists, and the three channels behind its revoke buttons
...
- [Chat Modes](../../chat/chat_modes/chat_modes.md) — the default mode supplies the Runs with panel's fallback credential, the starting point of its fallback *model* chain, and the Settings readiness line's credential check
...
- [Jobs](../../jobs/jobs/jobs.md) again, for the sidebar row — its chat button is the jobs row's run-now pill with a different verb: the same reserved trailing slot beside the title, shown on hover, and withheld for the same reason from a row the action could not honour
- [Agent Status](../agent_status/agent_status.md) — the `pendingAgentId` → `ChatWorkspace` hand-off used by sidebar shortcuts and the status overlay; the page uses an embedded composer
...
- [Settings Scope](../../core/settings_scope/settings_scope.md) — folder-agent configuration is machine-local and lives under **Default → Agents**; **Profile → Agents** is the server catalog
