...
- **The Runs with panel says one thing at a time, in a line that is always there.** It has a lot it *could* say — a refused save, a model it just dropped, a credential the manifest names and this machine lacks, no credential at all, a credential with no usable key, a model registry still loading or failed, a model that belongs to another catalogue, no model at all, no models listed yet, and the engine's own reason for skipping the agent. Stacked as separate conditional rows they changed the panel's height on every credential change, which moved the tab strip out from under the pointer that had just used the select. So there is one reserved fixed-height line, and the messages are **prioritised rather than concatenated**: what blocks the agent first and what the user can act on. The engine's skip reason sits below everything the panel worked out for itself, so the one skip that would restate a message already on screen — "its runtime names no model" — never takes the line. What the pickers offer and what a credential change does to the model belong to [The Local Engine](engine.md#choosing-a-runtime). The line truncates with the full sentence in `title`, because at the 800px minimum window a long credential name would otherwise wrap into a second row and reintroduce the shift; the surviving half of each sentence is the half naming the action
