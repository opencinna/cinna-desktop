...
The user-facing surface of Local Agents: a top-level **Agents** tab whose rows are folders on disk, a per-agent **page** that is a control surface first — open the folder in your own tool, start a chat, choose what it runs with — and a **viewer over that folder** underneath (almost every card names the file it reads, and several edit it in place), an **Add an agent** flow with two ways in (scaffold a new kit folder, or adopt one that already holds an `AGENT.md`), and a **Settings → Local Agents** section for the roots, the kit contract, the default tool and this machine's readiness.
...
- **Agent page** — The per-agent screen. A header (readiness dot, name, description, path) with three controls — **Open in <tool>**, **Start chat**, and a **⋯ menu** — then a **Runs with** panel, a readiness strip that appears only when something needs attention, and five tabs: **Overview** (Status, Description, Example prompts), **Prompts** (the three prompt documents), **Commands**, **Permissions** (what this agent may do without asking, and what the user has told it it may stop asking about), **Folder** (validation findings in full, identity, credentials, files, publications, runs). Every card still names the agent-relative file it reads — with exactly one exception, below
- **Bare agent page** — The same page for a folder adopted for its `AGENT.md` ([Bare Agents & External Roots](bare_agents.md)): four tabs (no **Commands**), `Instructions` alone on Prompts, and on Overview a **Name** card — the one card on this page that names no file, because the name is not in the folder — over the folder's own `Readme`, read-only and absent where the folder has no `README.md`. The Runs-with panel is the same panel, controls and all; only where it puts the answer differs
...
### Creating an agent
1. **+** — "Add an agent" — opens a choice of two cards: **New agent** (scaffold a folder) and **Add a folder** (adopt one that already is an agent). The fork is at the front because the two have nothing in common: one *writes* a kit folder into the agents home and hands it to an assistant to build, the other *reads* a folder the user already owns and changes nothing in it. An "or point at an existing folder" link under a name field would have made the second look like an option on the first
...
5. **Create** scaffolds the folder and returns immediately. The user lands on the agent page. With no description, main writes the **name** as the manifest description — the kit schema requires a non-empty one — and the page shows "No description yet" rather than echoing the name twice
...
3. With no usable default — never picked, or the tool has since been uninstalled — the button is the menu itself, labelled "Open in…". A machine with no assistant or editor at all still gets Terminal, Reveal and the copy, and a sentence pointing at Settings → Local Agents → Refresh
...
1. **Agents folders** — each registered root with its path, agent count and resolved kit contract version; Reveal on each; Forget on the added ones (the home cannot be removed); a rescan-everything button. An adopted folder carries an **Added folder** badge and reads `N agents · not a kit folder` instead of a contract version — there is no `.cinna-kit/` in it and none is coming, so naming a version would name one that governs nothing there. Not "read only": the badge's tooltip says Cinna installs nothing there and that the only file it writes is an agent's `AGENT.md`, and only when the user edits it on the agent page. Promising read-only would leave them with a modified working tree in a shared repository and a blocked **Update** two rows below. Under a root holding agents that are not in the list — removed, or never ticked when the folder was adopted — the count and an **Add them** button; a capped root also reads "(the first found)". Under a root that is a git working tree: the update check ([Agents Folder Updates](folder_updates.md))
2. **Add an agents folder** — opens the OS directory picker in main; a non-empty folder gets a native confirmation before templates are installed. This is still the *workshop* adoption; a folder that already holds agents is adopted from the Agents tab's **+** instead, and gains nothing
...
- **Above the fold is what the user *does*.** Open in, Start chat and the ⋯ menu in the header; the **Runs with** panel (credential, how hard the work is, engine status, declared secrets) directly below. Everything that is information rather than a control lives under the tabs
