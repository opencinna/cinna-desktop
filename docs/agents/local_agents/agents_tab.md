...
- **Bare agent page** — The same page for a folder adopted for its `AGENT.md` ([Bare Agents & External Roots](bare_agents.md)): four tabs (no **Commands**), a read-only Runs-with panel, `Instructions` over a read-only `Readme` on Prompts, and a **Name** card as the whole of Overview — the one card on this page that names no file, because the name is not in the folder
...
3. Several means a repository of them: a scrolling checkbox list with **Select all** / **Clear all**, already-added rows ticked and disabled, and names taken from each folder's own `AGENT.md`
