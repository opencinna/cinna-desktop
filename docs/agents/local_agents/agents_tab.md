...
- **Above the fold is what the user *does*.** Open in, Start chat and the ⋯ menu in the header; the **Runs with** panel (credential, how hard the work is, engine status, declared secrets) directly below. Everything that is information rather than a control lives under the four tabs
