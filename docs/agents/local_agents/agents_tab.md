...
- **Bare agent page** — The same page for a folder adopted for its `AGENT.md` ([Bare Agents & External Roots](bare_agents.md)): four tabs (no **Commands**), `Instructions` over a read-only `Readme` on Prompts, and a **Name** card as the whole of Overview — the one card on this page that names no file, because the name is not in the folder. The Runs-with panel is the same panel, controls and all; only where it puts the answer differs
...
- **The bare `Readme` card renders read-only.** It is the *builder's* document — what an assistant opening the folder is briefed from — so editing it from the agent's own page is the agent editing its own briefing, which is the one thing the assembled prompt tells it not to do. It is one click away in the user's editor. `InlineFileEditor`'s `readOnly` is distinct from `!canSave` ("the file is not there", which says so): this one shows the contents and refuses the click
