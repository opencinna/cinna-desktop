...
**A bare agent adds three more exceptions, and they are worth naming here rather than only in their own doc.** A folder adopted for its instructions file — `AGENT.md`, `AGENTS.md` or `CLAUDE.md` ([Bare Agents & External Roots](bare_agents.md)) — states no name, no membership and no runtime anywhere the desktop may write, so three more values are held outside the folder: `displayName` (what the user called it), `hidden` (removed from the list without the folder being deleted) and `runtime` (which credential and model the user picked for it). None is a SQLite column — all three live in that agent's state file under `<userData>` — but each is exactly the same *kind* of thing as `enabled`: a user's choice about a folder, which the folder cannot be asked to record. A rescan reproduces everything else about a bare agent and would silently undo those three if they were not kept.
...
**The launcher recorded on the row is not a fifth.** `driver_config.launcher` names the engine the folder's runtime declares, and a rescan reads that back — from the manifest for a kit agent, and from the bare agent's own state (already where `runtime` lives) for a bare one. It is written by the index like `name` and `description`; see [The row names the engine its folder's runtime names](#the-row-names-the-engine-its-folders-runtime-names). (`agents.driver` itself is `acp` for every folder agent and says nothing about the folder at all.)
...
2. It is **walked** for instructions files (`AGENT.md`, `AGENTS.md`, `CLAUDE.md`) rather than converted, and what was found is previewed before anything is registered
...
- **Nothing is installed into an external root.** No templates, no `.cinna-kit/`, no `Local/`, no `app-data/`; its agents' state lives under `<userData>` instead. The one file that is ever written there is an agent's own instructions file, and only when the user edits it on the agent page
...
For a folder agent, `agents.driver` is always `acp` — every local CLI agent runs on one [driver](../drivers/drivers.md) — and `driver_config.launcher` names the **engine** its runtime names. The launcher is index data like `name`: written by the scan, never the source of truth.

- **Every scan writes it**, both when a whole root is rebuilt and when a single folder is updated. Implemented folder launchers are `opencode`, `claude` and `codex`. For a kit agent the engine comes from the manifest's runtime; for a bare agent, from its desktop state. A folder without a recognized engine keeps OpenCode when it declares a credential/model, otherwise follows the saved machine default; a recognized unsupported launcher remains explicit. See [runtime precedence](engine.md#the-machine-default-is-selected-once-and-explicit-agents-keep-their-choice)
...
- **No turn trusts the column.** The driver re-reads the folder's engine when a turn starts, because the row can still lag an edit an assistant made a moment ago. This used to matter more than it does: with a driver per engine, an agent dispatched on a stale value had to be handed to its sibling, and while that hand-off was missing it answered "try again in a moment" for ever
...
- [Agent Drivers & Readiness](../drivers/drivers.md) — the `driver` and `driver_config` columns the scan writes, the driver that re-reads the folder's engine on every turn, and folder readiness as the first rung of an agent's readiness
