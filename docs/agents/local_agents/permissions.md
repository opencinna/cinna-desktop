...
- **Standing Grant** — one remembered decision: this agent may take this action on this resource without asking again. `{action, pattern, scope, decidedAt}`, stored in that agent's desktop state — `app-data/desktop.json` for a kit folder, a file under `<userData>/external-agents/` for a [bare](bare_agents.md) one, since adoption does not write desktop state into that folder
...
- **Handovers** — a per-agent setting on the same tab, for a [bare](bare_agents.md) folder on any engine: whether a `brief.md` left in that folder's `.cinna/handovers/` runs without asking. It is not a permission the engine enforces and not part of the profile — it decides whether the desktop starts a turn at all — but it belongs here because it is the broadest standing permission a folder can be given: running a brief unattended runs whatever a file in that folder says, under that folder's own settings, hooks and MCP servers. It defaults to asking, lives in the agent's desktop state under `<userData>` rather than in the folder, and *Run automatically* is refused while git tracks or fails to ignore that directory. See [File Handovers](../../jobs/tasks/file_handovers.md)
- **Delegations** — a kit agent's permission to receive local delegated work without an Inbox gate. **Cloud delegations** — a bare or kit requesting agent's permission to send work to cloud assignees without that gate. Both default to asking, require an explicit automatic request to take effect, and live under `userData` rather than in a kit or adopted folder. See [Handover Bus](../../jobs/tasks/handover_bus.md).
- **Permissions tab** — a tab under the agent page's **Settings**: what the profile allows, what the manifest has overridden, the list of standing grants with a per-row revoke, and — for a bare folder — the **Handovers** setting above. For a Claude or Codex agent the OpenCode profile paragraph gives way to its own Approvals setting and its control, since the profile describes rules that are not in force on that engine. Its examples name files the folder actually has — for a bare agent, "editing its own `AGENT.md`" rather than the manifest and `credentials/.env`, because two fictional examples out of three is how a reader comes to discount the third, and the third is the sentence about a command reaching anything they can
...
`cinna-agent.json`, `docs/WORKFLOW_PROMPT.md`, `AGENT.md`, `AGENTS.md` and `CLAUDE.md` are what the agent *is* — the system prompt the conversation is running on, and the manifest binding it to a credential. The assembled prompt reserves rewriting them for [building mode](engine.md#the-desktop-context-block-and-building-mode), which only the person's own request enters — and **an instruction is not a control.** This entry is what still stands between an agent that talks itself into building and its own prompt. Rewriting them is the one edit inside the folder worth a dialog, and now it is worth exactly one, because *Always allow* remembers it. <!-- nocheck -->

`AGENT.md`, `AGENTS.md` and `CLAUDE.md` are the [bare agent](bare_agents.md) entries — a bare agent's instructions file is whichever of the three its folder has first — and the list covers both folder shapes and all three names at once rather than being built per agent. So every kit agent asks too: the kit scaffolds `AGENTS.md` and `CLAUDE.md` into each agent as its builder guides, and asking before those are rewritten is right for the same reason. One dialog on a file that is not this agent's identity costs far less than the profile's own stated rule quietly not holding for the one kind of agent whose whole identity is a single file.

**The entry is OpenCode's alone.** This profile is generated for OpenCode. On [Claude](claude_engine.md) and [Codex](codex_engine.md) nothing singles these files out — their approval settings govern every edit alike — so an agent that switches into building mode unasked is held back by the prompt text and nothing else. That is a known gap, accepted rather than overlooked.

**`README.md` is deliberately not on the list.** For a bare agent the README is the builder's guide — what an assistant opening the folder is briefed from, and what building mode names for the agent itself to read, as background — so rewriting it is as much a build step as rewriting the instructions file. Only the instructions files ask, and that is a considered position:
...
- The blast radii differ in kind. The instructions file changes what the agent *is* on the next turn, silently and durably, with no human in the path. `README.md` changes text a **person** then reads and pastes, and in the git working tree this shape targets it is visible in `git status` and revertible
...
### Delegation grants belong to this machine

The receiving kit agent owns **Delegations**; the agent sending the brief owns **Cloud delegations**, because it chooses the content leaving the machine and the work that may cost money. Delegations reads Ask before running / Run automatically, like Handovers; Cloud delegations reads Ask before sending / Send without asking. They are separate from engine approvals: admitting a turn does not authorize every operation inside it.

Both grants are stored in a desktop-state `.delegations` sidecar under `userData`, for kit and bare agents alike. Only literal `auto` grants permission. A manifest, pulled folder or synced task cannot grant them; removing the agent's local state removes the sidecar. Bare incoming work retains **Handovers** and its git checks.
...
- **On Claude and Codex it does not guard the agent's own identity files.** Only OpenCode's profile asks before `cinna-agent.json`, `docs/WORKFLOW_PROMPT.md`, `AGENT.md`, `AGENTS.md` or `CLAUDE.md` is rewritten; elsewhere the prompt's building-mode rule is the only barrier. See "The agent's own identity files ask" <!-- nocheck -->
...
- [Handover Bus](../../jobs/tasks/handover_bus.md) — per-target admission, requester-owned cloud consent and asynchronous results
- [File Handovers](../../jobs/tasks/file_handovers.md) — the **Handovers** setting on this tab: what it permits, why it is the folder's own setup that then runs, and what git has to say before it may be given
