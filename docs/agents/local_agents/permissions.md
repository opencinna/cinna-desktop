...
- **On Claude and Codex it does not guard the agent's own identity files.** Only OpenCode's profile asks before `cinna-agent.json`, `docs/WORKFLOW_PROMPT.md` or `AGENT.md` is rewritten; elsewhere the prompt's building-mode rule is the only barrier. See "The agent's own identity files ask" <!-- nocheck -->
