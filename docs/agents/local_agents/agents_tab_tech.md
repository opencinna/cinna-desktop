...
- `src/renderer/src/hooks/useLocalAgents.ts` (grants) — `localAgentGrantsKey`, `useLocalAgentGrants` and `useForgetAgentGrants`. **Deliberately a separate query from `useLocalAgent`:** grants live in `app-data/desktop.json`, which the scanner does not fold into the agent DTO, and a DTO that moved with that file would re-render the whole page mid-answer. Both revoke handlers answer with the list they leave, written straight into the cache with `setQueryData`
...
- `src/renderer/src/components/agents/local/OpenInMenu.tsx` — the split button: primary launches the default tool, chevron opens the menu (assistants, editors, Terminal, Reveal, Copy prompt for another tool); a menu pick rewrites the default. Exports `MENU_ITEM` / `MENU_SURFACE`, which `AgentActionsMenu` shares
- `src/renderer/src/components/agents/local/OpenInMenu.test.tsx` — one-click default without rewriting it, a menu pick becomes the default with the editor action for an editor, the "Open in…" fallback, Terminal/Reveal never become the default; and the four copy cases — confirms in place without closing, a failure shown inside the still-open menu, a failure arriving after the close routed to the page slot, a confirmation arriving after the close dropped rather than re-armed
...
- `src/renderer/src/components/agents/local/PermissionsCard.tsx` / `PermissionsCard.test.tsx` — the Permissions tab body. See [Local Agent Permissions — Technical Details](permissions_tech.md)
...
| `local-agent:grant-forget` | `({agentId, key}) → StoredPermissionGrant[]` | Revoke one. Answers with the list it leaves, so the card does not refetch to stop showing a removed row |
| `local-agent:grants-clear` | `(agentId) → StoredPermissionGrant[]` | Revoke all. See [Local Agent Permissions — Technical Details](permissions_tech.md) |
| `local-agent:init-prompt` | `(agentId) → string` | The Open-in menu's "Copy prompt for another tool". Also an id and never a path, though this one only *reads*: main resolves the folder from the index row and names the first entry document the folder actually has. Throws `not_found` rather than returning a `LocalAgentOutcome`, since the renderer prints the message and branches on nothing. Owned by [Open in Tools](open_in_tools.md) |
...
| `LocalAgentPage` | Loading / not-found / not-indexed states; header (readiness dot, name, description or nudge, path-as-reveal-button, `OpenInMenu`, Start chat, `AgentActionsMenu`) and the one `actionError` slot (`role="alert"`, always rendered as exactly one `h-4` line, truncated with the full text in `title` — a two-line wrap at the minimum window width moved the panel below it) both menus report into, reset on selection change; the scroll container has `[scrollbar-gutter:stable]`; `ReadinessStrip`; `RuntimePanel`; the five-tab strip (`useState<AgentPageTab>`, kept across agents; Commands carries the catalog count, Permissions the standing-grant count, Folder the validation-findings count in the warning token — the page runs `useLocalAgentGrants` itself for the badge, and react-query serves the card's identical query from the same cache entry, so the badge costs no extra IPC) and the cards each tab mounts; the draft-on-arrival effect |
| `OpenInMenu` | Split button when a default tool resolves (primary launches it; chevron opens the menu), a plain "Open in…" menu button otherwise. Menu: `launchable` tools with a check on the default, Terminal, Reveal folder, **Copy prompt for another tool**, then a ruled-off prose tail holding the copy's failure and the no-tools sentence, in that order. A tool pick calls `useSetDefaultTool` when it differs from the current default, then `useOpenIn` with `actionForTool`. Takes `onError(message \| null)`; calls it with `null` at the start of every action and with the unwrapped message on a failed **launch**. The copy is the one exception to "renders no error text of its own": it does not close the menu, so the page's slot is covered by the popover and its failure is drawn in the tail instead — the page slot is used only for a failure that lands after the menu was closed, when it is no longer covered. Its confirmation is the item's own label (`ClipboardCheck` + "Copied", `COPIED_REVERT_MS`), because nothing else on screen changes when a clipboard is written |
...
| `PermissionsCard` | The Permissions tab. A fixed paragraph describing the generated profile, a second naming the shell's limits, a third naming any permission the manifest's `runtime.permissions` replaces (read from the DTO the page already holds, so it costs no query and cannot arrive late and move the list), then the standing grants newest first with a per-row revoke and a **Forget all**. `undefined` is not `[]` — "Nothing yet" is withheld while the query is in flight, because it is a positive claim about a security decision. The mutation is owned by the card, not the row, since a row unmounts the moment its grant is forgotten; the failure is one line **below** the list, so it never moves the row whose button raised it. Names `app-data/desktop.json`, like every other card names the file it renders |
...
- **Paths never arrive from the renderer as trusted input.** `local-agent:root-add` opens a native directory dialog in main; `local-agent:open-path` takes an *agent-relative* path re-resolved inside the folder; `local-agent:open-credentials` and the three `grants-*` channels take **no path at all**, because they write — `credentials/.env` is fixed in main and re-checked for containment after the file exists; `local-tools:open-in` re-validates against the registered roots. See [Open in Tools](open_in_tools.md)
