...
- **A real spawn against a genuinely logged-out install is untested.** The not-logged-in sentence is driven by matching the CLI's error text, and the ladder above it has never run end to end on a machine in that state
...
- **The Anthropic API SDK moved 0.89 → 0.93** to satisfy the Agent SDK's peer requirement. That bump touches the ordinary Anthropic chat adapter, `src/main/llm/anthropic.ts`, which **has no unit tests**. It typechecks and is otherwise unverified
