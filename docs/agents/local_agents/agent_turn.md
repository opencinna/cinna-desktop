...
Five rules come from watching the wire rather than from the protocol document:
...
- **A subagent's frames are a lane of their own.** The Claude adapter streams a subagent's child session on the same connection, every frame carrying `_meta.claudeCode.parentToolUseId` — the `Agent` call that launched it — while the agent keeps talking. Those frames' parts carry `cinna.parent_tool_id`, are filed under the message that owns the `Agent` call (or, for a call from an earlier turn, the message current when the lane first speaks, fixed from then on), and never move the agent's current message or end its run of text. Each lane counts its own tool calls, so the subagent's text → tool → text still splits as the rule above says. A permission ask or question raised by a subagent's tool call, and its decision, join that lane. Before this, the subagent's first tool call cut the agent's sentence mid-word. How the accumulator merges lanes and keeps them out of the answer is in [the streaming pipeline](../agents/streaming_pipeline.md#lanes--a-subagents-work-inside-the-agents-turn)
