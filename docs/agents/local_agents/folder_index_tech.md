...
| `kind` | `'workshop'` (default) or `'external'`. Added by a `hasColumn`-guarded `ALTER TABLE`; the default is what makes it backfill-free, since every root that predates the column is a workshop |
| `created_at` | ms |
...
- `addExternalRoot(userId, rawPath, label?)` — the same path rules and the same overlap refusal, with the three writes removed: no templates, no `.cinna-kit/`, no `Local/`. An already-registered path is returned as-is, so adopting the same folder twice is a safe no-op
- `countBareAgents(rootPath)` (module-private) — an external root's `agentCount` and `hiddenAgentCount`, from a name-less walk: this runs for every root on every `local-agent:list`
...
- `scanRoot(userId, root)` — dispatches to `scanExternalRoot` for an external root, else the full workshop scan and index rebuild. `ScanRootResult` also carries `hiddenCount` and `truncated`, which is what the settings row reads through `cachedScan(rootId, rootPath)` instead of walking again. Returns `{agents, rootMissing, indexed, pruned}`. The row's `description` is `describedAs(dto) || null` (`src/shared/localAgents.ts`) — `null` when the manifest's description is only the name repeated, so the `@` / `[+]` / Jobs pickers, which read the column and never the manifest, do not show the name twice for a name-only agent. `reindexAgent` applies the same
...
- `classifyExternalEvent(rootPath, filename)` — the same three-outcome type for an external root, but only `ignore` and `root` are ever returned; a path within reach is **stat'd** to tell a directory from a file, and an unstat-able one rescans
- `externalFallbackDirs(rootPath, agentDirs)` — the Linux-fallback directory set for an external root: each agent folder and each holder, and **nothing at or above the root's parent**
...
- `scanFolder(root, agentDir)` — the one single-folder read, dispatching on `root.kind`. A bare folder handed to `scanAgentFolder` returns a confident wrong answer (`unreadableAgent`), not a crash
- `kindOf(root)` — the root row → the `LocalAgentKind` that `desktopStateService` and `permissionGrantService` take
- `rescan(userId, rootId?)`, `create(userId, input)` (a missing `description` becomes the name), `updateField(userId, input)`, `delete(userId, input)` (trash *or* hide, under the lock, then rescan — see [Agents Tab — Technical Details](agents_tab_tech.md)), `validate(userId, agentId)`, `openPath(userId, input)`, `openCredentials(userId, agentId)` (seed-if-absent, then open — see [Agents Tab — Technical Details](agents_tab_tech.md)), `listRoots`, `addRoot`, `removeRoot`
