...
- `src/main/services/localAgents/promptAssembly.test.ts` (`assembleBareAgentPrompt`) — `AGENT.md` and nothing else in the folder, `README.md` absent, comments stripped, the empty-file stand-in, building mode pointed at `README.md` only when the folder has one (named, never inlined, and read as background rather than followed as setup steps) and at `AGENT.md` otherwise, and **no rule stated about a file a bare folder does not have**
