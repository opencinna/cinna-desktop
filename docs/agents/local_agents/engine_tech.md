...
- `src/renderer/src/components/settings/LocalAgentsSettingsSection.tsx` — the **Runtime** section: the one-line status under the Default runtime picker, which for OpenCode reports the binary (`Ready — opencode <version>, your own installation` / `, from the OpenCode path below` / `, downloaded by Cinna`), *Try again* on `failed` only, and the OpenCode path card below it
...
| `LocalAgentsSettingsSection` | The **Runtime** section: while OpenCode is the selected runtime, the status line under the picker reports the binary — muted when `ready`, `resolving` or not yet resolved, danger on `failed` — with *Try again* as an accent text action on that same line while `failed` or pending, and the OpenCode path card below it. A text action and not a `SettingsButton`: a 29px bordered button arriving in the 21px line grew the row and moved the cards below |
