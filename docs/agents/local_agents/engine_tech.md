...
`CONVERSATION_PERMISSIONS` (`:308`) — `'*': 'ask'` first (OpenCode's base rule is allow-everything, so an enumerated profile without this leaves every other tool on allow), then `read` (`*` allow + `SECRET_FILES`), `edit`/`write` (`*` allow + `IDENTITY_FILES` + `SECRET_FILES`), `bash` (`*` allow, then the `.env` accident guards and `sudo *` / `rm -r *` / `rm -rf *` / `rm -fr *` on ask), `webfetch: 'ask'`, `external_directory: 'ask'`. `SECRET_FILES` (`:246`) is `credentials/.env`, `*.env`, `*.pem`, `*.key` → **deny**; `IDENTITY_FILES` (`:299`) is `cinna-agent.json`, `docs/WORKFLOW_PROMPT.md`, `AGENT.md` → **ask**. <!-- nocheck -->
