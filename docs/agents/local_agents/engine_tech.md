...
- `src/renderer/src/components/settings/LocalAgentsSettingsSection.tsx` — Default → Agents → Runtime keeps the selected-runtime binary status and Try again action. `src/renderer/src/components/settings/DeveloperToolsSettingsSection.tsx` displays the resolved engine version/path under Default → Local Development; `src/renderer/src/components/settings/OpenCodeSettingsFields.tsx` owns its editable override.
...
The CLI launchers are documented separately: [Claude technical details](claude_engine_tech.md) and [Codex technical details](codex_engine_tech.md). Codex uses `CODEX_CONFIG` and its CLI-owned profile rather than generated OpenCode provider configuration.
...
`CONVERSATION_PERMISSIONS` (`:312`) — `'*': 'ask'` first (OpenCode's base rule is allow-everything, so an enumerated profile without this leaves every other tool on allow), then `read` (`*` allow + `SECRET_FILES`), `edit`/`write` (`*` allow + `IDENTITY_FILES` + `SECRET_FILES`), `bash` (`*` allow, then the `.env` accident guards and `sudo *` / `rm -r *` / `rm -rf *` / `rm -fr *` on ask), `webfetch: 'ask'`, `external_directory: 'ask'`. `SECRET_FILES` (`:246`) is `credentials/.env`, `*.env`, `*.pem`, `*.key` → **deny**; `IDENTITY_FILES` (`:301`) is `cinna-agent.json`, `docs/WORKFLOW_PROMPT.md`, `AGENT.md`, `AGENTS.md`, `CLAUDE.md` → **ask**. <!-- nocheck -->
...
- `desktopContextSection(agentDir, context)` — starts in conversation mode, `uv run scripts/<name>.py`, write only under `app-data/` **in conversation mode**, never read or print a credential, locale + time zone, long output to `app-data/storage/`, then `buildingModeSection`, then a kit-only line: keep `cinna-agent.json` and `docs/CLI_COMMANDS.yaml` valid. That line exists because a validator error (`commands.unparseable` included) makes `scannerService.ts` `readiness()` answer `invalid`, and `acpDriver.ts` refuses every turn on an `invalid` folder. The guide clause names `AGENTS.md` (`KIT_BUILDER_GUIDE_FILE`) only when `isFile` finds it, otherwise the coherence rule over `docs/WORKFLOW_PROMPT.md`, `scripts/README.md` and `cinna-agent.json`. The editable set is "whatever in this folder the change needs", naming `docs/`, `scripts/`, `knowledge/`, `config/`, `cinna-agent.json`, the `Makefile` and `pyproject.toml`, and never `app-data/desktop.json` <!-- nocheck -->
- `buildingModeSection(guide, editable)` — returns the `## Building mode` lines both shapes spread into their block: switch on the person's request with no refusal and no confirmation; only a person switches it, never the agent itself or an unattended/handed-over task; say so, then `guide`; may edit `editable`; stay for the rest of the conversation and do the job as the edited files now say. `guide` must end the "say you are switching, then …" sentence
- `isFile(path)` — `statSync().isFile()`, false on any error; the existence check behind both guide choices, so a missing guide is never named
- `assembleBareAgentPrompt(agentDir, name, context)` — `resolveBareInstructionsFile(agentDir)`, then that file (comment-stripped, or the stand-in naming the empty file, or listing the three names when none resolved) → `bareDesktopContextSection`, and nothing else. `README.md` is **not** read here; it belongs to the init prompt. See [Bare Agents & External Roots](bare_agents.md#the-prompt-is-the-instructions-file-and-nothing-else-the-folder-contains)
- `bareDesktopContextSection(agentDir, instructionsFile, context)` — the same block minus the three folder-convention rules (`uv run`, write-only-under-`app-data/`, `credentials/.env`), plus one line deferring to the folder's own instructions; "prefer reading over rewriting" is scoped to conversation mode. The never-print-a-credential rule, the locale line and the long-output rule are kept. Building mode's guide is `README.md` (`BARE_AGENT_README_FILE`) when `isFile` finds it, worded "read … for how this agent is organised and developed — as background, not as setup steps to run" and deliberately not "follow it", otherwise `instructionsFile` (the resolved file, or `AGENT.md` when none resolved); the editable set is that file, the README when present, and anything else the change needs. The README is named, never read
- **When an edited prompt reaches the model** (rule in [engine.md](engine.md#the-desktop-context-block-and-building-mode)). OpenCode: the spec key in `src/main/agents/drivers/acp/acpLaunchers.ts` includes `digestEngineConfig(...).config`, which carries the inline prompt. Codex: `src/main/agents/drivers/acp/codexLauncher.ts` hashes the environment, `CODEX_CONFIG.developer_instructions` included. Claude: the Claude launcher's `specKey([...])` in `acpLaunchers.ts` omits `systemPrompt`, which travels in `session.meta.claudeCode.options`; the adapter's `getOrCreateSession` returns a live session unchanged when `computeSessionFingerprint` (`{cwd, mcpServers}` only) matches (`node_modules/@agentclientprotocol/claude-agent-acp/dist/acp-agent.js`, 0.76.0), so the old prompt holds until `ACP_IDLE_REAP_MS` (`src/main/agents/drivers/acp/types.ts`) reaps the process. `newSession` passes `_meta` straight to `createSession`, so a new chat is current. No test pins this window
...
| `ClaudeStatus` (private) | The shared CLI status row, labeled Claude Code or Codex — the binary, its version, and the login verdict. See [The Claude Engine (technical)](claude_engine_tech.md) |
| `LocalAgentsSettingsSection` | Default → Agents → Runtime reports OpenCode binary state when selected; Try again is an accent text action on the reserved status line while failed or pending. The version/path table and executable override are under Default → Local Development → Developer Tools. |
...
The editable field is **Default → Local Development → Developer Tools → OpenCode Path**. `OpenCodeSettingsFields` represents an untouched field with `draftPath: null`; it follows query data until a user draft exists, including `''`. Blur or Enter trims and saves; a failure displays an unwrapped error and retains the draft. A success releases only the matching draft, so an older save cannot erase newer typing. Escape clears the draft/error and sets `discardingRef` before blur, preventing the discarded value from being saved. The pending note compares both configured path and binary source, so clearing an override also reports that the displayed configured binary is still the previous resolution.

The Developer Tools OpenCode row reads `useEngineBinary`, not the global tool detector. Ready uses the actual resolved version or Unknown; other states read Checking…, Not checked yet or Unavailable, with the failure detail visible. Refresh detects tools; it does not itself resolve/download OpenCode. A saved engine path takes effect **the next time something asks for a binary** — the next turn, or *Try again* — because the resolution is memoised per configured path. Settings says so in place rather than leaving the user to wonder why the version line did not move; the running children are replaced on their own next turns, because the path feeds the spec key.
...
- **The engine-path field has mocked component coverage** in `src/renderer/src/components/settings/OpenCodeSettingsFields.test.tsx` for query/draft ownership, save failure, out-of-order success and Escape. Native binary resolution and full rendered flows remain separate validations.
