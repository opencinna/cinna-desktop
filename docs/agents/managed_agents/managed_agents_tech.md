...
- `LocalAgentsList` exposes Managed entries separately from folder roots; + opens the existing creation chooser. Only its folder creation choice requires Agents Home setup. The Managed modal is keyed by profile and selected agent.
