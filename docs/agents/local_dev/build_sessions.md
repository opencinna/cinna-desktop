...
2. An attention state offers up to three actions, ordered by what ends it. **Reconnect workspace** appears only for `account_mismatch` and leads in the accent: it renames the workspace belonging to the other account aside and sets this one up fresh — see [Reconnect](local_dev.md#reconnect). **Re-authenticate** appears for `account_mismatch` and `token_expired` and runs the ordinary OAuth round trip without leaving the page. **Retry setup** is always last and repairs setup in any attention state; a capability-probe failure is toolchain attention and Repair reinstalls the tools. Only the pressed action shows a spinner, on its own icon rather than in a changed label, and none of them disables behind a running re-authentication. A missing Claude can open the existing install dialog. Codex and OpenCode never do: Cinna fetches both itself, so offering to install a PATH copy would install a tool no session uses, and a failed managed download is retried in Settings → Agents → Runtime. A legacy CLI blocker names **Cinna CLI 0.4.0 or later**, the installed version and **Default → Local Development**, where the managed CLI can be updated to the connected server's advertised version. If that pin is still older, the server administrator must update it. Runtime blockers open the build runtime details; account setup remains under Profile.
