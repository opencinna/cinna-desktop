...
- `src/main/localdev/localDevService.ts` — active-profile reconciliation, `recheckCapabilities` and `executionContext` with managed-tool PATH.
- `src/shared/cinnaCli.ts` — `MIN_JSON_WORKSPACE_CLI_VERSION` (0.4.0) and shared `cliWorkspaceRequirement` guidance; capability results, not a version comparison, select the protocol.
...
- `src/renderer/src/components/localdev/DevelopmentRecheckButton.tsx` — shared explicit check, duplicate-click guard, pending feedback and rejected-action error.
...
`customAgentService.runtime` reuses `desktopStateService.readExternal/patchExternal` for binding-scoped sessions and permission grants. The binding digest covers active profile, owner, row ID and configuration identity. The private file is written before the SQLite session mirror; an old mirror without the matching private session refuses reuse. No `AGENT.md`, manifest or desktop-state file is written into the CLI account workspace by builder preparation. See [external storage](../custom_agents/custom_agents_tech.md#database-schema-and-private-state).
...
|---|---|---|
| `localdev:session-context` / `localDev.sessionContext` | `(fresh?: boolean) → DevelopmentContext` | Activated session; only literal `true` requests fresh managed CLI/capability inspection before main probes the effective runtime |
...
- `developmentContext()` requires a connected profile and ready workspace, resolves the build engine/complexity/credential using shared runtime resolution and the cached catalogue, and assembles the same documents into runtime instructions. Legacy workspace protocol blocks building with `setupTarget = local-dev` and `cliWorkspaceRequirement(state.cliVersion)`: minimum 0.4.0, installed version, Default tooling destination and administrator guidance for an older server pin. Ordinary runtime remedies use `runtime`.
...
- `getDevelopmentSessionContext(probe, fresh = false)` owns the context load, prerequisite probe and final recheck as one service operation. With `fresh`, it first awaits `localDevService.recheckCapabilities(profileId)` and rejects a changed active profile; it never forces setup or writes consent. `developmentRuntimeBlocker` asks the selected launcher's readiness with `fresh: true`; OpenCode separately calls `engineBinaryService.ensure`. After the probe, changed model IDs also reject. An existing context blocker takes priority over a probe result.
...
- `restoreDevelopmentContext(row, options: { fresh?: boolean } = {})` joins `localDevService.reconcile(profileId)` only in idle/installing states for the owning active profile. It checks active profile identity after restoration, then calls `localDevService.recheckCapabilities(profileId)` only when `options.fresh` and the resulting phase is ready, and checks profile identity again afterward. It then warms the catalogue and validates the saved binding. A startup probe previously cached **Finish local development setup…** before restoration finished; waiting fixes that race without automatically retrying settled attention states. A rejected capability check retains its error; a profile change cannot turn a prior account's check into a current result.
- `readAcpRuntime(userId, agent, options?: ReadinessOptions)` in the driver registry is asynchronous. ACP `readiness` forwards its options through `AcpDriverDeps.readRuntime` to `restoreDevelopmentContext(agent, options)` for builders. Saved-chat **Check again** therefore carries `fresh: true` all the way to managed CLI probing; previously that flag reached launcher readiness but was dropped during runtime restoration, leaving a legacy blocker unchanged. Ordinary list checks and turn runtime reads do not request fresh capability probing. The existing custom-launcher readiness error path returns restoration failures as `invalid` with the original reason. A builder projects to an `AcpFolderView` rooted at the account workspace, with the selected engine, while retaining the external runtime's session/grant methods. Its validation composes external ownership/chat checks and development binding checks. The placeholder custom command is never the builder's launch plan.
...
- `useDevelopmentWorkspace` tracks **which** action is running, not merely that one is: `pending: DevelopmentAction | null` (`'send' | 'repair' | 'reconnect' | 'reauth' | 'check' | 'open'`) with `busy = pending !== null`. `runAction(which, fn)` clears `pending` in `finally` only when it is still its own, so an interrupted browser round trip cannot take the spinner off the action that is still running. A single boolean made all three attention buttons announce themselves for one click. `reconnectWorkspace` calls the store action; `reauthenticate` uses `useCinnaReauth().mutateAsync` — not `mutate` with callbacks, so a failure lands in the page's error line — and throws the mutation's own `error` when `success` is false. The mutation's `onSuccess` stays in `useCinnaReauth` and survives unmount; main reconciles local development after a successful re-auth, so the notice clears itself.
- The attention notice's buttons disable on `pending === action || (busy && pending !== 'reauth')`: a re-auth waits on `startOAuthCallback`'s ten-minute window and a tab the user may have closed, so it locks only itself. `ActionIcon` swaps the button's 13 px icon for a `Loader2` rather than changing its label — the row is `flex-wrap`, and a label that widened into "Reconnecting…" would re-wrap it and move the composer under the pointer ([UX Rules](../../development/ui_guidelines/ux_rules.md), rule 1). `DevelopmentComposer` takes `sending` beside `busy` for the same reason: repairing a workspace is not "Starting…".
- `src/renderer/src/components/chat/AgentConnectionDetails.tsx` branches on `development` before generic ACP configuration. It displays the saved engine through `DEVELOPMENT_RUNTIME_NAMES` (Claude Code/Codex/OpenCode, otherwise Not recorded), protocol/location and Local Development ownership directly from public metadata. No custom configuration request, private driver configuration or current-default lookup is needed; see [connection detail lookup](../../chat/chat_routing/chat_routing_tech.md#renderer-components).
- `DevelopmentComposer` alone subscribes to the profile draft. Its textarea auto-sizes to at most 180 px and uses `resize-none`. Settings hides rather than discards the composer; readiness and visibility control autofocus. When active and no dialog owns focus, its effect focuses the restored textarea, sets both selection endpoints to the current value length and scrolls to the last line. It does not rerun on each keystroke. This remains the build-only text store; general chat draft fields are owned by [Conversation UI](../../chat/conversation_ui/conversation_ui_tech.md#draft-ownership). `AmbientGrid` receives that active/ready state and textarea ref; CSS propagates normal/focused border tint, and secondary actions opt into the Shell scheduler. The submit row contains only Start building; Enter/Shift+Enter/composition behavior remains in the textarea handler. See [Appearance](../../ui/appearance/appearance_tech.md). The guide mounts only on demand and memoizes the selected document, keeping Markdown parsing and runtime probes off the keystroke path.
...
- `DevelopmentRecheckButton` uses `SettingsButton` with `aria-busy`, a spinner and **Checking…**. Its ref guard prevents duplicate callbacks; local pending lasts at least 600 ms and also respects the parent query's fetching state. Unmount guards suppress its late state writes. `useDevelopmentWorkspace.checkWorkspace` calls `sessionContext(true)` through `runAction`, then invalidates `local-development-context`; action failures use the page's existing error state. Both the composer warning and runtime details use this callback. The compatibility remedy selects app settings tab `local-dev` (Default), while account setup selects `profile-local-dev`; runtime remedies retain the build details view.
...
- `ComposerWarning` is shared by build setup/action errors and `ComposerReadinessWarning` in ordinary chats. It wraps the full reason, places recovery controls below it, and renders nothing for healthy readiness. The saved-chat readiness action now also uses `SettingsButton`: a 13 px refresh icon, **Checking…**, and focus-preserving `aria-disabled` with guarded duplicate clicks. Its 600 ms feedback only holds an unchanged mounted warning; actual pending can last longer and recovery can remove the warning immediately. Timer cleanup and shared presentation are documented in [Driver readiness renderer behavior](../drivers/drivers_tech.md#readiness-and-renderer-behavior). Warning/danger severity remains defined by the readiness state; catalogue `/run:` refusal exemptions are unchanged.
...
- `src/main/localdev/developmentSessionService.test.ts` — fixed docs and secret/symlink exclusion, inherited/explicit runtime, complexity, stale checks, reuse, startup wait, settled failure and OpenCode warmup; saved-chat legacy recovery only with a fresh request, preserved probe errors and account switches during restoration/recheck.
- `src/main/agents/drivers/index.test.ts`, `src/main/agents/drivers/acp/acpDriver.test.ts`, `src/main/agents/drivers/acp/claudeAuth.test.ts` — production launcher selection/effort, freshness forwarding through builder runtime wiring (including the ordinary undefined-options path), cancellation before runtime/plan settlement and real-vs-false timeout behavior.
...
- `src/renderer/src/components/localdev/DevelopmentRecheckButton.test.tsx` — immediate/slow checks, 600 ms feedback, duplicate suppression and inline rejection; `LocalDevelopmentPage.test.tsx` also covers `sessionContext(true)` from both surfaces and Default tooling navigation.
