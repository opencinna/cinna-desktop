...
### Renderer
- `src/renderer/src/stores/localDev.store.ts` — `useLocalDevStore`: `state`, `subscribed`, `answeredHosts`, `subscribe()`, `set()`, `consent()`, `resetConsent()`, `repair()`, `reconnectWorkspace()`, `openWorkspace()`. `reconnectWorkspace` applies main's reply through the same revision guard `repair` uses and invalidates nothing — the answer *is* the next state
...
| `localdev:reconnect-workspace` | `() → LocalDevState` | Renames the mismatched workspace aside, then `reconcile(force = true)`. The only exit from `attention/account_mismatch`; a refused rename returns an attention state rather than rejecting |
...
`reconnectWorkspace(userId)` runs its rename **inside `enqueueOperation`**, not merely after `inFlight`: a reconcile admitted in the gap would be spawning `cinna account status` with its cwd inside the directory being renamed. Inside the queued step it rechecks profile generation and user ID, re-reads `state` (anything but `attention/account_mismatch` falls through to a plain forced reconcile, so a run that reached `ready` while the click waited is not archived), and resolves `workspacePathFor(userId, host)` again. `isDirectory` guards the rename, `archiveStamp()` produces `YYYYMMDD-HHMMSS` in UTC with no colons, and a `-2`, `-3`… suffix loop avoids renaming onto an existing archive. The step returns one of three things: `'skip'` (profile replaced, or no longer a Cinna user with a server URL — current state is returned unchanged), `'reconcile'`, or a `LocalDevState` it published itself. The last is the refused-rename branch: it `setState`s `attention/account_mismatch` with `Could not move the old workspace aside: <message>` and returns it, because the IPC surface promises a state and neither caller catches. After a `'reconcile'` decision it tail-calls `this.reconcile(userId, true)`.
...
| `ProfileLocalDevSettingsSection` | Profile → Local Development renders every account phase and workspace actions. Consent host comes from consent/declined state or the active profile URL when ready; otherwise the card is absent. `SettingsPage` keys it by account ID to discard page busy state on switches. The card states that accounts on this server share consent. For `attention/account_mismatch` **Reconnect workspace** is the `PrimaryButton` and Repair becomes a `SettingsButton` beside it — demoted, not removed, since it is still the right button for every other reason. `attentionHint(reason)` carries one or two lines per reason, and gains an `account_mismatch` arm; a hint that ran five lines for one reason and two for the others would be the card changing shape according to how it broke |
