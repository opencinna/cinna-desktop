...
6. `toolchain.ensure(pins, onProgress)` / `.repair(…)`, then `toolchain.toolchainEnv(pins)`. A `ToolchainError` goes through `fromToolchainError`
7. `mkdir` the **`Cloud/` parent**, not the workspace — cinna-cli creates and populates the workspace directory, and creating it first would only teach `account setup` that something is already there
...
| `LocalDevOnboardingStep` | Waits for the first answer, falls through on `unsupported` / `declined`, and gives up after `IDLE_GRACE_MS` (8 s). Reads the Agents Home from `localAgents.rootsList()` purely for the consent copy |
