...
- `src/main/ipc/localdev.ipc.ts` — `registerLocalDevHandlers()`; nine request channels, no state push of its own (the service broadcasts)
...
- `src/renderer/src/hooks/useLocalDev.ts` — `useLocalDev()`, subscribes on mount; `useManagedLocalDevCli()` queries the shared installation and refetches on phase changes; `useAddManagedCliToPath()` converts rejected IPC calls into inline refusal results
...
- `src/renderer/src/components/settings/DeveloperToolsSettingsSection.tsx` — detected tools, resolved OpenCode version and bundled contract
...
| `DeveloperToolsSettingsSection` / `OpenCodeSettingsFields` | The tool table excludes runtime assistants and raw detected OpenCode. OpenCode uses `useEngineBinary` state/version/path, while a detected Cinna can come from PATH and legitimately differ from the managed CLI. The override field keeps dirty and empty drafts across query refreshes and save failures; success releases only the draft it saved, and Escape discards without a blur save. See [engine configuration](../local_agents/engine_tech.md#configuration). |
