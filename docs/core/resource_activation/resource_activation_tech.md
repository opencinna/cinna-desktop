...
- `_activate` calls `localDevService.clear()` immediately after closing activation and before any awaited provider reload. Same-user pending requests join before entering `_activate`, so the clear occurs once. Local/default destinations and logout use this same path; deactivation-only cleanup would leave old Cinna work authoritative. `src/main/auth/activation.test.ts` verifies the direct-activation cases. Local-development generation/queue guards are documented in [its technical reference](../../agents/local_dev/local_dev_tech.md#services--key-methods).
