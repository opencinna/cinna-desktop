...
vi.mock('../db/chats', () => ({ chatRepo: { listMessages: () => fixtures.messages, getOwned: () => ({ agentId: 'root' }) } }))
...
describe('fresh session transcript', () => {
  it('replays the full attributed conversation and historical PDF text without repeating the current turn', async () => {
    ...
    const blocks = await replayTranscript('chat', 'current', connection, 'profile', 'root')
  ...
  it('leaves a participant to its catch-up packet', async () => {
    fixtures.messages = [{ id: 'one', role: 'user', content: 'earlier' }, { id: 'current', role: 'user', content: 'now' }]
    expect(await replayTranscript('chat', 'current', connection, 'profile', 'participant')).toEqual([])
  })
  it('replays historical images as native blocks when supported', async () => {
    ...
    expect(await replayTranscript('chat', 'current', connection, 'profile', 'root')).toContainEqual({ type: 'image', data: Buffer.from('image').toString('base64'), mimeType: 'image/png' })
  })
})
