...
describe('scanning a workshop', () => {
  ...
  it('keeps the desktop-owned state out of the renderer’s reach', () => {
    ...
    expect(agent.desktop).toEqual({
      ...
      sessionCount: 1,
      lastStatusAt: null,
      claudeApproval: null, codexApproval: null
    })
...
describe('the launcher a folder row names', () => {
  ...
  it('stays on Claude while the manifest is unparseable', () => {
    // A scan changes readiness, never identity: reading the missing runtime as
    // "the default engine" would send the next turn to the wrong runner.
    const dir = scaffold('alpha')
    setEngine(dir, 'claude')
    const [agent] = scannerService.scanRoot(USER, root).agents
    writeFileSync(manifestPath(dir), '{ "name": "half a fi')
  ...
  it('is what a bare folder’s desktop state names', () => {
    const external = mkdtempSync(join(tmpdir(), 'cinna-external-driver-'))
    try {
      const dir = join(external, 'alpha')
      mkdirSync(dir, { recursive: true })
      writeFileSync(join(dir, 'AGENT.md'), '# Alpha\n')
      desktopStateService.patch(dir, 'bare', { runtime: { engine: 'claude' } })
      const externalRoot = agentRootRepo.create(USER, {
        path: external,
