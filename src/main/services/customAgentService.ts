...
interface Receipt {
  ...
  encryptedToken: Buffer | null; credentialDigest: string
...
/** Private external ACP configuration and initialize-only probes. Tokens never enter the DTO. */
export const customAgentService = {
  launcher,
  configuration(id: string): { name: string; config: CustomAgentConfig; hasAccessToken: boolean; grants: StoredPermissionGrant[] } {
    ...
    return { name: row.name, hasAccessToken: !!row.accessTokenEncrypted, config: parseCustomAgentConfig(row.driverConfig), grants: Object.entries(state.permissionGrants).map(([key, value]) => ({ key, ...value })).sort((a, b) => b.decidedAt - a.decidedAt) }
  },
  ...
  runtime(ownerId: string, supplied: AgentRow): AcpRuntimeView {
    ...
    return {
      ...
      accessToken: config.transport === 'websocket' && row.accessTokenEncrypted ? decryptApiKey(row.accessTokenEncrypted) : undefined,
      validate,
      readiness: async (options) => {
        ...
        catch (error) { return { state: 'unreachable', reason: error instanceof Error ? error.message : 'The ACP agent did not answer.' } }
      },
      readSession: (chatId) => {
        validate(chatId)
        const session = state().sessions[chatId]
        if (session) return session.sessionId
        if (agentSessionRepo.getByChatAndAgent(chatId, row.id)?.contextId) throw new Error('This chat belongs to an earlier ACP configuration or its state is unavailable. Start a new chat.')
        return null
      },
  ...
  async test(input: { id?: string; config: CustomAgentConfig; accessToken?: string }): Promise<CustomAgentTestResult> {
    ...
    const accessToken = this.resolveToken(config, input.accessToken, row)
    const encryptedToken = accessToken ? encryptApiKey(accessToken) : null
    const testingSavedConfiguration = row && digest(config) === digest(parseCustomAgentConfig(row.driverConfig)) &&
      accessToken === this.resolveToken(config, undefined, row)
    const scope = JSON.stringify([ownerId, profileId, input.id ?? 'new'])
    probes.get(scope)?.abort()
    const controller = new AbortController()
    probes.set(scope, controller)
    const validate = (): void => {
      if (controller.signal.aborted || probes.get(scope) !== controller || getProfileScopeUserId() !== profileId || getSettingsScopeUserId() !== ownerId) throw new Error('This ACP configuration test was superseded or the active profile changed.')
      if (input.id) {
        const current = agentRepo.getOwned(ownerId, input.id)
        if (!current || identity(current) !== original) throw new Error('This ACP configuration changed while it was being tested. Reopen it and try again.')
    ...
    try {
      validate()
      const plan = await launcher.plan({ userId: ownerId, agentId: input.id ?? 'preview', custom: config, binding: original ?? undefined, accessToken })
      ...
      const result: CustomAgentTestResult = {
        token: randomUUID(), name: (initialized.agentInfo?.title ?? initialized.agentInfo?.name ?? (config.transport === 'websocket' ? 'Remote ACP agent' : config.command[0])).slice(0, 200),
      ...
      receipts.set(result.token, { ownerId, profileId, id: input.id, original, config, encryptedToken, credentialDigest: digest(accessToken ?? null), result, expires: Date.now() + 10 * 60_000 })
      if (row && testingSavedConfiguration) rememberReadiness(this.stateKey(ownerId, row).binding, { state: 'ok', reason: null })
      return result
    } catch (error) {
      ...
      try {
        validate()
        if (row && testingSavedConfiguration) {
          rememberReadiness(this.stateKey(ownerId, row).binding, { state: 'unreachable', reason: error instanceof Error ? error.message : 'The ACP agent did not answer initialization.' })
        }
  ...
  save(input: { id?: string; name?: string; config: CustomAgentConfig; accessToken?: string; testToken: string }): { id: string } {
    ...
    const ownerId = getSettingsScopeUserId(), profileId = getProfileScopeUserId()
    if (!receipt || receipt.expires < Date.now() || receipt.ownerId !== ownerId || receipt.profileId !== profileId || receipt.id !== input.id || digest(receipt.config) !== digest(config)) throw new Error('Test this exact ACP configuration before saving it.')
    const suppliedToken = this.resolveToken(config, input.accessToken, input.id ? agentRepo.getOwned(ownerId, input.id) ?? null : null)
    if (digest(suppliedToken ?? null) !== receipt.credentialDigest) throw new Error('The access token changed. Test the connection again before saving.')
    ...
    const row = input.id ? agentRepo.updateRuntime(ownerId, input.id, 'acp', { name, config: { ...config }, accessTokenEncrypted: receipt.encryptedToken }) : agentRepo.createRuntime(ownerId, { name, driver: 'acp', config: { ...config }, accessTokenEncrypted: receipt.encryptedToken })
  ...
  resolveToken(config: CustomAgentConfig, supplied: unknown, row: AgentRow | null): string | undefined {
    if (config.transport !== 'websocket') return undefined
    if (supplied !== undefined) return parseAcpAccessToken(supplied)
    // Never forward a saved credential to a newly edited endpoint implicitly.
    if (row?.accessTokenEncrypted) {
      const previous = parseCustomAgentConfig(row.driverConfig)
      if (previous.transport !== 'websocket' || previous.url !== config.url) throw new Error('The endpoint changed. Enter a token for this endpoint, or clear the token field explicitly.')
      return parseAcpAccessToken(decryptApiKey(row.accessTokenEncrypted))
    }
    return undefined
  },
