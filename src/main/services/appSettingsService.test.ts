...
it('validates the routing preference and keeps AI Functions independent', () => {
  ...
  expect(appSettingsService.getAll().defaultMultiAgentRouting).toBe('coordinator')
  expect(() => appSettingsService.set('defaultMultiAgentRouting', 'direct')).toThrow(/You route or AI routes/)
  appSettingsService.set('aiFunctionsCredentialId', 'first')
  appSettingsService.set('aiFunctionsModelId', 'first-model')
  appSettingsService.set('aiFunctionsCredentialId', 'second')
  expect(appSettingsService.getAll().aiFunctionsModelId).toBe('')
  expect(appSettingsService.getAll().localAgentsDefaultCredentialId).toBe('')
})
