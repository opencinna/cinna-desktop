...
describe('createAcpProcessPool', () => {
  it('shares a process across chat aliases while preserving independent holds and retirement', async () => {
    const connection = stubConnection(42)
    const { pool, start, timers } = poolWith([connection])
    const changes = vi.fn()
    pool.onStatus(changes)
    pool.share?.('chat-one', 'mode')
    pool.share?.('chat-two', 'mode')
    const releaseOne = pool.hold('chat-one')
    const releaseTwo = pool.hold('chat-two')
    const [one, two] = await Promise.all([pool.acquire('chat-one', spec('same'), INIT), pool.acquire('chat-two', spec('same'), INIT)])
    expect(one).toBe(two)
    expect(start).toHaveBeenCalledOnce()
    expect(changes).toHaveBeenCalledWith('chat-one', expect.objectContaining({ state: 'running' }))
    expect(changes).toHaveBeenCalledWith('chat-two', expect.objectContaining({ state: 'running' }))
    expect(pool.hasOtherOwners?.('chat-one')).toBe(true)
    pool.retire('chat-one')
    expect(pool.status('chat-one')).toEqual({ state: 'stopped' })
    expect(pool.status('chat-two').state).toBe('running')
  ...
  it('keeps a utility hold alive after the last chat owner leaves', async () => {
    const connection = stubConnection(42)
    const { pool } = poolWith([connection])
    pool.share?.('chat', 'mode')
    await pool.acquire('chat', spec('same'), INIT)
    const releaseUtility = pool.hold('mode')
    expect(pool.hasOtherOwners?.('chat')).toBe(true)
    pool.retire('chat')
    expect(connection.disposals).toBe(0)
    expect(pool.status('chat').state).toBe('stopped')
    releaseUtility()
    expect(connection.disposals).toBe(1)
  })

  it('fans a process exit out to all remaining chat owners', async () => {
    const connection = stubConnection(42)
    const { pool } = poolWith([connection])
    const changes = vi.fn()
    pool.onStatus(changes)
    ...
    expect(changes).toHaveBeenCalledWith('chat-one', expect.objectContaining({ state: 'exited' }))
    expect(changes).toHaveBeenCalledWith('chat-two', expect.objectContaining({ state: 'exited' }))
  })

  it('does not cancel a shared startup when its first chat is canceled', async () => {
    const connection = stubConnection(42)
    let finish!: (connection: AcpConnection) => void
    const start = vi.fn(() => new Promise<AcpConnection>((resolve) => { finish = resolve }))
    ...
    const first = pool.acquire('chat-one', spec('same'), INIT, canceled.signal)
    const second = pool.acquire('chat-two', spec('same'), INIT)
    canceled.abort()
    pool.retire('chat-one')
    releaseOne()
    finish(connection)
    ...
    await first
    expect(start).toHaveBeenCalledWith(spec('same'), INIT)
    expect(connection.disposals).toBe(0)
    releaseTwo()
  })
  ...
  it('rebinds one idle chat to another runtime without changing its former peers', async () => {
    const { pool, start } = poolWith([stubConnection(1), stubConnection(2)])
    pool.share?.('chat-one', 'mode')
    pool.share?.('chat-two', 'mode')
    const first = await pool.acquire('chat-one', spec('same'), INIT)
    pool.share?.('chat-one', 'other-mode')
    const second = await pool.acquire('chat-one', spec('different'), INIT)
    expect(first).not.toBe(second)
    expect(pool.peek?.('chat-two', 'same')).toBe(first)
    expect(first.alive).toBe(true)
    expect(start).toHaveBeenCalledTimes(2)
  })
  it('only peeks a matching live non-retired process and never starts one', async () => {
    const connection = stubConnection(42)
    const { pool, start } = poolWith([connection])
    expect(pool.peek?.('a', 'one')).toBeUndefined()
    expect(start).not.toHaveBeenCalled()
    await pool.acquire('a', spec('one'), INIT)
    expect(pool.peek?.('a', 'one')).toBe(connection)
    expect(pool.peek?.('a', 'two')).toBeUndefined()
