...
export function registerLocalAgentHandlers(): void {
  ...
  ipcHandle(
    ...
    (_event, data: UpdateLocalAgentFieldInput): LocalAgentOutcome<LocalAgentDto> => {
      ...
      /**
       * **Nothing is pushed at the engine, and nothing needs to be.**
       *
       * A saved runtime or a reworded prompt changes what the engine would run,
       * and before phase 3 that meant reconciling one shared `opencode serve`
       * from here — six call sites in this file, each firing an
       * `applyConfigChange` that rebuilt a global config, compared its bytes and
       * restarted a server once every streaming turn had let go of the lock.
       *
       * The ACP driver has no shared process to reconcile. Its launcher
       * generates this agent's config at the top of every turn and the launch
       * spec's `key` is a digest of exactly those bytes, so a change replaces
       * that agent's process on its next turn, by construction — and an
       * in-flight turn keeps the process it started with, which is what waiting
       * on the turn lock used to buy.
       */
      return outcome
    }
  )
