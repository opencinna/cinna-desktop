...
function StartTaskControls({ task }: { task: TaskDto }): React.JSX.Element {
  const { data: agents, isPending: agentsPending } = useAgents()
  const { start, isPending } = useStartTask()
  ...
  const onStart = async (): Promise<void> => {
    ...
    try {
      await start(task.id, selected === 'model' ? { kind: 'model' } : { kind: 'agent', agentId: selected })
    } catch (err) {
      setError(err instanceof Error ? err.message : 'This task could not be started.')
    }
  }
  return (
    <section aria-label="Continue this task" className="rounded-md border border-[var(--color-border)] px-3 py-2">
      <div className="text-xs text-[var(--color-text-secondary)] mb-2">
        Start a conversation with the task’s goal and handoff note.
      </div>
      <div className="flex items-center gap-2">
        <select aria-label="Continue with" value={selected} disabled={isPending}
          onChange={(event) => { setSelected(event.target.value); setError(null) }}
          className="min-w-0 flex-1 rounded-md border border-[var(--color-border)] bg-[var(--color-bg)] px-2 py-1.5 text-xs text-[var(--color-text)]">
          <option value="">Choose who continues</option>
          <option value="model">Default chat model</option>
          {available.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}
        </select>
        <button type="button" disabled={isPending || (agentsPending && selected !== 'model')}
          onClick={() => void onStart()}
          className="inline-flex w-24 shrink-0 items-center justify-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-white disabled:opacity-50">
          {isPending && <Loader2 size={11} className="animate-spin" />}
          Continue
        </button>
      </div>
      <div role={error ? 'alert' : undefined} className="min-h-8 pt-1 text-[11px] leading-4 text-[var(--color-danger)]">{error}</div>
    </section>
  )
}
...
function Attention({ task, asks }: { task: TaskDto; asks: AskState }): React.JSX.Element | null {
  ...
  */
  ...
  if (!task.chatId && task.executor === 'desktop' && task.runsHere &&
    ...
    return <StartTaskControls task={task} />
  }
