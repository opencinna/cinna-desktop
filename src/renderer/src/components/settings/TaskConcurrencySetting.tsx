...
import { SettingsRow, SettingsLabel, settingsDropdownRowClass, settingsInputClass } from './SettingsLayout'
    ...
    <SettingsRow>
      <div className={settingsDropdownRowClass}>
        <SettingsLabel
          ...
          info={
            <p>
              Limit how many tasks and agent turns can run at once on this device. A busy agent
              waits until its current turn finishes.
            </p>
          }
        >
        ...
        <select
          id="task-runner-concurrency"
          className={settingsInputClass}
          disabled={!settings.data || settings.isError || save.isPending}
          value={settings.data?.taskRunnerConcurrency ?? 2}
          onChange={(event) => {
            setError(null)
