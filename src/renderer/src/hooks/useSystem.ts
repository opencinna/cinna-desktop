...
/** What `app:open-external` refuses with. A closed set, both from `app.ipc.ts`. */
export type OpenExternalCode = 'unsupported_protocol' | 'invalid_url'
 ...
 * `app:open-external` returns `unsupported_protocol` or `invalid_url` — which
 ...
 *
 * Both sentences are deliberately short enough to fit one line at the app's
...
export function useOpenExternal(): (url: string) => Promise<OpenExternalResult> {
  return useCallback(async (url: string): Promise<OpenExternalResult> => {
    ...
    return {
      ...
      code: code === 'unsupported_protocol' || code === 'invalid_url' ? code : null
    }
  }, [])
}
